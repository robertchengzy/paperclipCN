...
export const TRUST_PRESET_LABELS: Record<TrustPreset, string> = {
  get standard() { return t("app.lib.trustPolicyUi.presetStandard"); },
  get low_trust_review() { return t("app.lib.trustPolicyUi.presetLowTrustReview"); },
};

export const TRUST_PRESET_DESCRIPTIONS: Record<TrustPreset, string> = {
  get standard() { return t("app.lib.trustPolicyUi.presetStandardDescription"); },
  get low_trust_review() { return t("app.lib.trustPolicyUi.presetLowTrustReviewDescription"); },
};
...
export function summarizeLowTrustBoundaryTarget(
  ...
  if (target?.type === "project") return t("app.lib.trustPolicyUi.boundaryProject", { id: target.id.slice(0, 8) });
  if (target?.type === "root_issue") return t("app.lib.trustPolicyUi.boundaryRootIssue", { id: target.id.slice(0, 8) });
  if (target?.type === "issue") return t("app.lib.trustPolicyUi.boundaryIssue", { id: target.id.slice(0, 8) });
  if (!boundary || countBoundaryTargets(boundary) === 0) return t("app.lib.trustPolicyUi.noBoundary");
  return t("app.lib.trustPolicyUi.boundaries", { count: countBoundaryTargets(boundary) });
}
...
export function sourceTrustLabel(sourceTrust: SourceTrustMetadata | null | undefined) {
  ...
  if (sourceTrust.disposition === "promoted") return t("app.lib.trustPolicyUi.promoted");
  return t("app.lib.trustPolicyUi.lowTrustSource");
}
