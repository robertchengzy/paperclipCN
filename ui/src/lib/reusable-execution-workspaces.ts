...
export function buildReusableExecutionWorkspaceOptionGroups<T extends ReusableExecutionWorkspaceLike>(
  ...
  return [
    ...(recent.length > 0 ? [{ id: "recent" as const, label: t("app.common.labels.recent"), options: recent }] : []),
    { id: "all", label: t("app.workspaces.reusableExecutionWorkspaces.allWorkspaces"), options: all },
  ];
}
