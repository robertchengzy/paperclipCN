...
export function testTaskLinkState(run: {
  ...
  if (run.taskExpired || !run.harnessIssue) {
    return { enabled: false, reason: t("app.skills.skillStudio.testTaskExpired") };
  }
...
export function evaluateRunGate(input: RunGateInput): RunGateResult {
  if (input.skillFileCount <= 0) {
    return { disabled: true, reason: t("app.skills.skillStudio.runGate.noFilesToTest") };
  }
  if (!input.hasAgent) {
    return { disabled: true, reason: t("app.skills.skillStudio.runGate.pickAgent") };
  }
  if (!input.hasInput) {
    return { disabled: true, reason: t("app.skills.skillStudio.runGate.addInput") };
  }
  if (input.hasUnsavedSkillEdits) {
    return { disabled: true, reason: t("app.skills.skillStudio.runGate.saveBeforeRun") };
  }
  if (input.runInFlight) {
    return { disabled: true, reason: t("app.skills.skillStudio.runGate.runInProgress") };
  }
...
function runHarnessUnavailableTitle(reason: CompanySkillTestRunHarnessContentUnavailableReason | null) {
  if (reason === "expired") return t("app.skills.skillStudio.testTaskExpired");
  if (reason === "deleted") return t("app.skills.skillStudio.testTaskDeleted");
  return t("app.skills.skillStudio.testTaskUnavailable");
}
...
export function runHarnessUnavailableCopy(
  ...
  return {
    ...
    body: t("app.skills.skillStudio.harnessUnavailableBody"),
  };
}
...
export function skillEditorAvatar(
  ...
  const name = lastEditor.name?.trim() || t("app.skills.skillStudio.unknownEditor");
  return {
    name,
