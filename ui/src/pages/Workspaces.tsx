...
export function Workspaces() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("app.common.nouns.workspaces") }]);
  }, [setBreadcrumbs, t]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold">{t("app.common.nouns.workspaces")}</h2>
      </div>
      ...
      <SummarySlotCard
        ...
        description={t("app.workspaces.workspaces.summaryDescription")}
      />

      {groups.length === 0 ? (
        <p className="text-sm text-muted-foreground">{t("app.workspaces.workspaces.empty")}</p>
      ) : (
              ...
              <div className="flex flex-wrap items-end justify-between gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {group.summaries.length === 1
                    ? t("app.workspaces.workspaces.oneWorkspace")
                    : t("app.workspaces.workspaces.workspacesCount", { count: group.summaries.length })}
                </span>
              </div>
          ...
          {overviewQuery.hasNextPage ? (
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
              <p className="text-sm text-muted-foreground">
                {t("app.workspaces.workspaces.showingOf", { shown: overviewItems.length, total: totalWorkspaceCount })}
              </p>
              <Button
                type="button"
                variant="outline"
              ...
              >
                {overviewQuery.isFetchingNextPage ? t("app.workspaces.workspaces.loadingDots") : t("app.common.actions.loadMore")}
              </Button>
            </div>
          ) : null}
        </div>
