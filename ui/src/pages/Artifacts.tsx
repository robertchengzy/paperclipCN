...
export const ARTIFACT_KIND_FILTERS: { value: ArtifactKindFilter; label: string }[] = [
  { value: "all", get label() { return t("app.common.labels.all"); } },
  { value: "image", get label() { return t("app.shell.artifacts.images"); } },
  { value: "video", get label() { return t("app.shell.artifacts.videos"); } },
  { value: "document", get label() { return t("app.common.nouns.documents"); } },
  { value: "text", get label() { return t("app.common.labels.text"); } },
  { value: "file", get label() { return t("app.common.labels.files"); } },
];

export const ARTIFACT_GROUP_OPTIONS: { value: ArtifactGroupBy; label: string }[] = [
  { value: "none", get label() { return t("app.common.none"); } },
  { value: "task", get label() { return t("app.common.nouns.task"); } },
  { value: "parent_task", get label() { return t("app.shell.artifacts.parentTask"); } },
];
...
export function artifactGroupByLabel(value: ArtifactGroupBy): string {
  return ARTIFACT_GROUP_OPTIONS.find((option) => option.value === value)?.label ?? t("app.common.none");
}
...
export function Artifacts() {
  ...
  useEffect(() => {
    if (viewingSelectedStack && selectedGroup) {
      setBreadcrumbs([
        { label: t("app.common.nouns.artifacts"), href: "/artifacts" },
        { label: `${selectedGroup.issue.identifier} · ${selectedGroup.title}` },
      ]);
    } else {
      setBreadcrumbs([{ label: t("app.common.nouns.artifacts") }]);
    }
  }, [t, setBreadcrumbs, viewingSelectedStack, selectedGroup]);

  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message={t("app.shell.artifacts.selectAnOrganizationToViewArtifacts")} />;
  }
  ...
  const emptyMessage = showGroupCards
    ? searching
      ? t("app.shell.artifacts.noArtifactStacksMatchThisSearch")
      : t("app.shell.artifacts.noArtifactStacksYet")
    : searching
      ? t("app.shell.artifacts.noArtifactsMatchThisSearch")
      : viewingSelectedStack
        ? t("app.shell.artifacts.noArtifactsInThisStackMatchTheCurrent")
        : kind === "all"
          ? t("app.shell.artifacts.noArtifactsYetOutputsAttachedToIssuesWill")
          : t("app.shell.artifacts.noArtifactsOfThisTypeYet");
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input
            ...
            placeholder={t("app.shell.artifacts.searchArtifacts")}
            aria-label={t("app.shell.artifacts.searchArtifacts2")}
          ...
          {draftQuery.length > 0 ? (
            <button
              ...
              aria-label={t("app.shell.artifacts.clearArtifactSearch")}
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                variant="outline"
                size="icon"
                aria-label={t("app.shell.artifacts.groupArtifactsCurrently", { value0: artifactGroupByLabel(groupBy) })}
                title={t("app.shell.artifacts.groupArtifacts")}
            ...
            <DropdownMenuContent align="end" className="w-44">
              <DropdownMenuLabel>{t("app.shell.artifacts.groupBy")}</DropdownMenuLabel>
          ...
          <div className="flex flex-wrap items-center gap-1.5" role="tablist" aria-label={t("app.shell.artifacts.filterArtifactsByType")}>
      ...
      {viewingSelectedStack ? (
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          >
            ...
            {t("app.shell.artifacts.allStacks")}
          </Link>
          {selectedGroup ? (
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div ref={loadMoreRef} className="flex min-h-10 items-center justify-center pb-2 text-xs text-muted-foreground">
            {isFetchingNextPage
              ? t("app.shell.artifacts.loadingMoreArtifacts")
              : hasNextPage
                ? null
                : isFetching
                  ? t("app.shell.artifacts.updatingArtifacts")
                  : null}
          </div>
        </>
      )}
