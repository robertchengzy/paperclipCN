...
export const CAPABILITY_LABEL: Record<ToolCapability, string> = {
  get read() {
    return t("app.common.labels.readOnly");
  },
  get write() {
    return t("app.tools.profileModel.capability.write");
  },
  get destructive() {
    return t("app.tools.profileModel.capability.destructive");
  },
};
...
export function appSelectionLabel(group: AppGroup, selection: AppSelection | undefined): string {
  ...
  if (state === "unchecked") return t("app.tools.profileModel.selection.none");
  if (selection?.kind === "all" || (selection?.kind === "all_except" && selection.excluded.length === 0)) {
    return t("app.tools.profileModel.selection.all", { name: group.name, total });
  }
  if (selection?.kind === "all_except") {
    const n = selection.excluded.length;
    return t("app.tools.profileModel.selection.allExcept", { name: group.name, count: n });
  }
  const n = selectedToolIds(group, selection).size;
  return t("app.tools.profileModel.selection.some", { count: n, total, name: group.name });
}
...
export const TEMPLATES: TemplateDef[] = [
  {
    ...
    get description() {
      return t("app.tools.profileModel.templates.readOnly.description");
    },
  },
  {
    key: "everyday",
    get title() {
      return t("app.tools.profileModel.templates.everyday.title");
    },
    get description() {
      return t("app.tools.profileModel.templates.everyday.description");
    },
  },
  {
    key: "full_access",
    get title() {
      return t("app.tools.profileModel.templates.fullAccess.title");
    },
    get description() {
      return t("app.tools.profileModel.templates.fullAccess.description");
    },
  },
  {
    key: "scratch",
    get title() {
      return t("app.tools.profileModel.templates.scratch.title");
    },
    get description() {
      return t("app.tools.profileModel.templates.scratch.description");
    },
  },
  {
    key: "copy",
    get title() {
      return t("app.tools.profileModel.templates.copy.title");
    },
    get description() {
      return t("app.tools.profileModel.templates.copy.description");
    },
  },
];
