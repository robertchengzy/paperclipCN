...
export function ProfilesIndex({
  ...
  const duplicate = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("app.tools.profilesIndex.toasts.duplicated"), body: t("app.tools.profilesIndex.toasts.duplicatedBody"), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) =>
      pushToast({ title: t("app.tools.profilesIndex.toasts.duplicateFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const archive = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("app.tools.profilesIndex.toasts.archived"), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) => pushToast({ title: t("app.tools.profilesIndex.toasts.archiveFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const restore = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("app.tools.profilesIndex.toasts.restored"), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) => pushToast({ title: t("app.tools.profilesIndex.toasts.restoreFailed"), body: errorBody(error), tone: "error" }),
  });
  ...
  const header = (
    <ToolsPageHeader
      ...
      description={t("app.tools.profilesIndex.description")}
      actions={
        <>
          <Button variant="outline" onClick={() => setResolverOpen(true)}>
            <ShieldCheck className="mr-1.5 h-4 w-4" />
            {t("app.tools.profilesIndex.checkAccess")}
          </Button>
    ...
    <Sheet open={resolverOpen} onOpenChange={setResolverOpen}>
      <SheetContent className="w-full gap-0 p-0 sm:max-w-xl">
        <SheetHeader className="border-b border-border">
          <SheetTitle>{t("app.tools.profilesIndex.checkAccess")}</SheetTitle>
          <SheetDescription>
            {t("app.tools.profilesIndex.checkAccessDescription")}
          </SheetDescription>
        </SheetHeader>
  ...
  if (profiles.isLoading) {
    return (
      <div className="space-y-5">
        {header}
        <LoadingState label={t("app.tools.profilesIndex.loading")} />
      </div>
    );
  }
  if (profiles.isError) {
    ...
    <div className="space-y-5">
      ...
      <div className="inline-flex rounded-md border border-border p-0.5">
        {(["active", "archived"] as const).map((key) => (
          ...
          >
            {key === "active" ? t("app.common.states.active") : t("app.common.states.archived")}
          </button>
        ))}
      </div>
      ...
      {rows.length === 0 ? (
        statusFilter === "archived" ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-5 text-sm text-muted-foreground">
            {t("app.tools.profilesIndex.noArchived")}
          </div>
        ) : (
          ...
          <table className="w-full table-fixed text-sm">
            ...
            <thead>
              <tr className="border-b border-border bg-muted/40 text-left text-xs font-medium text-muted-foreground">
                <th className="px-3 py-2 font-medium">{t("app.common.nouns.profile")}</th>
                <th className="px-3 py-2 font-medium">{t("app.tools.profilesIndex.columns.allows")}</th>
                <th className="px-3 py-2 font-medium">{t("app.tools.profilesIndex.columns.assignedTo")}</th>
                <th className="px-3 py-2 font-medium">{t("app.common.labels.status")}</th>
                <th className="px-3 py-2 font-medium">{t("app.common.labels.updated")}</th>
                      ...
                      <span className="inline-flex items-center gap-2">
                        ...
                        {(profile.newToolsPendingCount ?? 0) > 0 ? (
                          <Badge variant="outline" className="border-amber-500/50 bg-amber-500/10 text-amber-800 dark:text-amber-200">
                            {t("app.tools.profilesIndex.newToolsCount", { count: profile.newToolsPendingCount })}
                          </Badge>
                        ) : null}
                      </span>
                    </td>
                    <td className="px-3 py-1.5">
                      {assigned.unassigned ? (
                        <span className="text-muted-foreground">
                          {assigned.text}
                          <span className="ml-1 text-xs text-muted-foreground/70">{t("app.tools.profilesIndex.noEffect")}</span>
                        </span>
                      ) : (
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          aria-label={t("app.tools.profilesIndex.rowActions")}
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onSelect={onEdit}>{t("app.common.actions.edit")}</DropdownMenuItem>
        <DropdownMenuItem onSelect={onDuplicate}>{t("app.tools.profilesIndex.duplicate")}</DropdownMenuItem>
        {onRestore ? (
          ...
          <DropdownMenuItem onSelect={onArchive}>{t("app.common.actions.archive")}</DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
  ...
  return (
    <div className="rounded-lg border border-dashed border-border p-6">
      <div className="mb-4 max-w-2xl">
        <h3 className="text-base font-semibold text-foreground">{t("app.tools.profilesIndex.emptyTitle")}</h3>
        <p className="text-sm text-muted-foreground">
          {t("app.tools.profilesIndex.emptyBody")}
        </p>
      </div>
