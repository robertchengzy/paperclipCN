...
export function AppNotConnected() {
  ...
  const appName = application?.name ?? t("app.common.nouns.app");
  useEffect(() => {
    if (!activeTab) return;
    setBreadcrumbs([
  ...
  if (!application) {
    return (
      <div className="max-w-3xl space-y-3 p-6 text-sm text-muted-foreground">
        <p>{t("app.apps.appNotConnected.appNoLongerExists")}</p>
        <Button variant="outline" size="sm" onClick={() => navigate("/apps")}>{t("app.apps.appNotConnected.backToConnectors")}</Button>
      </div>
    );
  }
  if (activeConnection) {
    ...
    <div className="max-w-3xl space-y-6 pb-12">
      ...
      {activeTab === "review" && (
        previousConnection ? (
          ...
          <EmptyTab
            title={t("app.apps.reviewQueueCard.nothingWaiting")}
            body={t("app.apps.appNotConnected.reviewRequestsAppearAfterConnect")}
          />
        )
      )}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center rounded-full border border-border bg-background px-2 py-0.5 text-xs font-medium text-muted-foreground">
            {connectedCount > 0 ? t("app.apps.appNotConnected.connectedCount", { count: connectedCount }) : t("app.common.states.notConnected")}
          </span>
        </div>
        {description && (
    ...
    <section className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border px-4 py-3">
      <div>
        <h2 className="text-sm font-semibold text-foreground">
          {previousConnection ? t("app.common.states.needsAttention") : t("app.common.states.notConnected")}
        </h2>
        <p className="mt-0.5 text-sm text-muted-foreground">
          {previousConnection
            ? previousConnection.authKind === "oauth"
              ? t("app.apps.appNotConnected.signInAgain", { app: applicationName })
              : t("app.apps.appNotConnected.addWorkingKey", { app: applicationName })
            : t("app.apps.appNotConnected.connectSoAgentsCanUse", { app: applicationName })}
        </p>
      ...
      {!previousConnection || canReconnect ? (
        <Button onClick={onConnect}>{previousConnection ? t("app.common.actions.reconnect") : t("app.common.actions.connect")}</Button>
      ) : null}
    </section>
  );
}
...
function PermissionsTab({ previousConnection }: { previousConnection: ToolConnection | null }) {
  ...
  return (
    <section>
      <h2 className="text-sm font-bold text-foreground">{t("app.apps.appNotConnected.permissionsPaused")}</h2>
      <p className="mt-1 text-sm text-muted-foreground">
        {t("app.apps.appNotConnected.permissionsPausedBody")}
      </p>
      {previousConnection && (
        <p className="mt-3 text-xs text-muted-foreground">
          {t("app.apps.appNotConnected.previousSetupRetained")}
        </p>
      )}
    </section>
  );
