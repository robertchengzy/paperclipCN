...
function ExpiryValue({ token }: { token: ToolMcpGatewayToken }) {
  ...
  if (!expiresAt) return <>{t("app.apps.tokensPanel.noExpiry")}</>;
  if (expiresAt.getTime() <= Date.now()) {
    return <><span className="font-medium text-foreground">{t("app.common.states.expired")}</span> <RelativeTime value={token.expiresAt} /></>;
  }
  return <><Trans i18nKey="app.apps.tokensPanel.expiresRelative" components={{ time: <RelativeTime value={token.expiresAt} /> }} /></>;
}
...
export function TokensPanel({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (token) => {
      ...
      pushToast({
        title: t("app.apps.tokensPanel.tokenIssued"),
        body: t("app.apps.tokensPanel.copyItNowYouWonTSee"),
        tone: "success",
      });
    ...
    onError: (error) =>
      pushToast({
        title: t("app.apps.tokensPanel.tokenWasNotIssued"),
  ...
  const revokeMutation = useMutation({
    ...
    onError: (error) =>
      pushToast({
        title: t("app.apps.tokensPanel.tokenWasNotRevoked"),
        body: error instanceof Error ? error.message : String(error),
        tone: "error",
      }),
  });
  ...
  async function copyToken(value: string) {
    try {
      ...
      pushToast({ title: t("app.common.states.copied"), body: t("app.apps.tokensPanel.accessToken"), tone: "success" });
    } catch (error) {
      pushToast({
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="max-w-2xl space-y-1">
          <p className="text-sm text-muted-foreground">{t("app.apps.tokensPanel.issueAReusableTokenForAnExternal")}</p>
          <p className="text-xs text-muted-foreground">{t("app.apps.tokensPanel.paperclipCreatesAFreshOneHourRuntime")}</p>
        </div>
        <Button size="sm" onClick={startIssuing}>
          <Plus className="mr-1.5 h-3.5 w-3.5" />{t("app.apps.tokensPanel.issueToken")}</Button>
      </div>
        ...
        <form className="space-y-3" onSubmit={submit}>
          <div className="grid gap-3 md:grid-cols-[1fr_auto]">
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("app.apps.tokensPanel.tokenName")}</span>
              <Input value={name} onChange={(event) => setName(event.target.value)} required autoFocus />
              <span className="text-xs text-muted-foreground">{t("app.apps.tokensPanel.alsoUsedAsTheClientLabel")}</span>
            </label>
            <label className="space-y-1.5 text-sm">
          ...
          <label className="block space-y-1.5 text-sm">
            <span className="text-xs font-medium text-muted-foreground">{t("app.apps.tokensPanel.ownerNoteOptional")}</span>
            <Input
              value={ownerNote}
              onChange={(event) => setOwnerNote(event.target.value)}
              placeholder={t("app.apps.tokensPanel.whoUsesThisTokenOrWhyIt")}
            />
          </label>
          <div className="flex justify-end gap-2">
            <Button type="button" variant="ghost" size="sm" onClick={() => setMinting(false)}>{t("app.common.actions.cancel")}</Button>
            <Button type="submit" size="sm" disabled={createMutation.isPending || !name.trim()}>
              {createMutation.isPending ? t("app.apps.tokensPanel.issuing") : t("app.apps.tokensPanel.issueToken")}
            </Button>
          </div>
        </form>
      ) : null}
      ...
      {created ? (
        <div className="space-y-2 border-y border-border py-4">
          <div className="flex items-center justify-between gap-2">
            <div>
              <div className="text-sm font-semibold text-foreground">{t("app.apps.tokensPanel.newTokenCopyNow")}</div>
              <div className="text-xs text-muted-foreground">{t("app.apps.tokensPanel.itIsNowAvailableInClientSnippets")}</div>
            </div>
            <Button variant="ghost" size="sm" onClick={() => setCreated(null)} aria-label={t("app.apps.tokensPanel.dismissNewToken")}>{t("app.common.actions.dismiss")}</Button>
          </div>
          <div className="flex items-center gap-2">
            ...
            {revealed ? (
              ...
              <Button variant="outline" size="sm" onClick={() => setRevealed(true)}>{t("app.common.actions.show")}</Button>
            )}
          </div>
        </div>
      ) : null}
      ...
      <Collapsible open={historyOpen} onOpenChange={setHistoryOpen}>
        <CollapsibleTrigger asChild>
          <Button variant="ghost" className="h-auto w-full justify-between px-0 py-1 hover:bg-transparent">
            <span className="flex items-center gap-2 text-sm font-semibold text-foreground">
              <KeyRound className="h-4 w-4 text-muted-foreground" />{t("app.apps.tokensPanel.tokenHistory")}<span className="font-normal text-muted-foreground">{tokens.length}</span>
            </span>
        ...
        <CollapsibleContent className="space-y-3 pt-3">
          {tokens.length === 0 ? (
            <p className="py-4 text-sm text-muted-foreground">{t("app.apps.tokensPanel.noTokensHaveBeenIssuedForThis")}</p>
          ) : (
            <>
                ...
                <table className="w-full min-w-(--sz-44rem) text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/40 text-left text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                      <th className="px-4 py-2.5">{t("app.apps.tokensPanel.token")}</th>
                      <th className="px-4 py-2.5">{t("app.common.nouns.owner")}</th>
                      <th className="px-4 py-2.5">{t("app.common.labels.created")}</th>
                      <th className="px-4 py-2.5">{t("app.common.labels.lastUsed")}</th>
                      <th className="px-4 py-2.5">{t("app.apps.tokensPanel.expiry")}</th>
                        ...
                        <tr key={token.id} className="border-b border-border last:border-0">
                          ...
                          <td className="px-4 py-3 text-right">
                            {status !== "revoked" ? (
                              ...
                              >{t("app.common.actions.revoke")}</Button>
                            ) : null}
                          </td>
                        </tr>
                      );
                  ...
                  return (
                    <div key={token.id} className="border-b border-border py-3 last:border-0">
                      ...
                      <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                        <TokenField label={t("app.common.nouns.owner")} value={token.clientLabel || token.ownerNote || "—"} />
                        <TokenField label={t("app.common.labels.created")} value={<RelativeTime value={token.createdAt} />} />
                        <TokenField label={t("app.common.labels.lastUsed")} value={token.lastUsedAt ? <RelativeTime value={token.lastUsedAt} /> : "—"} />
                        <TokenField label={t("app.apps.tokensPanel.expiry")} value={<ExpiryValue token={token} />} />
                      </dl>
                      {status !== "revoked" ? (
                        <Button
                          variant="ghost"
              ...
              {pageCount > 1 ? (
                <div className="flex items-center justify-between gap-3">
                  <p className="text-xs text-muted-foreground">{t("app.apps.tokensPanel.pagination", { page: historyPage, pages: pageCount, count: tokens.length })}
                  </p>
                  <div className="flex items-center gap-1">
                    <Button
                      ...
                      aria-label={t("app.apps.tokensPanel.previousTokenPage")}
                    >
                    ...
                    <Button
                      ...
                      aria-label={t("app.apps.tokensPanel.nextTokenPage")}
                    >
      ...
      {confirmToken ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md space-y-3 rounded-lg border border-border bg-card p-5 shadow-lg">
            <div>
              <h3 className="text-sm font-semibold text-foreground">{t("app.apps.tokensPanel.revokeThisToken")}</h3>
              <p className="mt-1 text-sm text-muted-foreground"><Trans i18nKey="app.apps.tokensPanel.revokeWarning" values={{ name: confirmToken.name }} components={{ token: <span className="font-medium text-foreground" /> }} /></p>
            </div>
            <Input
              value={revokeName}
              onChange={(event) => setRevokeName(event.target.value)}
              placeholder={confirmToken.name}
              aria-label={t("app.apps.tokensPanel.typeTheTokenNameToConfirm")}
              autoFocus
            />
            <div className="flex justify-end gap-2">
              <Button
              ...
              >{t("app.common.actions.cancel")}</Button>
              <Button
                variant="destructive"
                size="sm"
              ...
              >
                {revokeMutation.isPending ? t("app.apps.tokensPanel.revoking") : t("app.apps.tokensPanel.revokeToken")}
              </Button>
            </div>
          </div>
        </div>
