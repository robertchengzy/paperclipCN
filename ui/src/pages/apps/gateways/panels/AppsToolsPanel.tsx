...
export function AppsToolsPanel({
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">{profile ? t("app.apps.appsToolsPanel.boundProfile", { profile: profile.name, tools: allowedToolsLabel(profile) }) : t("app.apps.appsToolsPanel.noProfile")}</p>

      {apps.length === 0 ? (
        <div className="rounded-md border border-dashed border-border p-6 text-center text-sm text-muted-foreground">{t("app.apps.appsToolsPanel.noAppsAreAssignedToThisGateway")}</div>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-border">
          <table className="w-full min-w-(--sz-32rem) text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40 text-left text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                <th className="px-4 py-2.5">{t("app.common.nouns.app")}</th>
                <th className="px-4 py-2.5">{t("app.common.nouns.tools")}</th>
                ...
                return (
                  <tr key={app.application.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link to={href} className="text-xs font-medium text-primary hover:underline">{t("app.apps.appsToolsPanel.open")}</Link>
                    </td>
                  </tr>
                );
              })}
