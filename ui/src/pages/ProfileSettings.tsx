...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("app.settings.profileSettings.profile") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("app.settings.profileSettings.failedToUpdate"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("app.settings.profileSettings.selectOrganizationBeforeUpload"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("app.settings.profileSettings.failedToUploadAvatar"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("app.settings.profileSettings.failedToRemoveAvatar"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("app.settings.profileSettings.loading")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("app.settings.profileSettings.failedToLoad")}
      </div>
    );
  }
  ...
  const uploadHint = selectedCompany
    ? t("app.settings.profileSettings.storedFor", { name: selectedCompany.name })
    : t("app.settings.profileSettings.selectOrganizationToUpload");
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("app.settings.profileSettings.profile")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("app.settings.profileSettings.description")}
        </p>
      </div>
      ...
      <section className="space-y-8">
            ...
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("app.settings.profileSettings.changePhoto") : t("app.settings.profileSettings.uploadPhoto")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("app.settings.profileSettings.noEmail")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("app.settings.profileSettings.clickAvatarHint", { hint: uploadHint })}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("app.settings.profileSettings.displayName")}</Label>
            <Input
              id="profile-name"
              value={name}
          ...
          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("app.common.labels.email")}</Label>
            <Input
              id="profile-email"
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("app.settings.profileSettings.emailHint")}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("app.settings.profileSettings.saving") : t("app.settings.profileSettings.saveProfile")}
            </Button>
          </div>
        </form>
