...
export function releaseName(release: CompanySkillVersion): string {
  return release.releaseName ?? release.label ?? release.releaseId ?? t("app.skills.agentSkillReleasePicker.release");
}
...
export function releaseOptionLabel(release: CompanySkillVersion): string {
  ...
  return date ? t("app.skills.agentSkillReleasePicker.releasedDate", { name, date }) : name;
}

/** Compact badge label for a pinned release, e.g. `V7`. */
export function releaseShortLabel(release: CompanySkillVersion): string {
  const name = release.releaseName ?? release.label ?? release.releaseId ?? t("app.skills.agentSkillReleasePicker.release");
...
export function AgentSkillReleasePicker({
  ...
  const DEFAULT_LABEL = t("app.skills.agentSkillReleasePicker.defaultLabel");
  ...
  return (
    ...
    >
      <SelectTrigger
        ...
        aria-label={t("app.skills.agentSkillReleasePicker.skillRelease")}
      >
          ...
          <SelectItem key={release.id} value={release.id}>
            <span className="flex items-center gap-2">
              ...
              <Badge variant="secondary" className="shrink-0 text-(length:--text-nano)">
                {t("app.skills.agentSkillReleasePicker.beta")}
              </Badge>
            </span>
          </SelectItem>
        ))}
