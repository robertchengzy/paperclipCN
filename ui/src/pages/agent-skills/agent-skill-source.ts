...
function displayLocalSourceLabel(label: string | null | undefined) {
  ...
  if (!trimmed || isFilesystemLikeLabel(trimmed)) return t("app.skills.agentSkillSource.localFolder");
  return trimmed;
}

function displayCatalogSourceLabel(label: string | null | undefined) {
  const trimmed = label?.trim();
  if (!trimmed || isFilesystemLikeLabel(trimmed)) return t("app.skills.agentSkillSource.catalog");
  return trimmed;
}
...
export function buildAgentSkillSourceMeta(skill: SourceSkill): AgentSkillSourceMeta {
  ...
  if (skill.sourceBadge === "paperclip") {
    return { icon: Paperclip, label: skill.sourceLabel?.trim() || t("app.skills.agentSkillSource.paperclipManaged") };
  }
