...
export function MyUserSecretsTab({ companyId }: { companyId: string }) {
  ...
  const clear = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("app.secrets.myUserSecretsTab.valueCleared"), tone: "info" });
    },
    onError: (err) =>
      pushToast({
        title: t("app.secrets.myUserSecretsTab.clearFailed"),
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-start gap-2 rounded-md border border-violet-500/30 bg-violet-500/5 px-4 py-3 text-xs text-violet-800 dark:text-violet-200">
        ...
        <p>
          {t("app.secrets.myUserSecretsTab.intro")}
          {missingCount > 0 ? (
            <span className="font-medium">
              {" "}
              {missingCount === 1 ? t("app.secrets.myUserSecretsTab.oneRequiredMissing") : t("app.secrets.myUserSecretsTab.manyRequiredMissing", { count: missingCount })}
            </span>
          ) : null}
        </p>
      </div>
      ...
      <div>
        {mySecretsQuery.isError ? (
          <div className="flex items-center gap-2 py-4 text-sm text-destructive">
            ...
            {t("app.secrets.myUserSecretsTab.loadFailed", { message: (mySecretsQuery.error as Error).message })}
          ...
          <EmptyState
            icon={KeyRound}
            message={t("app.secrets.myUserSecretsTab.empty")}
          />
        ) : (
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {!disabledDefinition ? (
          <Button size="sm" variant={secret ? "outline" : "default"} onClick={onSet}>
            {secret ? t("app.common.actions.update") : t("app.secrets.missingUserSecretsBanner.setValue")}
          </Button>
        ) : null}
        {secret ? (
          <Button
            ...
            onClick={onClear}
            disabled={clearing}
            title={t("app.secrets.myUserSecretsTab.clearMyValue")}
          >
