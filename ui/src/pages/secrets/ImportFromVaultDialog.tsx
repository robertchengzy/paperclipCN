...
function statusBadgeLabel(status: RemoteSecretImportCandidate["status"]) {
  switch (status) {
    case "duplicate":
      return translate("app.secrets.importFromVaultDialog.statusImported");
    case "conflict":
      return translate("app.secrets.importFromVaultDialog.statusConflict");
    case "ready":
    default:
      return translate("app.common.states.ready");
  }
}
  ...
  switch (status) {
    case "imported":
      return (
        ...
        >
          <CheckCircle2 className="h-3 w-3" /> {t("app.secrets.importFromVaultDialog.created")}
        </Badge>
      );
    case "skipped":
      return (
        ...
        >
          <Link2 className="h-3 w-3" /> {t("app.secrets.importFromVaultDialog.skipped")}
        </Badge>
      );
    case "error":
    default:
      return (
        ...
        >
          <XCircle className="h-3 w-3" /> {t("app.common.states.failed")}
        </Badge>
      );
  }
}
...
function validateDraftRow(
  ...
  if (!draft.name.trim()) return translate("app.secrets.importFromVaultDialog.nameRequired");
  if (draft.name.length > 160) return translate("app.secrets.importFromVaultDialog.nameTooLong");
  if (!draft.key.trim()) return translate("app.secrets.importFromVaultDialog.keyRequired");
  if (!KEY_PATTERN.test(draft.key)) {
    return translate("app.secrets.importFromVaultDialog.keyFormat");
  }
  if (draft.key.length > 120) return translate("app.secrets.importFromVaultDialog.keyTooLong");
  if (draft.description.length > 500) return translate("app.secrets.importFromVaultDialog.descriptionTooLong");
  ...
  for (const existingSecret of existing) {
    if (existingSecret.name.trim().toLowerCase() === lowerName) {
      return translate("app.secrets.importFromVaultDialog.nameInUse");
    }
    if (existingSecret.key.trim().toLowerCase() === lowerKey) {
      return translate("app.secrets.importFromVaultDialog.keyInUse");
    }
  }
  ...
  for (const other of otherDrafts) {
    ...
    if (other.name.trim().toLowerCase() === lowerName) {
      return translate("app.secrets.importFromVaultDialog.nameInBatch");
    }
    if (other.key.trim().toLowerCase() === lowerKey) {
      return translate("app.secrets.importFromVaultDialog.keyInBatch");
    }
  }
...
export function ImportFromVaultDialog({
  ...
  const importMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      if (result.errorCount === draftList.length && result.errorCount > 0) {
        toast.pushToast({
          title: t("app.secrets.importFromVaultDialog.importFailed"),
          body: t("app.secrets.importFromVaultDialog.noneImported", { vault: vaultName }),
          tone: "error",
        });
      } else {
        toast.pushToast({
          title: result.errorCount > 0 ? t("app.secrets.importFromVaultDialog.importCompletedWithErrors") : t("app.secrets.importFromVaultDialog.importComplete"),
          body: t("app.secrets.importFromVaultDialog.resultCounts", {
  ...
  function handleLoadMore() {
    ...
    secretsApi
      ...
      .catch((error) => {
        toast.pushToast({
          title: t("app.secrets.importFromVaultDialog.loadMoreFailed"),
          body: readableErrorMessage(error),
          tone: "error",
        });
      })
  ...
  function handleClose(force = false) {
    ...
    if (!force && step !== "result" && selection.size > 0 && !importResult) {
      const ok = window.confirm(
        selection.size === 1 ? t("app.secrets.importFromVaultDialog.discardOne") : t("app.secrets.importFromVaultDialog.discardMany", { count: selection.size }),
      );
      if (!ok) return;
    }
    onOpenChange(false);
        ...
        <header className="flex items-start justify-between gap-3 border-b border-border/60 px-5 py-4">
          <div className="flex flex-col gap-1">
            <DialogTitle className="text-base font-semibold">
              {t("app.secrets.importFromVaultDialog.title")}
            </DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">
              {t("app.secrets.importFromVaultDialog.description")}
            </DialogDescription>
            <Stepper step={step} />
          </div>
          <button
            ...
            onClick={() => handleClose()}
            aria-label={t("app.secrets.importFromVaultDialog.closeDialog")}
          >
            <X className="h-4 w-4" />
          </button>
        </header>
        ...
        <footer className="flex items-center justify-between gap-3 border-t border-border/60 bg-muted/20 px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {step === "select" && (
              ...
              >
                {t("app.secrets.importFromVaultDialog.continueReview")}
              </Button>
            )}
            {step === "review" && (
              <Button
              ...
              >
                {importMutation.isPending ? (
                  <>
                    <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> {t("app.secrets.importFromVaultDialog.importing")}
                  </>
                ) : (
                  t("app.secrets.importFromVaultDialog.importCount", { count: draftList.length })
                )}
              </Button>
            )}
            {step === "result" && (
              <Button size="sm" onClick={() => handleClose(true)}>
                {t("app.common.actions.done")}
              </Button>
            )}
          </div>
        </footer>
...
function Stepper({ step }: { step: Step }) {
  ...
  const steps: { id: Step; label: string }[] = [
    { id: "select", label: t("app.common.actions.select") },
    { id: "review", label: t("app.common.actions.review") },
    { id: "result", label: t("app.secrets.importFromVaultDialog.stepResult") },
  ];
  ...
  if (noEligibleVaults) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6" data-testid="select-empty-vaults">
        <EmptyState
          icon={Cloud}
          message={t("app.secrets.importFromVaultDialog.noVaults")}
          action={onManageVaults ? t("app.secrets.importFromVaultDialog.manageVaults") : undefined}
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-5 py-3">
        <label className="text-xs uppercase tracking-wide text-muted-foreground">{t("app.secrets.importFromVaultDialog.vault")}</label>
        {awsVaults.length === 1 && eligible.length === 1 ? (
          ...
          >
            <SelectTrigger size="sm" className="text-xs" aria-label={t("app.secrets.importFromVaultDialog.selectVaultLabel")}>
              <SelectValue placeholder={t("app.secrets.importFromVaultDialog.selectVaultPlaceholder")} />
            </SelectTrigger>
            <SelectContent>
              {awsVaults.map((vault) => {
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {vault.isDefault && (
                        <Badge variant="outline" className="px-1 py-0 text-(length:--text-nano)">{t("app.secrets.importFromVaultDialog.badgeDefault")}</Badge>
                      )}
                      {vault.status === "warning" && (
                        <Badge variant="outline" className="px-1 py-0 text-(length:--text-nano) text-amber-500 border-amber-500/40">{t("app.secrets.importFromVaultDialog.badgeWarning")}</Badge>
                      )}
                      {blocked && (
                        <Badge variant="outline" className="px-1 py-0 text-(length:--text-nano) text-muted-foreground">
                          {vault.status === "coming_soon" ? t("app.secrets.importFromVaultDialog.badgeComingSoon") : vault.status}
                        </Badge>
                      )}
                    </span>
                  </SelectItem>
        ...
        <div className="relative ml-auto w-64">
          ...
          <Input
            ...
            placeholder={t("app.secrets.importFromVaultDialog.searchPlaceholder")}
            className="pl-7 pr-7 text-xs"
            aria-label={t("app.secrets.importFromVaultDialog.searchLabel")}
        ...
        <Button
          ...
          aria-label={t("app.secrets.importFromVaultDialog.refreshLabel")}
        >
      ...
      {selectedNotVisible > 0 && (
        <div className="flex items-center justify-between border-b border-border/60 bg-muted/20 px-5 py-1.5 text-xs text-muted-foreground">
          <span>
            {t("app.secrets.importFromVaultDialog.selectedHidden", { selected: selection.size, hidden: selectedNotVisible })}
          </span>
          <Button
            variant="ghost"
            size="sm"
          ...
          >
            {showOnlySelected ? t("app.secrets.importFromVaultDialog.showAll") : t("app.secrets.importFromVaultDialog.showSelected")}
          </Button>
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" data-testid="vault-table-scroll">
              ...
              <tr>
                <th className="px-3 py-2 text-left">
                  <Checkbox
                    ...
                    aria-label={t("app.secrets.importFromVaultDialog.selectAllLoaded", { count: selectableInLoaded.length })}
                    disabled={selectableInLoaded.length === 0}
                  />
                </th>
                <th className="px-2 py-2 text-left font-medium">{t("app.secrets.importFromVaultDialog.colRemoteName")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("app.secrets.importFromVaultDialog.colReference")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("app.secrets.importFromVaultDialog.colLastChanged")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("app.secrets.importFromVaultDialog.colSuggestedName")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("app.secrets.importFromVaultDialog.colState")}</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        ...
                        aria-label={t("app.secrets.importFromVaultDialog.selectRow", { name: candidate.remoteName })}
                      />
                    </td>
                        ...
                        {candidate.status === "duplicate" &&
                          candidate.conflicts.find((c) => c.type === "exact_reference")?.existingSecretId && (
                            <span className="text-(length:--text-micro) text-muted-foreground">
                              {t("app.secrets.importFromVaultDialog.alreadyImported")}
                            </span>
                          )}
                      </div>
        ...
        {hasNextPage && !previewError && (
          <div className="flex items-center justify-between border-t border-border/60 px-5 py-2 text-xs text-muted-foreground">
            <span>
              {t("app.secrets.importFromVaultDialog.loadedCount", { count: candidates.length })}
              {selectableInLoaded.length > 0 && (
                <span>{t("app.secrets.importFromVaultDialog.selectableCount", { count: selectableInLoaded.length })}</span>
              )}
            </span>
            <Button
              variant="outline"
            ...
            >
              {pageLoading ? (
                <>
                  <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> {t("app.common.loading")}
                </>
              ) : (
                t("app.secrets.importFromVaultDialog.loadMoreCount", { count: PAGE_SIZE })
              )}
            </Button>
          </div>
        )}
    ...
    >
      ...
      <div className="flex-1">
        <div className="font-medium">
          {isPermission
            ? t("app.secrets.importFromVaultDialog.deniedTitle")
            : isThrottling
              ? t("app.secrets.importFromVaultDialog.throttledTitle")
              : t("app.secrets.importFromVaultDialog.loadRemoteFailed")}
        </div>
        <div className="mt-1 text-xs leading-relaxed text-destructive/80">
          {isPermission
            ? t("app.secrets.importFromVaultDialog.deniedBody")
            : message}
        </div>
        <div className="mt-2 flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={onRetry}>
            <RefreshCw className="mr-1.5 h-3.5 w-3.5" /> {t("app.common.actions.retry")}
          </Button>
          {isPermission && (
            <a
            ...
            >
              {t("app.secrets.importFromVaultDialog.iamReference")} <ExternalLink className="h-3 w-3" />
            </a>
          )}
        </div>
      </div>
...
function EmptyCandidates({ query }: { query: string }) {
  ...
  if (query) {
    return (
      <EmptyState
        icon={Search}
        message={t("app.secrets.importFromVaultDialog.noMatch", { query })}
      />
    );
  }
  return (
    <EmptyState
      icon={Database}
      message={t("app.secrets.importFromVaultDialog.noneVisible")}
    />
  );
}
  ...
  if (drafts.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6">
        <EmptyState
          icon={Info}
          message={t("app.secrets.importFromVaultDialog.noneSelected")}
        />
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-3 border-b border-border/60 bg-muted/20 px-5 py-3 text-xs">
        <span className="font-medium">{t("app.secrets.importFromVaultDialog.readyToImport", { count: ready })}</span>
        {blocked > 0 && (
          <span className="text-amber-600 dark:text-amber-400">
            {t("app.secrets.importFromVaultDialog.needAttention", { count: blocked })}
          </span>
        )}
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1 space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("app.secrets.importFromVaultDialog.paperclipName")}</span>
                      <Input
                        value={draft.name}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("app.common.labels.key")}</span>
                      <Input
                        value={draft.key}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("app.secrets.importFromVaultDialog.descriptionOptional")}</span>
                      <Input
                ...
                <Button
                  ...
                  aria-label={t("app.secrets.importFromVaultDialog.removeRow", { name: draft.candidate.remoteName })}
                  className="h-7 w-7"
                  disabled={importing}
                >
...
function ResultStep({ result, draftList }: ResultStepProps) {
  ...
  const heading =
    result.errorCount === result.results.length && result.errorCount > 0
      ? t("app.secrets.importFromVaultDialog.importFailed")
      : result.errorCount === 0 && result.skippedCount === 0
        ? t("app.secrets.importFromVaultDialog.allImported", { count: result.importedCount })
        : t("app.secrets.importFromVaultDialog.importComplete");
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-border/60 px-5 py-3" data-testid="result-summary">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="text-emerald-600 dark:text-emerald-400">✓ {t("app.secrets.importFromVaultDialog.createdCount", { count: result.importedCount })}</span>
          <span>⊘ {t("app.secrets.importFromVaultDialog.skippedCount", { count: result.skippedCount })}</span>
          <span className="text-destructive">⨯ {t("app.secrets.importFromVaultDialog.failedCount", { count: result.errorCount })}</span>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {grouped.created.length > 0 && (
          <ResultGroup label={t("app.secrets.importFromVaultDialog.created")} rows={grouped.created} draftLookup={draftLookup} />
        )}
        {grouped.skipped.length > 0 && (
          <ResultGroup label={t("app.secrets.importFromVaultDialog.skipped")} rows={grouped.skipped} draftLookup={draftLookup} />
        )}
        {grouped.failed.length > 0 && (
          <ResultGroup label={t("app.common.states.failed")} rows={grouped.failed} draftLookup={draftLookup} />
        )}
      </div>
    </div>
  );
  ...
  if (step === "select") {
    return (
      <div className="text-xs text-muted-foreground">
        {totalSelected === 0
          ? t("app.secrets.importFromVaultDialog.selectPrompt")
          : t("app.secrets.importFromVaultDialog.selectedCount", { count: totalSelected })}
      </div>
    );
  }
  if (step === "review") {
    return (
      <div className="text-xs text-muted-foreground">
        {t("app.secrets.importFromVaultDialog.readyCount", { count: readyReviewCount })}
        {blockedReviewCount > 0 && (
          <span className="ml-2 text-amber-600 dark:text-amber-400">
            {t("app.secrets.importFromVaultDialog.blockedCount", { count: blockedReviewCount })}
          </span>
        )}
      </div>
    );
  ...
  if (result) {
    return (
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>{t("app.secrets.importFromVaultDialog.createdCount", { count: result.importedCount })}</span>
        <span>{t("app.secrets.importFromVaultDialog.skippedCount", { count: result.skippedCount })}</span>
        <span>{t("app.secrets.importFromVaultDialog.failedCount", { count: result.errorCount })}</span>
      </div>
    );
  }
  return null;
