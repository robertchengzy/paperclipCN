...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    case "skills_sh":
      return { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: translate("app.skills.companySkills.skillsShManaged") };
    case "github":
      return isSkillsShManaged
        ? { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: translate("app.skills.companySkills.skillsShManaged") }
        : { icon: GithubIcon, label: sourceLabel ?? "GitHub", managedLabel: translate("app.skills.companySkills.githubManaged") };
    case "url":
      return { icon: Link2, label: sourceLabel ?? "URL", managedLabel: translate("app.skills.companySkills.urlManaged") };
    case "local":
      return { icon: Folder, label: sourceLabel ?? translate("app.common.labels.folder"), managedLabel: translate("app.skills.companySkills.folderManaged") };
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Paperclip", managedLabel: translate("app.skills.companySkills.paperclipManaged") };
    default:
      return { icon: Boxes, label: sourceLabel ?? translate("app.skills.companySkills.catalogLabel"), managedLabel: translate("app.skills.companySkills.catalogManaged") };
  }
}
...
function formatProjectScanSummary(result: CompanySkillProjectScanResult) {
  const parts = [
    translate("app.skills.companySkills.scanSummary.found", { count: result.discovered }),
    translate("app.skills.companySkills.scanSummary.imported", { count: result.imported.length }),
    translate("app.skills.companySkills.scanSummary.updated", { count: result.updated.length }),
  ];
  if (result.conflicts.length > 0) parts.push(translate("app.skills.companySkills.scanSummary.conflicts", { count: result.conflicts.length }));
  if (result.skipped.length > 0) parts.push(translate("app.skills.companySkills.scanSummary.skipped", { count: result.skipped.length }));
  const summary = parts.join(translate("app.skills.companySkills.scanSummary.separator"));
  return result.scannedWorkspaces === 1
    ? translate("app.skills.companySkills.scanSummary.oneWorkspace", { summary, count: result.scannedWorkspaces })
    : translate("app.skills.companySkills.scanSummary.manyWorkspaces", { summary, count: result.scannedWorkspaces });
}
...
const SOURCE_FILTER_LABEL_KEYS: Record<SourceFilter, string> = {
  all: "app.common.labels.all",
  company: "app.common.nouns.organization",
  bundled: "app.skills.skillFolderTree.bundled",
  optional: "app.common.labels.optional",
  external: "app.common.labels.external",
};
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={activeFilterCount > 0 ? t("app.skills.companySkills.filtersCount", { count: activeFilterCount }) : t("app.common.actions.filter")}
        >
      ...
      <DropdownMenuContent align="end" className="w-48">
        <DropdownMenuLabel>{t("app.common.labels.source")}</DropdownMenuLabel>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="max-h-(--sz-calc-32) w-56 overflow-y-auto">
        <DropdownMenuLabel>{t("app.common.labels.type")}</DropdownMenuLabel>
        <DropdownMenuRadioGroup value={kindFilter} onValueChange={(next) => onKindChange(next as "all" | "bundled" | "optional")}>
          <DropdownMenuRadioItem value="all">{t("app.common.labels.all")}</DropdownMenuRadioItem>
          <DropdownMenuRadioItem value="bundled">{t("app.skills.skillFolderTree.bundled")}</DropdownMenuRadioItem>
          <DropdownMenuRadioItem value="optional">{t("app.common.labels.optional")}</DropdownMenuRadioItem>
        </DropdownMenuRadioGroup>
        <DropdownMenuSeparator />
        <DropdownMenuLabel>{t("app.skills.companySkills.category")}</DropdownMenuLabel>
        <DropdownMenuRadioGroup value={categoryFilter || "__all__"} onValueChange={(next) => onCategoryChange(next === "__all__" ? "" : next)}>
          <DropdownMenuRadioItem value="__all__">{t("app.skills.companySkills.allCategories")}</DropdownMenuRadioItem>
...
function TrustChip({ level }: { level: CompanySkillTrustLevel }) {
  ...
  const map = {
    markdown_only: {
      icon: ShieldCheck,
      label: t("app.skills.companySkills.trust.markdownOnly"),
      tooltip: t("app.skills.companySkills.trust.markdownOnlyTooltip"),
      className: "border-border bg-muted/40 text-muted-foreground",
    },
    assets: {
      icon: Folder,
      label: t("app.skills.companySkills.trust.assets"),
      tooltip: t("app.skills.companySkills.trust.assetsTooltip"),
      className: "border-cyan-500/30 bg-cyan-500/10 text-cyan-800 dark:text-cyan-200",
    },
    scripts_executables: {
      icon: AlertTriangle,
      label: t("app.skills.companySkills.trust.scripts"),
      tooltip: t("app.skills.companySkills.trust.scriptsTooltip"),
...
function CompatChip({ compatibility }: { compatibility: CompanySkillCompatibility }) {
  ...
  const map = {
    unknown: {
      icon: HelpCircle,
      label: t("app.skills.companySkills.compat.unknown"),
      tooltip: t("app.skills.companySkills.compat.unknownTooltip"),
      className: "border-yellow-500/40 bg-yellow-500/10 text-yellow-800 dark:text-yellow-200",
    },
    invalid: {
      icon: XOctagon,
      label: t("app.skills.companySkills.compat.invalid"),
      tooltip: t("app.skills.companySkills.compat.invalidTooltip"),
...
function ProvenanceBadge({ packageName, packageVersion }: { packageName: string | null; packageVersion: string | null }) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>{t("app.skills.companySkills.provenanceTooltip")}</TooltipContent>
    </Tooltip>
  );
}
...
export function skillDetailBreadcrumbs(
  ...
  const trail = detail.folderId
    ...
    : [];
  return [
    { label: translate("app.common.nouns.skills"), href: "/skills" },
...
const DISCOVERY_SORT_LABEL_KEYS: Record<DiscoverySort, string> = {
  agents: "app.skills.companySkills.sort.agents",
  stars: "app.skills.companySkills.sort.stars",
  forks: "app.skills.companySkills.sort.forks",
  recent: "app.common.labels.recentlyUpdated",
  alphabetical: "app.common.labels.alphabetical",
};
...
function skillSettingsToastBody(skill: Pick<CompanySkillDetail, "categories" | "sharingScope">) {
  const sharing = skill.sharingScope === "private"
    ? translate("app.skills.companySkills.settingsToast.sharingPrivate")
    : translate("app.skills.companySkills.settingsToast.sharingOrganization");
  const categories = skill.categories.length
    ? translate("app.skills.companySkills.settingsToast.categories", { categories: skill.categories.join(", ") })
    : translate("app.skills.companySkills.settingsToast.categoriesNone");
...
export function buildDiscoveryCards(
  ...
  for (const skill of installedByKey.values()) {
    ...
    cards.push({
      ...
      name: skill.name,
      slug: skill.slug,
      author: skill.authorName ?? skill.sourceLabel ?? translate("app.skills.companySkills.authorYou"),
  ...
  for (const [identity, entry] of catalogByKey) {
    ...
    cards.push({
      ...
      updatedAt: 0,
      sourceBadge: "catalog",
      sourceLabel: entry.packageName ?? translate("app.skills.companySkills.catalogLabel"),
    ...
    >
      <div className="flex items-start gap-3">
        {selectMode && canMove ? (
          <input
            ...
            checked={selected}
            aria-label={t("app.skills.companySkills.selectCard", { name: card.name })}
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-xs text-muted-foreground">
            {t("app.skills.companySkills.byAuthor", { author: card.author })}{card.version ? ` · ${card.version}` : ""}
          </div>
          {badgeFolder !== undefined ? (
            <div className="mt-1 flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
              <FolderSwatch color={badgeFolder?.color} className="h-2 w-2" />
              <span className="truncate">{badgeFolder ? badgeFolder.name : t("app.skills.skillFolderTree.unfiled")}</span>
            </div>
          ) : null}
        </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                aria-label={t("app.skills.companySkills.moreActionsFor", { name: card.name })}
                onClick={(event) => event.stopPropagation()}
              >
              ...
              {onOpenMove ? (
                <>
                  <DropdownMenuItem onSelect={() => onOpenMove(card)}>
                    ...
                    {t("app.skills.companySkills.moveToFolder")}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                </>
              ) : null}
      ...
      {card.forkedFrom ? (
        <div className="mt-2 inline-flex items-center gap-1 text-(length:--text-micro) text-muted-foreground">
          ...
          {t("app.skills.companySkills.forked")}
        </div>
      ) : null}
        ...
        <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
          <span>
            {card.installed
              ? card.agentCount > 0
                ? card.agentCount === 1
                  ? t("app.skills.companySkills.enabledForOne", { count: card.agentCount })
                  : t("app.skills.companySkills.enabledForMany", { count: card.agentCount })
                : t("app.skills.companySkills.notEnabled")
              : t("app.skills.companySkills.availableToInstall")}
          </span>
          {card.starCount > 0 ? (
            <>
...
function CategoryNav({
  ...
  return (
    <nav className="flex flex-col gap-0.5 px-2">
      ...
      >
        <span>{t("app.common.labels.all")}</span>
...
export function DiscoveryGrid({
  ...
  const viewTitle = installedView ? t("app.skills.companySkills.installedSkills") : t("app.skills.companySkills.discoverSkills");
  const viewDescription = installedView
    ? t("app.skills.companySkills.installedDescription")
    : t("app.skills.companySkills.discoverDescription");
  const searchLabel = installedView ? t("app.skills.companySkills.searchInstalled") : t("app.skills.companySkills.searchDiscoverable");
  ...
  const availableSources = useMemo(() => {
    ...
    for (const card of cards) {
      if (card.sourceKind) {
        facets.set(`kind:${card.sourceKind}`, card.sourceKind === "bundled" ? t("app.skills.skillFolderTree.bundled") : t("app.common.labels.optional"));
      }
      if (card.sourceBadge) {
      ...
      {showBrowseRails ? (
        <aside className={cn("hidden w-60 shrink-0 flex-col overflow-hidden border-r border-border md:flex", showFolderRail && "md:hidden")}>
          <div className="border-b border-border px-4 py-4">
            <h2 className="text-sm font-semibold text-foreground">{t("app.skills.companySkills.browseByCategory")}</h2>
            <p className="text-xs text-muted-foreground">
              {installedView ? t("app.skills.companySkills.filterInstalled") : t("app.skills.companySkills.filterDiscoverable")}
            </p>
          </div>
          <div className="px-4 pb-1 pt-3 text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
            {t("app.common.labels.categories")}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm">
                <span className="text-muted-foreground">{t("app.common.actions.sort")}</span>
          ...
          {availableSources.length > 1 ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm">
                  <span className="text-muted-foreground">{t("app.common.labels.source")}</span>
                  <span className="ml-1.5">
                    {sourceBadgeFilter === "all"
                      ? t("app.common.labels.all")
                      : availableSources.find((source) => source.value === sourceBadgeFilter)?.label ?? t("app.common.labels.all")}
                  </span>
                  <ChevronDown className="ml-1 h-3.5 w-3.5" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuRadioGroup value={sourceBadgeFilter} onValueChange={setSourceBadgeFilter}>
                  <DropdownMenuRadioItem value="all">{t("app.skills.companySkills.allSources")}</DropdownMenuRadioItem>
          ...
          <Button
            ...
            aria-label={t("app.skills.companySkills.scanWorkspaces")}
            title={t("app.skills.companySkills.scanWorkspaces")}
          >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onSelect={onCreate}>
                ...
                {t("app.skills.companySkills.createNewSkill")}
              </DropdownMenuItem>
              ...
              <DropdownMenuItem onSelect={onImportFromProject}>
                ...
                {t("app.skills.companySkills.importFromProject")}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          {showBrowseRails && folderResult && onFolderSelect ? (
            <div className="w-full md:hidden">
              <FolderChip
                result={folderResult}
                selection={folderSelection}
                allLabel={t("app.skills.skillFolderTree.allSkills")}
          ...
          {onCreateFolder && !showFolderRail ? (
            <Button variant="outline" size="sm" onClick={onCreateFolder}>
              <Plus className="mr-1 h-3.5 w-3.5" />
              {t("app.skills.skillFolderTree.newFolder")}
            </Button>
          ) : null}
          {onToggleSelectMode ? (
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="w-full justify-between">
                  <span className="capitalize">{activeCategory ?? t("app.skills.companySkills.allCategories")}</span>
              ...
              <DropdownMenuContent align="start" className="max-h-72 overflow-auto">
                ...
                >
                  <DropdownMenuRadioItem value="__all__">{t("app.skills.companySkills.allWithCount", { count: categoryTotal })}</DropdownMenuRadioItem>
                  {categories.map((category) => (
            ...
            <div className="mb-4 flex items-center justify-between gap-2">
              ...
              {activeProjectFolder && activeProjectId ? (
                <Button
                  ...
                  disabled={scanPending}
                  aria-label={t("app.skills.companySkills.refreshProjectSkills", { name: activeProjectFolder.name })}
                  title={t("app.skills.companySkills.refreshSkillsFrom", { name: activeProjectFolder.name })}
                >
            ...
            <div className="py-12">
              <EmptyState
                ...
                message={
                  totalCount === 0
                    ? installedView
                      ? t("app.skills.companySkills.emptyInstalled")
                      : t("app.skills.companySkills.emptyDiscover")
                    : search || activeCategory || sourceFilterActive
                      ? t("app.skills.companySkills.emptyFiltered")
                      : t("app.skills.companySkills.emptyView")
                }
              />
              {totalCount === 0 ? (
            ...
            <>
              <p className="mb-3 text-xs text-muted-foreground">
                {sourceFilteredCards.length === 1
                  ? t("app.skills.companySkills.oneSkill", { count: sourceFilteredCards.length })
                  : t("app.skills.companySkills.manySkills", { count: sourceFilteredCards.length })}
...
function NewSkillWizard({
  ...
  const steps = [t("app.skills.companySkills.wizard.basics"), t("app.skills.companySkills.wizard.design"), t("app.skills.companySkills.wizard.content"), t("app.skills.companySkills.wizard.review")];
    ...
    <div className="space-y-4">
      ...
      {draft.forkedFromName ? (
        <div className="flex items-center gap-2 rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
          ...
          {t("app.skills.companySkills.wizard.forking", { name: draft.forkedFromName })}
        </div>
      ) : null}
      ...
      {step === 0 ? (
        <div className="space-y-3">
          <Input
            ...
            placeholder={t("app.skills.companySkills.wizard.skillName")}
            className="h-9"
          />
          <Input
            value={draft.slug}
          ...
          <Textarea
            ...
            placeholder={t("app.skills.companySkills.wizard.tagline")}
            className="min-h-20"
          />
        </div>
      ) : step === 1 ? (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <SkillCardIcon
              ...
              card={{
                ...
                name: draft.name || t("app.skills.companySkills.wizard.newSkill"),
            ...
            <div className="min-w-0">
              <div className="truncate text-sm font-medium">{draft.name || t("app.skills.companySkills.wizard.newSkill")}</div>
              <div className="truncate text-xs text-muted-foreground">{draft.tagline || t("app.skills.companySkills.wizard.noTagline")}</div>
            </div>
          </div>
          <div>
            <label className="mb-2 block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.skills.companySkills.wizard.color")}</label>
            <div className="flex flex-wrap gap-2">
              {SKILL_CREATE_ACCENTS.map((color) => (
                <button
                  ...
                  aria-label={t("app.skills.companySkills.wizard.useColor", { color })}
                />
              ))}
              <Input
                value={draft.color}
          ...
          <div>
            <label className="mb-2 block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.common.labels.categories")}</label>
            <Input
              value={categoryDraft}
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-(--gtc-26) gap-y-2">
            <span className="text-muted-foreground">{t("app.common.labels.name")}</span>
            <span>{draft.name || t("app.skills.companySkills.wizard.untitled")}</span>
            <span className="text-muted-foreground">{t("app.skills.companySkills.wizard.slug")}</span>
            <span className="font-mono">{effectiveSlug || "skill"}</span>
            <span className="text-muted-foreground">{t("app.common.labels.scope")}</span>
            <span>{draft.sharingScope === "private" ? t("app.common.labels.private") : t("app.common.nouns.organization")}</span>
            <span className="text-muted-foreground">{t("app.common.labels.categories")}</span>
            <span>{draft.categories.length ? draft.categories.join(", ") : t("app.skills.companySkills.wizard.none")}</span>
          </div>
          <div className="space-y-2">
            <label className="block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.skills.companySkills.wizard.sharing")}</label>
            <div className="grid gap-2 sm:grid-cols-3">
              {(["company", "private"] as const).map((scope) => (
                ...
                >
                  <span className="block font-medium">{scope === "company" ? t("app.common.nouns.organization") : t("app.common.labels.private")}</span>
                  <span className="mt-1 block text-xs text-muted-foreground">
                    {scope === "company" ? t("app.skills.companySkills.wizard.visibleOrganization") : t("app.skills.companySkills.wizard.visiblePrivate")}
                  </span>
                </button>
              ))}
              <button
              ...
              >
                <span className="block font-medium">{t("app.skills.companySkills.wizard.publicLink")}</span>
                <span className="mt-1 block text-xs">{t("app.skills.companySkills.wizard.comingLater")}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {step < steps.length - 1 ? (
            <Button size="sm" onClick={() => setStep((value) => Math.min(steps.length - 1, value + 1))} disabled={!nameValid}>
              {t("app.common.actions.next")}
            </Button>
          ) : (
            <Button size="sm" onClick={submit} disabled={isPending || !nameValid}>
              {isPending ? t("app.common.progress.creating") : draft.forkedFromSkillId ? t("app.skills.companySkills.wizard.createFork") : t("app.skills.companySkills.wizard.createSkill")}
            </Button>
          )}
        </div>
      </div>
  ...
  if (filtered.length === 0) {
    return (
      <div className="px-4 py-6 text-sm text-muted-foreground">
        {t("app.skills.companySkills.catalog.noMatch")}
      </div>
    );
  }
      ...
      <div key={skill.id} className="border-b border-border">
        ...
        >
          ...
          <button
            ...
            aria-label={expanded ? t("app.skills.companySkills.collapseName", { name: skill.name }) : t("app.skills.companySkills.expandName", { name: skill.name })}
          >
  ...
  return (
    <div>
      {bundled.length > 0 && kindFilter !== "optional" ? (
        <div>
          <div className="border-b border-border bg-background px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("app.skills.companySkills.catalog.bundledCount", { count: bundled.length })}
          </div>
      ...
      {optional.length > 0 && kindFilter !== "bundled" ? (
        <div>
          <div className="border-b border-border bg-background px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("app.skills.companySkills.catalog.optionalCount", { count: optional.length })}
          </div>
      ...
      {installed.length > 0 ? (
        <div>
          <div className="border-b border-border bg-background px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            {t("app.skills.companySkills.catalog.installedCount", { count: installed.length })}
          </div>
...
function CatalogDetailPane({
  ...
  const { t } = useTranslation();
  if (!skill) {
    return <EmptyState icon={Boxes} message={t("app.skills.companySkills.catalog.selectSkill")} />;
  }
  ...
  if (skill.compatibility === "invalid") {
        ...
        <TooltipTrigger asChild>
          <span>
            <Button disabled>
              ...
              {t("app.skills.companySkills.installSkill")}
            </Button>
          </span>
        </TooltipTrigger>
        <TooltipContent>{t("app.skills.companySkills.catalog.invalidTooltip")}</TooltipContent>
      </Tooltip>
    );
  } else if (!isInstalled) {
    cta = (
      <Button onClick={onInstall} disabled={loadingPrimaryAction}>
        ...
        {loadingPrimaryAction ? t("app.skills.companySkills.catalog.preparing") : t("app.skills.companySkills.catalog.installInOrganization")}
      </Button>
    );
    ...
    cta = (
      <Button onClick={onUpdate} disabled={loadingPrimaryAction} className="border-amber-500/40 bg-amber-500/20 text-amber-900 dark:text-amber-100 hover:bg-amber-500/30">
        ...
        {t("app.skills.companySkills.catalog.updateFromCatalog")}
      </Button>
    );
  } else {
    cta = (
      <Button variant="ghost" onClick={() => installedSkillId && onOpenInstalled(installedSkillId)}>
        ...
        {t("app.skills.companySkills.catalog.installedOpen")}
      </Button>
    );
  }
    ...
    <div className="min-w-0">
      <div className="border-b border-border px-5 py-4">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Badge variant="outline" className="border-amber-500/40 bg-amber-500/10 text-(length:--text-micro) text-amber-800 dark:text-amber-200">
                  ...
                  {t("app.common.states.updateAvailable")}
                </Badge>
              </TooltipTrigger>
              <TooltipContent>{t("app.skills.companySkills.catalog.hashChanged")}</TooltipContent>
            </Tooltip>
          ) : null}
          {skill.requires.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("app.skills.companySkills.catalog.requires", { value: skill.requires.join(", ") })}
            </Badge>
          ) : null}
          {skill.recommendedForRoles.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("app.skills.companySkills.catalog.roles", { value: skill.recommendedForRoles.join(" · ") })}
            </Badge>
          ) : null}
          {skill.tags.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("app.skills.companySkills.catalog.tags", { value: skill.tags.join(" · ") })}
            </Badge>
          ) : null}
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          ...
          <CopyText
            ...
            copiedLabel={t("app.skills.companySkills.catalog.copiedHash")}
            ariaLabel={t("app.skills.companySkills.catalog.copyHash")}
            title={t("app.skills.companySkills.catalog.copyHash")}
      ...
      <div className="min-h-(--sz-400px) px-5 py-5">
        {fileQuery.isLoading ? (
          ...
          <div className="text-sm text-destructive">{fileQuery.error instanceof Error ? fileQuery.error.message : t("app.skills.companySkills.failedToLoadFile")}</div>
        ) : !fileQuery.data ? (
...
export function InstallPreviewDialog({
  ...
  let confirmLabel = t("app.skills.companySkills.installSkill");
  let confirmVariant: "default" | "destructive" = "default";
  if (defaultAction === "update") {
    confirmLabel = t("app.skills.companySkills.install.installUpdate");
  } else if (defaultAction === "replace") {
    confirmLabel = t("app.skills.companySkills.install.replaceExisting");
    confirmVariant = "destructive";
  }
  if (isPending) confirmLabel = t("app.common.progress.installing");
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => (!isPending ? onOpenChange(value) : null)}>
      <DialogContent className="sm:max-w-2xl" showCloseButton={!isPending}>
        <DialogHeader>
          <DialogTitle>
            {defaultAction === "update" ? t("app.common.actions.update") : defaultAction === "replace" ? t("app.skills.companySkills.install.replace") : t("app.common.actions.install")} · {skill.name}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4 text-sm">
          <div className="rounded-md border border-border p-3">
            <div className="grid grid-cols-(--gtc-26) gap-y-2 text-xs">
              <div className="text-muted-foreground">{t("app.skills.companySkills.install.trust")}</div>
              <div className="flex items-center gap-2">
                <TrustChip level={skill.trustLevel} />
                {skill.trustLevel === "markdown_only" ? (
                  <span className="text-muted-foreground">{t("app.skills.companySkills.install.safe")}</span>
                ) : skill.trustLevel === "scripts_executables" ? (
                  <span className="text-amber-800 dark:text-amber-200">{t("app.skills.companySkills.install.reviewRequired")}</span>
                ) : (
                  <span className="text-muted-foreground">{t("app.skills.companySkills.install.nonScriptAssets")}</span>
                )}
              </div>
              <div className="text-muted-foreground">{t("app.skills.companySkills.install.compatibility")}</div>
              <div className="flex items-center gap-2">
                {skill.compatibility === "compatible" ? (
                  <span className="inline-flex items-center gap-1 text-muted-foreground">
                    <Check className="h-3 w-3" aria-hidden="true" />
                    {t("app.skills.companySkills.install.compatible")}
                  </span>
                ) : (
                  <CompatChip compatibility={skill.compatibility} />
                )}
              </div>
              <div className="text-muted-foreground">{t("app.skills.companySkills.install.requires")}</div>
              <div className="text-foreground">{skill.requires.length === 0 ? t("app.skills.companySkills.wizard.none") : skill.requires.join(", ")}</div>
              <div className="text-muted-foreground">{t("app.skills.companySkills.install.roles")}</div>
              <div className="text-foreground">{skill.recommendedForRoles.length === 0 ? t("app.skills.companySkills.install.any") : skill.recommendedForRoles.join(" · ")}</div>
              <div className="text-muted-foreground">{t("app.skills.companySkills.install.provenance")}</div>
              <div className="min-w-0">
          ...
          <div className="rounded-md border border-border">
            <div className="border-b border-border px-3 py-2 text-xs uppercase tracking-wide text-muted-foreground">
              {t("app.skills.companySkills.install.files", { count: skill.files.length })}
            </div>
          ...
          {conflict ? (
            <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-200">
              <Trans
                i18nKey={defaultAction === "update" ? "app.skills.companySkills.install.conflictUpdate" : "app.skills.companySkills.install.conflictReplace"}
                values={{ key: conflict.key, source: conflict.sourceLabel ?? conflict.sourceType }}
          ...
          {defaultAction === "install" ? (
            <div className="rounded-md border border-border p-3">
              <div className="mb-1 text-xs uppercase tracking-wide text-muted-foreground">{t("app.skills.companySkills.install.enableForAgents")}</div>
              <p className="mb-2 text-xs text-muted-foreground">
                {t("app.skills.companySkills.install.enableHelp")}
              </p>
              <AgentMultiSelect
                agents={agents}
                ...
                emptyMessage={t("app.skills.companySkills.noAgentsSupportSkills")}
                ...
                getDescription={(agent) => {
                  ...
                  return [
                    option.adapterType,
                    option.required ? t("app.skills.companySkills.agentRequired") : null,
                    !option.supportsSkills ? t("app.skills.companySkills.agentSkillsNotSupported") : null,
                  ].filter(Boolean).join(" · ");
                }}
              />
            </div>
          ...
          {advancedOpen ? (
            <div className="space-y-3 rounded-md border border-border p-3 text-xs">
              <div>
                <label className="mb-1 block uppercase tracking-wide text-muted-foreground">{t("app.skills.companySkills.install.slugOverride")}</label>
              ...
              <label className="flex items-center gap-2">
                ...
                <span>{t("app.skills.companySkills.install.forceReplace")}</span>
              </label>
            </div>
          ) : null}
...
function AttachAgentsPopover({
  ...
  return (
    <AgentMultiSelect
      ...
      pending={pending}
      triggerLabel={t("app.skills.companySkills.addToAgent")}
      ...
      headerContent={sortedVersions.length > 0 ? (
        <div className="mt-2 flex items-center gap-2 text-xs">
          <span className="shrink-0 text-muted-foreground">{t("app.common.labels.version")}</span>
          <select
          ...
          >
            <option value="__latest__">{t("app.common.labels.latest")}</option>
      ...
      emptyMessage={eligible.length === 0 ? t("app.skills.companySkills.noAgentsSupportSkills") : t("app.skills.companySkills.noAgentsYet")}
      isAgentDisabled={(agent) => {
        const option = agent as AttachAgentOption;
        return option.required || !option.supportsSkills;
      }}
...
function SkillList({
  ...
  if (filteredSkills.length === 0) {
    if (sourceFilter !== "all" && skills.length > 0) {
      return (
        <div className="px-4 py-6 text-sm text-muted-foreground">
          {t("app.skills.companySkills.noSourceSkillsInstalled", { source: t(SOURCE_FILTER_LABEL_KEYS[sourceFilter]).toLowerCase() })}{" "}
          <button type="button" className="text-foreground underline" onClick={onClearFilters}>
            {t("app.skills.companySkills.clearFilter")}
          </button>
        </div>
      );
    }
    return (
      <div className="px-4 py-6 text-sm text-muted-foreground">
        {t("app.skills.companySkills.noSkillsMatchFilter")}
      </div>
    );
  }
...
const SKILL_DETAIL_TABS: Array<{ value: SkillDetailTab; labelKey: string; icon: typeof FileText }> = [
  { value: "overview", labelKey: "app.common.labels.overview", icon: FileText },
  { value: "files", labelKey: "app.common.labels.files", icon: FolderOpen },
  { value: "versions", labelKey: "app.skills.companySkills.detail.versions", icon: History },
  { value: "agents", labelKey: "app.common.nouns.agents", icon: Users },
];
...
function versionLabel(version: CompanySkillVersion | null | undefined) {
  if (!version) return translate("app.common.labels.latest");
      ...
      <DialogContent className="flex max-h-(--sz-85vh) w-full !max-w-(--pct-90) flex-col overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <DialogHeader className="shrink-0">
            <DialogTitle>{t("app.skills.companySkills.diff.title")}</DialogTitle>
          </DialogHeader>
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <label className="flex items-center gap-2">
              <Badge variant="outline" className="border-red-500/30 bg-red-500/10 uppercase tracking-wider text-red-400">{t("app.skills.companySkills.diff.old")}</Badge>
              <select
                value={leftVersionId ?? ""}
              ...
              >
                <option value="">{t("app.skills.companySkills.diff.initial")}</option>
                {sorted.map((version) => (
            ...
            <label className="flex items-center gap-2">
              <Badge variant="outline" className="border-green-500/30 bg-green-500/10 uppercase tracking-wider text-green-400">{t("app.skills.companySkills.diff.new")}</Badge>
              <select
                value={right?.id ?? ""}
        ...
        <div className="flex min-h-0 flex-1 gap-3">
          ...
          <div className="min-w-0 flex-1 overflow-auto rounded-md border border-border text-xs">
            {!right ? (
              <div className="p-6 text-center text-sm text-muted-foreground">{t("app.skills.companySkills.diff.selectVersion")}</div>
            ) : left?.id === right.id ? (
              <div className="p-6 text-center text-sm text-muted-foreground">{t("app.skills.companySkills.diff.sameVersion")}</div>
            ) : (
              <div className="font-mono text-xs leading-6">
                <div className="grid grid-cols-(--gtc-1) border-b border-border/60 bg-muted/30 px-3 py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
                  <span>{t("app.skills.companySkills.diff.old")}</span>
                  <span>{t("app.skills.companySkills.diff.new")}</span>
                  <span />
...
function SkillLocationCard({
  ...
  const canonical = folderPath && folderPath.length > 0 ? folderPath : t("app.skills.skillFolderTree.unfiled");
  return (
    <section>
      <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.skills.companySkills.detail.location")}</div>
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        >
          ...
          {copied ? t("app.common.actions.copied") : failed ? t("app.common.messages.copyFailed") : t("app.skills.companySkills.detail.copyPath")}
        </Button>
        {onMove ? (
          <Button size="sm" variant="outline" onClick={onMove}>
            <FolderInput className="mr-1.5 h-3.5 w-3.5" />
            {t("app.skills.companySkills.detail.move")}
          </Button>
        ) : null}
      </div>
    </section>
  ...
  return (
    <section>
      <div className="mb-2 flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted-foreground">
        ...
        {t("app.skills.skillFolderTree.tags")}
      </div>
      <div className="flex flex-wrap gap-1.5">
        {categories.map((tag) => (
          ...
          >
            ...
            <button
              type="button"
              aria-label={t("app.skills.companySkills.detail.removeTag", { tag })}
              disabled={pending}
        ...
        {categories.length === 0 ? (
          <span className="text-xs text-muted-foreground">{t("app.skills.companySkills.detail.noTags")}</span>
        ) : null}
      </div>
      <Input
        value={draft}
        ...
        placeholder={t("app.skills.companySkills.detail.addTag")}
        disabled={pending}
...
export function SkillDetailPage({
  ...
  if (!detail) {
    return loading ? <PageSkeleton variant="detail" /> : <EmptyState icon={Boxes} message={t("app.skills.companySkills.detail.notFound")} />;
  }
    ...
    return (
      <div className="grid min-h-(--sz-560px) gap-0 lg:grid-cols-(--gtc-28)">
        <aside className="border-b border-border pb-3 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-3">
          <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.common.labels.files")}</div>
          <SkillTree
            ...
            <div className="flex items-center gap-2">
              {file?.markdown && !editMode ? (
                <div className="flex items-center border border-border">
                  ...
                  >
                    <span className="flex items-center gap-1.5"><Eye className="h-3.5 w-3.5" /> {t("app.common.actions.view")}</span>
                  </button>
                  <button
                    className={cn("border-l border-border px-3 py-1.5 text-sm", viewMode === "code" ? "text-foreground" : "text-muted-foreground")}
                    onClick={() => setViewMode("code")}
                  >
                    <span className="flex items-center gap-1.5"><Code2 className="h-3.5 w-3.5" /> {t("app.common.labels.code")}</span>
                  </button>
                </div>
              ) : null}
              {skill.editable && file?.editable ? (
                editMode ? (
                  <>
                    <Button variant="ghost" size="sm" onClick={() => setEditMode(false)} disabled={savePending}>{t("app.common.actions.cancel")}</Button>
                    <Button size="sm" onClick={onSave} disabled={savePending}>
                      <Save className="mr-1.5 h-3.5 w-3.5" />
                      {savePending ? t("app.common.actions.saving") : t("app.common.actions.save")}
                    </Button>
                  </>
                ) : (
                  <Button variant="ghost" size="sm" onClick={() => setEditMode(true)}>
                    <Pencil className="mr-1.5 h-3.5 w-3.5" /> {t("app.common.actions.edit")}
                  </Button>
                )
              ) : !skill.editable ? (
                <Button
  ...
  function renderOverviewBody() {
    return (
      <div className="space-y-6">
        <section>
          <h2 className="mb-2 text-sm font-medium">{t("app.skills.companySkills.detail.about")}</h2>
          {fileLoading ? (
            <PageSkeleton variant="detail" />
          ) : file?.markdown ? (
            <MarkdownBody softBreaks={false} linkIssueReferences={false}>{body || skill.description || t("app.skills.companySkills.detail.noOverview")}</MarkdownBody>
          ) : (
            <p className="text-sm text-muted-foreground">{skill.description ?? t("app.skills.companySkills.detail.noOverview")}</p>
          )}
        </section>
        <section className="grid min-w-0 gap-3 text-sm sm:grid-cols-2">
          ...
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("app.common.labels.source")}</div>
            <div className="mt-1 min-w-0 [overflow-wrap:anywhere]">{sourceLocatorText ?? source.label}</div>
          </div>
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("app.common.labels.version")}</div>
            <div className="mt-1">{versionLabel(skill.currentVersion ?? null)}</div>
          </div>
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("app.common.labels.mode")}</div>
            <div className="mt-1 flex flex-wrap items-center gap-2">
              {skill.editable ? (
                t("app.skills.companySkills.detail.editable")
              ) : (
                <>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm text-muted-foreground">
            {versionsLoading
              ? t("app.skills.companySkills.detail.loadingVersions")
              : versions.length === 1
                ? t("app.skills.companySkills.detail.oneVersion", { count: versions.length })
                : t("app.skills.companySkills.detail.manyVersions", { count: versions.length })}
          </div>
          <Button
            type="button"
            variant="outline"
          ...
          >
            <History className="mr-1.5 h-3.5 w-3.5" /> {t("app.skills.companySkills.detail.compare")}
          </Button>
        </div>
        <div className="border-y border-border">
          {versionsLoading ? (
            ...
            <div className="py-6 text-sm text-muted-foreground">{t("app.skills.companySkills.detail.noVersions")}</div>
          ) : (
            sortedVersions.map((version) => (
              <div key={version.id} className="grid gap-2 border-b border-border px-0 py-3 text-sm last:border-b-0 sm:grid-cols-(--gtc-13)">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    {relativeTime(version.createdAt)} · {t("app.skills.companySkills.detail.fileCount", { count: version.fileInventory.length })}
                  </div>
                </div>
                <Button
                  type="button"
                ...
                >
                  {t("app.skills.companySkills.detail.viewDiff")}
                </Button>
              </div>
            ))
          )}
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">
            {attached.length === 1
              ? t("app.skills.companySkills.detail.oneAgentAttached", { count: attached.length })
              : t("app.skills.companySkills.detail.manyAgentsAttached", { count: attached.length })}
            {` · ${selectedVersion ? versionLabel(selectedVersion) : t("app.common.labels.latest")}`}
          </p>
          <AttachAgentsPopover
            agents={attachAgents}
        ...
        {attached.length === 0 ? (
          <div className="rounded-md border border-dashed border-border py-8 text-center text-sm text-muted-foreground">
            {t("app.skills.companySkills.detail.noAgentsUsing")}
          </div>
        ) : (
          <div className="border-y border-border">
            {attached.map((agent) => {
    ...
    <div className="min-h-(--sz-calc-30)">
              ...
              <div className="min-w-0">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span
                        ...
                        aria-label={t("app.skills.companySkills.detail.installedFrom", { source: source.label })}
                      >
                        <SourceIcon className="h-4 w-4" />
                      </span>
                    </TooltipTrigger>
                    <TooltipContent>{t("app.skills.companySkills.detail.installedFrom", { source: source.label })}</TooltipContent>
                  </Tooltip>
                </div>
                ...
                {detail.authorName ? (
                  <p className="mt-0.5 text-sm text-muted-foreground">
                    <Trans
                      i18nKey="app.skills.companySkills.detail.byAuthor"
                      values={{ author: detail.authorName }}
                      components={{ author: <span className="text-foreground" /> }}
                    />
                  </p>
                ) : null}
                {subtitleText ? (
                  <div className="mt-1 max-w-2xl">
                    ...
                    {descClamped ? (
                      ...
                      >
                        {descExpanded ? t("app.skills.companySkills.detail.showLess") : t("app.common.actions.viewAll")}
                      </button>
                    ) : null}
                  </div>
                ) : null}
          ...
          <div className="flex flex-wrap items-center justify-end gap-1">
            <Button variant="outline" size="sm" asChild>
              <Link to={resolvedStudioHref}>
                ...
                {t("app.skills.companySkills.detail.openInStudio")}
              </Link>
            </Button>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-muted-foreground">
                    ...
                    <span className="hidden sm:inline">{detail.attachedAgentCount === 1 ? t("app.skills.companySkills.detail.install") : t("app.skills.companySkills.detail.installs")}</span>
                  </span>
                </TooltipTrigger>
                <TooltipContent>{t("app.skills.companySkills.detail.installsTooltip")}</TooltipContent>
              </Tooltip>
              <button
                type="button"
                ...
                title={detail.starredByCurrentActor ? t("app.skills.companySkills.detail.unstar") : t("app.skills.companySkills.detail.star")}
              >
                <Star className={cn("h-3.5 w-3.5", detail.starredByCurrentActor && "fill-current text-yellow-400")} />
                <span className="hidden sm:inline">{detail.starredByCurrentActor ? t("app.skills.companySkills.detail.starred") : t("app.skills.companySkills.detail.starShort")}</span>
      ...
      <div className="grid gap-6 px-4 py-4 xl:grid-cols-(--gtc-29)">
        ...
        <aside className="min-w-0 space-y-6 border-t border-border pt-4 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
          ...
          />
          <section>
            <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.common.nouns.agents")}</div>
            <div className="space-y-3">
              ...
              {detail.usedByAgents.length === 0 ? (
                <p className="text-xs text-muted-foreground">{t("app.skills.companySkills.detail.noAgentsAttached")}</p>
              ) : (
                <div className="space-y-0.5">
                  ...
                  {detail.usedByAgents.slice(0, 3).map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {meta?.paused ? (
                          <Pause className="h-3 w-3 shrink-0 text-amber-500" aria-label={t("app.common.states.paused")} />
                        ) : null}
                      </Link>
                    );
                  })}
                  {detail.usedByAgents.length > 3 ? (
                    <p className="px-1.5 pt-0.5 text-xs text-muted-foreground">
                      {t("app.skills.companySkills.detail.andMore", { count: detail.usedByAgents.length - 3 })}
                    </p>
                  ) : null}
                </div>
              )}
          ...
          {/* Provenance: where this skill came from, with org/path linked when
              ...
              (PAP-10907). */}
          <section>
            <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.common.labels.source")}</div>
            {githubSource ? (
          ...
          {detail.sourceType === "github" ? (
            <section>
              <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.skills.companySkills.detail.updates")}</div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Pin className="h-3.5 w-3.5 shrink-0" aria-label={t("app.skills.companySkills.detail.pinnedRevision")} />
                    </TooltipTrigger>
                    <TooltipContent>{t("app.skills.companySkills.detail.pinnedRevision")}</TooltipContent>
                  </Tooltip>
                  <span className="truncate font-mono text-foreground">{currentPin ?? t("app.skills.companySkills.detail.untracked")}</span>
                </div>
                <Button variant="outline" size="sm" className="w-full" onClick={onCheckUpdates} disabled={checkUpdatesPending || updateStatusLoading}>
                  ...
                  {t("app.skills.companySkills.detail.checkForUpdates")}
                </Button>
                {updateStatus?.supported && updateStatus.hasUpdate ? (
                  <Button size="sm" className="w-full" onClick={onInstallUpdate} disabled={installUpdatePending}>
                    ...
                    {t("app.skills.companySkills.install.installUpdate")}{latestPin ? ` ${latestPin}` : ""}
                  </Button>
                ) : updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading ? (
                  <p className="text-xs text-muted-foreground">{t("app.skills.companySkills.detail.upToDate")}</p>
                ) : null}
              </div>
            </section>
          ) : null}
          ...
          <section>
            ...
            >
              ...
              <span className="flex-1">{t("app.common.nouns.settings")}</span>
            </button>
          </section>
        </aside>
      </div>
      ...
      {isDirty ? (
        <div className="fixed bottom-6 left-1/2 z-40 flex -translate-x-1/2 items-center gap-3 rounded-full border border-border bg-background/95 px-4 py-2 shadow-lg backdrop-blur">
          <span className="text-sm text-muted-foreground">{t("app.common.states.unsavedChanges")}</span>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => {
          ...
          >
            {t("app.skills.companySkills.detail.discard")}
          </Button>
          <Button size="sm" onClick={onSave} disabled={savePending}>
            <Save className="mr-1.5 h-3.5 w-3.5" />
            {savePending ? t("app.common.actions.saving") : t("app.common.actions.saveChanges")}
          </Button>
        </div>
      ) : null}
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("app.skills.companySkills.settings.title")}</DialogTitle>
            <DialogDescription>{t("app.skills.companySkills.settings.description", { name: detail.name })}</DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.common.labels.categories")}</label>
              <Input
              ...
              />
              <p className="text-xs text-muted-foreground">{t("app.skills.companySkills.settings.categoriesHelp")}</p>
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("app.skills.companySkills.wizard.sharing")}</label>
              <select
              ...
              >
                <option value="company">{t("app.skills.companySkills.settings.scopeOrganization")}</option>
                <option value="private">{t("app.skills.companySkills.settings.scopePrivate")}</option>
              </select>
              <p className="text-xs text-muted-foreground">{t("app.skills.companySkills.settings.publicLinkLater")}</p>
            </div>
            <div className="flex justify-end gap-2 border-t border-border pt-4">
              ...
              >
                <Save className="mr-1.5 h-3.5 w-3.5" />
                {updateSettingsPending ? t("app.common.actions.saving") : t("app.skills.companySkills.settings.save")}
              </Button>
            </div>
            {detail.editable ? (
              <div className="rounded-md border border-destructive/40 p-3">
                <div className="text-xs font-semibold uppercase tracking-wide text-destructive">{t("app.skills.companySkills.settings.dangerZone")}</div>
                <div className="mt-2 flex items-center justify-between gap-3">
                  <p className="min-w-0 text-xs text-muted-foreground">{t("app.skills.companySkills.settings.removeHelp")}</p>
                  <Button
                    variant="destructive"
                    size="sm"
                    className="shrink-0"
                    ...
                    title={detail.usedByAgents.length > 0 ? t("app.skills.companySkills.detachBeforeRemove") : undefined}
                  >
                    <Trash2 className="mr-1.5 h-3.5 w-3.5" />
                    {deletePending ? t("app.common.progress.removing") : t("app.common.actions.remove")}
                  </Button>
                </div>
              </div>
            ) : null}
...
function SkillPane({
  ...
  if (!detail) {
    ...
    return (
      <EmptyState
        icon={Boxes}
        message={t("app.skills.companySkills.pane.selectSkill")}
      />
    );
  }
  ...
  const removeDisabledReason = removeBlocked
    ? t("app.skills.companySkills.detachBeforeRemove")
    : null;
      ...
      <div className="border-b border-border px-5 py-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            {detail.editable ? (
              ...
              >
                ...
                {editMode ? t("app.skills.companySkills.pane.stopEditing") : t("app.common.actions.edit")}
              </button>
            ) : (
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div className="flex min-w-0 items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("app.common.labels.source")}</span>
                ...
                {detail.sourcePath && displaySourcePath ? (
                  <>
                    ...
                    <CopyText
                      ...
                      copiedLabel={t("app.skills.companySkills.pane.copiedPath")}
                      ariaLabel={t("app.skills.companySkills.pane.copySourcePath")}
                      title={t("app.skills.companySkills.pane.copySourcePath")}
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("app.skills.companySkills.pane.pin")}</span>
                <span className="font-mono text-xs">{currentPin ?? t("app.skills.companySkills.detail.untracked")}</span>
                {updateStatus?.trackingRef && (
                  <span className="text-xs text-muted-foreground">{t("app.skills.companySkills.pane.tracking", { ref: updateStatus.trackingRef })}</span>
                )}
                <Button
                  variant="ghost"
                  size="sm"
                ...
                {updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading && (
                  <span className="text-xs text-muted-foreground">{t("app.skills.companySkills.pane.upToDate")}</span>
                )}
            ...
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("app.common.labels.mode")}</span>
              <span>{detail.editable ? t("app.skills.companySkills.detail.editable") : t("app.skills.companySkills.detail.readOnly")}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("app.skills.companySkills.install.trust")}</span>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Badge variant="outline" className="border-violet-500/40 bg-violet-500/10 text-(length:--text-micro) text-violet-200">
                    ...
                    {t("app.skills.companySkills.pane.locallyModified")}
                  </Badge>
                </TooltipTrigger>
                <TooltipContent>{t("app.skills.companySkills.pane.locallyModifiedTooltip")}</TooltipContent>
              </Tooltip>
            ) : null}
            {(() => {
          ...
          <div className="flex flex-wrap items-start gap-x-3 gap-y-2">
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("app.skills.companySkills.pane.usedBy")}</span>
              <AttachAgentsPopover
                agents={attachAgents}
            ...
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">{t("app.skills.companySkills.pane.noAgentsAttached")}</span>
            ) : (
...
export function CompanySkills() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("app.common.nouns.skills"), href: "/skills" },
      ...(isStudioNew
        ? [{ label: studioForkFromId ? t("app.skills.companySkills.page.forkSkill") : t("app.skills.companySkills.page.newSkill") }]
        : activeDetail
          ? skillDetailBreadcrumbs(activeDetail, skillFoldersQuery.data).slice(1)
          : routeSkillToken
            ? [{ label: t("app.skills.companySkills.page.detail") }]
            : []),
    ]);
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("app.skills.companySkills.toast.skillsImported"),
        body: result.imported.length === 1
          ? t("app.skills.companySkills.toast.oneSkillAdded", { count: result.imported.length })
          : t("app.skills.companySkills.toast.manySkillsAdded", { count: result.imported.length }),
      });
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: t("app.skills.companySkills.toast.importWarnings"), body: result.warnings[0] });
      }
      setSource("");
    },
    onError: (error) => {
      reportSkillError(error, t("app.skills.companySkills.toast.importFailed"), t("app.skills.companySkills.toast.importFailedBody"), t("app.skills.companySkills.action.importing"));
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onMutate: (projectId) => {
      setScanStatusMessage(
        projectId ? t("app.skills.companySkills.scan.refreshingProject") : t("app.skills.companySkills.scan.scanning"),
      );
    },
    onSuccess: async (result) => {
      setScanStatusMessage(t("app.skills.companySkills.scan.refreshingList"));
      await Promise.all([
      ...
      pushToast({
        tone: "success",
        title: t("app.skills.companySkills.toast.scanComplete"),
        body: summary,
      });
      if (result.conflicts[0]) {
        pushToast({
          tone: "warn",
          title: t("app.skills.companySkills.toast.conflictsFound"),
          body: result.conflicts[0].reason,
        });
      } else if (result.warnings[0]) {
        pushToast({
          tone: "warn",
          title: t("app.skills.companySkills.toast.scanWarnings"),
    ...
    onError: (error) => {
      ...
      reportSkillError(error, t("app.skills.companySkills.toast.scanFailed"), t("app.skills.companySkills.toast.scanFailedBody"), t("app.skills.companySkills.action.scanning"));
    },
  });
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: skill.forkedFromSkillId ? t("app.skills.companySkills.toast.forkCreated") : t("app.skills.companySkills.toast.skillCreated"),
        body: t("app.skills.companySkills.toast.nowEditable", { name: skill.name }),
      });
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("app.skills.companySkills.toast.createFailedBody");
      setCreateError(message);
      reportSkillError(error, t("app.skills.companySkills.toast.createFailed"), t("app.skills.companySkills.toast.createFailedBody"), t("app.skills.companySkills.action.creating"));
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("app.skills.companySkills.toast.saved"),
        body: result.path,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.saveFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.saveFailedBody"),
      });
    },
  });
  ...
  const toggleStar = useMutation({
    mutationFn: () => {
      if (!activeDetail) throw new Error(t("app.skills.companySkills.toast.selectSkillFirst"));
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.starFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.starFailedBody"),
      });
    },
  });
  ...
  const updateSkillSettings = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({ tone: "success", title: t("app.skills.companySkills.toast.settingsUpdated"), body: skillSettingsToastBody(skill) });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.settingsFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.settingsFailedBody"),
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("app.skills.companySkills.toast.skillUpdated"),
        body: skill.sourceRef ? t("app.skills.companySkills.toast.pinnedTo", { ref: shortRef(skill.sourceRef) }) : skill.name,
      });
    },
    onError: (error) => {
      reportSkillError(error, t("app.skills.agentsUsingSkillDialog.updateFailed"), t("app.skills.companySkills.toast.updateFailedBody"), t("app.skills.companySkills.action.updating"));
    },
  });
  ...
  const installCatalog = useMutation({
    ...
    onSuccess: async (result, payload) => {
      ...
      pushToast({
        tone: "success",
        title: result.action === "created" ? t("app.skills.companySkills.toast.installed") : result.action === "updated" ? t("app.skills.companySkills.toast.skillUpdated") : t("app.skills.companySkills.toast.upToDate"),
        body: result.action === "created"
          ? enabledCount > 0
            ? enabledCount === 1
              ? t("app.skills.companySkills.toast.enabledForOne", { name: result.skill.name, count: enabledCount })
              : t("app.skills.companySkills.toast.enabledForMany", { name: result.skill.name, count: enabledCount })
            : t("app.skills.companySkills.toast.notEnabled", { name: result.skill.name })
          : result.skill.name,
      });
      if (enableFailures > 0) {
        pushToast({
          tone: "warn",
          title: t("app.skills.companySkills.toast.enableFailed"),
          body: enableFailures === 1
            ? t("app.skills.companySkills.toast.enableFailedOne", { name: result.skill.name, count: enableFailures })
            : t("app.skills.companySkills.toast.enableFailedMany", { name: result.skill.name, count: enableFailures }),
        });
      }
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: t("app.skills.companySkills.toast.installWarnings"), body: result.warnings[0] });
      }
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("app.skills.companySkills.toast.installFailedBody");
      ...
      policyDenial.capture(error, t("app.skills.companySkills.action.installing"));
    },
  });
    ...
    onSuccess: async (folder) => {
      ...
      if (moveAfterCreateSkillIds.length > 0) {
        ...
        try {
          ...
          pushToast({
            tone: "error",
            title: t("app.skills.companySkills.toast.folderCreatedMoveFailed"),
            body: moveError instanceof Error ? moveError.message : t("app.skills.companySkills.toast.moveSelectedFailedBody"),
          });
          return;
        }
      } else {
      ...
      pushToast({ tone: "success", title: t("app.skills.companySkills.toast.folderCreated"), body: folder.name });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.folderSaveFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.folderSaveFailedBody"),
      });
    },
  });
  const updateFolder = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.folderSaveFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.folderUpdateFailedBody"),
      });
    },
  });
  const moveFolder = useMutation({
    ...
    onSuccess: async (folder) => {
      ...
      pushToast({
        tone: "success",
        title: t("app.skills.companySkills.toast.folderMoved"),
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.folderMoveFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.folderMoveFailedBody"),
      });
    },
  });
  const deleteFolder = useMutation({
    ...
    onSuccess: async (_, folderId) => {
      ...
      pushToast({ tone: "success", title: t("app.skills.companySkills.toast.folderDeleted"), body: t("app.skills.companySkills.toast.skillsMovedToUnfiled") });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.folderDeleteFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.folderDeleteFailedBody"),
      });
    },
  });
  const moveSkillToFolder = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.moveFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.moveSkillFailedBody"),
      });
    },
  });
  ...
  async function moveSelectedSkills(folderId: string | null) {
    ...
    try {
      ...
      pushToast({
        tone: "success",
        title: t("app.skills.companySkills.toast.skillsMoved"),
        body: ids.length === 1 ? t("app.skills.companySkills.toast.oneSkillFiled", { count: ids.length }) : t("app.skills.companySkills.toast.manySkillsFiled", { count: ids.length }),
      });
    } catch (moveError) {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.moveSkillsFailed"),
        body: moveError instanceof Error ? moveError.message : t("app.skills.companySkills.toast.moveSelectedFailedBody"),
      });
    }
  }
  ...
  const ensureMyFolder = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.openMySkillsFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.openMySkillsFailedBody"),
      });
    },
  });
  ...
  async function createFolderInline(parentId: string | null, name: string): Promise<string | null> {
    try {
      ...
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.folderCreateFailed"),
        body: error instanceof Error ? error.message : t("app.skills.companySkills.toast.folderCreateFailedBody"),
      });
      return null;
    }
  }
  ...
  async function performDialogMove(folderId: string | null) {
    ...
    try {
      ...
      pushToast({
        tone: "success",
        title: ids.length === 1 ? t("app.skills.companySkills.toast.skillMoved") : t("app.skills.companySkills.toast.skillsMoved"),
        body: folderId
          ? t("app.skills.companySkills.toast.filedUnder", { folder: skillFolderResult?.folders.find((folder) => folder.id === folderId)?.name ?? t("app.skills.companySkills.toast.folderFallback") })
          : t("app.skills.companySkills.toast.movedToUnfiled"),
      });
    } catch (moveError) {
      pushToast({
        tone: "error",
        title: t("app.skills.companySkills.toast.moveFailed"),
        body: moveError instanceof Error ? moveError.message : t("app.skills.companySkills.toast.moveFailedBody"),
      });
    }
  }
  ...
  async function handleAttachSubmit(nextAgentIds: string[], versionId: string | null = null) {
    ...
    try {
      ...
      pushToast({ tone: "success", title: t("app.skills.companySkills.toast.agentsUpdated"), body: t("app.skills.companySkills.toast.agentsAttached", { count: nextAgentIds.length }) });
    } catch (error) {
      pushToast({ tone: "error", title: t("app.skills.agentsUsingSkillDialog.updateFailed"), body: error instanceof Error ? error.message : t("app.skills.agentsUsingSkillDialog.updateFailedFallback") });
    }
  }
  ...
  const deleteSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("app.skills.companySkills.toast.removed"),
        body: t("app.skills.companySkills.toast.removedBody", { name: skill.name }),
      });
    },
    onError: (error) => {
      reportSkillError(error, t("app.skills.companySkills.toast.removeFailed"), t("app.skills.companySkills.toast.removeFailedBody"), t("app.skills.companySkills.action.removing"));
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Boxes} message={t("app.skills.companySkills.page.selectOrganization")} />;
  }
  ...
  const studioTitle = studioForkFromId ? t("app.skills.companySkills.page.forkSkill") : t("app.skills.companySkills.page.createNewSkill");
  const studioDescription = studioForkFromId
    ? t("app.skills.companySkills.page.forkDescription")
    : t("app.skills.companySkills.page.createDescription");
  return (
    <>
      {policyDenial.denial ? (
      ...
      <Dialog open={deleteOpen} onOpenChange={closeDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("app.skills.companySkills.remove.title")}</DialogTitle>
            <DialogDescription>
              {t("app.skills.companySkills.remove.description")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <p>
              {deleteTargetDetail
                ? t("app.skills.companySkills.remove.aboutToRemoveName", { name: deleteTargetDetail.name })
                : t("app.skills.companySkills.remove.aboutToRemove")}
            </p>
            {deleteTargetDetail?.usedByAgents?.length ? (
              <div className="rounded-md border border-border px-3 py-3 text-muted-foreground">
                {t("app.skills.companySkills.remove.currentlyUsedBy", { agents: deleteTargetDetail.usedByAgents.map((agent) => agent.name).join(", ") })}
              </div>
            ) : null}
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              <p className="text-muted-foreground">
                {t("app.skills.companySkills.remove.detachToEnable")}
              </p>
            ) : null}
          </div>
          <DialogFooter>
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              ...
              <>
                ...
                >
                  {deleteSkill.isPending ? t("app.common.progress.removing") : t("app.skills.companySkills.remove.title")}
                </Button>
              </>
            )}
          </DialogFooter>
      ...
      <Dialog open={emptySourceHelpOpen} onOpenChange={setEmptySourceHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("app.skills.companySkills.source.addTitle")}</DialogTitle>
            <DialogDescription>
              {t("app.skills.companySkills.source.addDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <a
            ...
            >
              <span>
                <span className="block font-medium">{t("app.skills.companySkills.source.browseSkillsSh")}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("app.skills.companySkills.source.findCommands")}
                </span>
              </span>
            ...
            >
              <span>
                <span className="block font-medium">{t("app.skills.companySkills.source.searchGithub")}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("app.skills.companySkills.source.lookForReposHere")}
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
      ...
      <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("app.skills.companySkills.source.importTitle")}</DialogTitle>
            <DialogDescription>
              {t("app.skills.companySkills.source.importDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="flex items-center gap-2 border-b border-border pb-2">
              <Input
                ...
                placeholder={t("app.skills.companySkills.source.importPlaceholder")}
              ...
              <Button size="sm" onClick={handleAddSkillSource} disabled={importSkill.isPending}>
                {importSkill.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : t("app.common.actions.import")}
              </Button>
            </div>
            <a
              href="https://skills.sh"
            ...
            >
              <span>
                <span className="block font-medium">{t("app.skills.companySkills.source.browseSkillsSh")}</span>
                <span className="mt-1 block text-muted-foreground">{t("app.skills.companySkills.source.findCommands")}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
            <a
            ...
            >
              <span>
                <span className="block font-medium">{t("app.skills.companySkills.source.searchGithub")}</span>
                <span className="mt-1 block text-muted-foreground">{t("app.skills.companySkills.source.lookForRepos")}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
          </div>
      ...
      <MoveToFolderDialog
        ...
        result={railSkillFolderResult}
        title={moveDialog?.title ?? t("app.skills.companySkills.move.title")}
      ...
      {isStudioNew ? (
          ...
          <div className="px-4 py-4">
            <div className="max-w-3xl">
              {studioForkFromId && studioForkDetailQuery.isLoading ? (
                ...
                <EmptyState icon={Boxes} message={t("app.skills.companySkills.page.forkSourceNotFound")} />
              ) : (
                <NewSkillWizard
        ...
        {!streamlinedUiEnabled ? (
          <div className="px-4 pt-4">
            <Tabs value={legacyDiscoveryTab} onValueChange={(value) => setLegacyDiscoveryTab(value as "all" | "installed" | "catalog" | "bundled")}>
              <TabsList variant="line" aria-label={t("app.skills.companySkills.page.skillsView")}>
                <TabsTrigger value="all">{t("app.common.labels.all")}</TabsTrigger>
                <TabsTrigger value="installed">{t("app.common.states.installed")}</TabsTrigger>
                <TabsTrigger value="catalog">{t("app.skills.companySkills.catalogLabel")}</TabsTrigger>
                <TabsTrigger value="bundled">{t("app.skills.skillFolderTree.bundled")}</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        ) : null}
        <DiscoveryGrid
          ...
          onOpenMoveCard={showInstalledFolders ? (card) => {
            ...
            setMoveDialog({
              ...
              title: t("app.skills.companySkills.move.moveName", { name: card.name }),
              subtitle: t("app.skills.companySkills.move.chooseDestination"),
          ...
          onMoveCard={showInstalledFolders ? (card, folderId) => {
            ...
            pushToast({
              tone: "success",
              title: t("app.skills.companySkills.toast.skillMoved"),
              body: folderId
                ? t("app.skills.companySkills.move.movedTo", { name: card.name, folder: skillFolderResult?.folders.find((folder) => folder.id === folderId)?.name ?? t("app.skills.companySkills.toast.folderFallback") })
                : t("app.skills.companySkills.move.movedToUnfiled", { name: card.name }),
              action: {
        ...
        <SkillDetailPage
          ...
          onMoveToFolder={activeDetail ? () => setMoveDialog({
            ...
            title: t("app.skills.companySkills.move.moveName", { name: activeDetail.name }),
            subtitle: t("app.skills.companySkills.move.chooseDestination"),
        ...
        <div className="min-h-(--sz-calc-30)">
          <div className="border-b border-border px-4 py-3">
            ...
            >
              <ChevronLeft className="h-4 w-4" />
              {t("app.skills.companySkills.page.backToStore")}
            </Link>
          </div>
          {catalogListQuery.isLoading || catalogDetailQuery.isLoading ? (
            <PageSkeleton variant="detail" />
          ) : !selectedCatalogSkill ? (
            <EmptyState icon={Boxes} message={t("app.skills.companySkills.page.catalogSkillNotFound")} />
          ) : (
        ...
        <div className="min-h-(--sz-calc-30)">
          {skillsQuery.isLoading ? (
            ...
            <EmptyState icon={Boxes} message={t("app.skills.companySkills.detail.notFound")} />
          )}
        </div>
      )}
    </>
