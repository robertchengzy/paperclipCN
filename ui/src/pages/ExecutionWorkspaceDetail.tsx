...
function getExecutionWorkspaceBaseTabItems(): OrderedExecutionWorkspaceTabItem[] {
  return [
    { value: "issues", label: translate("app.common.nouns.tasks"), order: 10 },
    { value: "services", label: translate("app.workspaces.workspaceRuntimeControls.services"), order: 20 },
    { value: "configuration", label: translate("app.common.labels.configuration"), order: 30 },
    { value: "runtime_logs", label: translate("app.workspaces.executionWorkspaceDetail.tabs.runtimeLogs"), order: 40 },
    { value: "routines", label: translate("app.common.nouns.routines"), order: 60 },
  ];
}
...
function formatOptionalDateTime(value: Date | string | null | undefined) {
  return value ? formatDateTime(value) : translate("app.workspaces.executionWorkspaceDetail.never");
}
...
function parseWorkspaceRuntimeJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: translate("app.workspaces.executionWorkspaceDetail.jsonMustBeObject"),
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : translate("app.workspaces.executionWorkspaceDetail.invalidJson"),
    };
  }
}
  ...
  const addServices = (collection: ConfiguredRuntimeServicePort["collection"], services: unknown, commandsRequireServiceKind: boolean) => {
    ...
    services.forEach((service, index) => {
      ...
      entries.push({
        collection,
        index,
        name: typeof config.name === "string" && config.name.trim() ? config.name : translate("app.workspaces.executionWorkspaceDetail.serviceNumber", { number: index + 1 }),
...
export function getConfiguredRuntimeServicePortWarnings(services: ConfiguredRuntimeServicePort[]) {
  ...
  return Array.from(servicesByPort.entries())
    ...
    .map(([port, servicesForPort]) =>
      translate("app.workspaces.executionWorkspaceDetail.portConflict", {
        port,
...
function validateForm(form: WorkspaceFormState) {
  ...
  if (repoUrl) {
    try {
      ...
      return translate("app.workspaces.executionWorkspaceDetail.repoUrlInvalid");
    }
  }
  ...
  if (!form.inheritRuntime) {
    ...
    if (invalidPort) return translate("app.workspaces.executionWorkspaceDetail.invalidFixedPort", { name: invalidPort.name });
  }
...
function workspaceOperationPhaseLabel(phase: string) {
  switch (phase) {
    case "worktree_prepare":
      return translate("app.agentDetail.workspaceOps.phase.worktreePrepare");
    case "workspace_config_freshness":
      return translate("app.agentDetail.workspaceOps.phase.configFreshness");
    case "workspace_provision":
      return translate("app.agentDetail.workspaceOps.phase.provision");
    case "workspace_seed":
      return translate("app.workspaces.executionWorkspaceDetail.phase.databaseSeed");
    case "workspace_runtime_provision":
      return translate("app.workspaces.executionWorkspaceDetail.phase.runtimeProvision");
    case "workspace_teardown":
      return translate("app.agentDetail.workspaceOps.phase.teardown");
    case "worktree_cleanup":
      return translate("app.agentDetail.workspaceOps.phase.worktreeCleanup");
    case "workspace_finalize":
      return translate("app.workspaces.executionWorkspaceDetail.phase.finalize");
    default:
      return phase;
  }
}
...
export function RuntimeProvisionStatusValue({
  ...
  if (status.kind === "eager") {
    return (
      <span className="text-sm text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.provision.eager")}</span>
    );
  }
  if (status.kind === "deferred") {
    return (
      <div className="flex flex-col gap-1">
        <StatusPill className="border-amber-500/40 text-amber-600 dark:text-amber-400">{t("app.workspaces.executionWorkspaceDetail.provision.deferred")}</StatusPill>
        <span className="text-xs text-muted-foreground">
          {t("app.workspaces.executionWorkspaceDetail.provision.deferredHint")}
        </span>
      </div>
    );
  }
  if (status.kind === "provisioning") {
    return (
      <StatusPill className="border-border text-muted-foreground">
        ...
        {t("app.workspaces.workspaceServiceControlBar.provisioning")}
      </StatusPill>
    );
  }
  if (status.kind === "provisioned") {
    return (
      <StatusPill className="border-emerald-500/40 text-emerald-600 dark:text-emerald-400">
        {status.at
          ? t("app.workspaces.executionWorkspaceDetail.provision.provisionedAt", { time: formatDateTime(status.at) })
          : t("app.workspaces.executionWorkspaceDetail.provision.provisioned")}
      </StatusPill>
    );
  }
  return (
    <div className="flex flex-col gap-1">
      <StatusPill className="border-destructive/50 text-destructive">
        {status.at
          ? t("app.workspaces.executionWorkspaceDetail.provision.failedAt", { time: formatDateTime(status.at) })
          : t("app.workspaces.executionWorkspaceDetail.provision.failed")}
      </StatusPill>
      <button type="button" onClick={onViewLogs} className="self-start text-xs text-muted-foreground underline underline-offset-2 hover:text-foreground">
        {t("app.workspaces.executionWorkspaceDetail.viewRuntimeLogs")}
      </button>
    </div>
  );
}
...
function MonoValue({ value, copy }: { value: string; copy?: boolean }) {
  ...
  return (
    <div className="inline-flex max-w-full items-start gap-2">
      ...
      {copy ? (
        <CopyText text={value} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel={t("app.common.states.copied")}>
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>{routine.assigneeAgentId ? t("app.workspaces.executionWorkspaceDetail.routines.defaultAgentSet") : t("app.workspaces.executionWorkspaceDetail.routines.chooseAgent")}</span>
          <span>{t("app.workspaces.executionWorkspaceDetail.routines.lastRun", { time: formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt) })}</span>
      ...
      >
        ...
        {isRunning ? t("app.workspaces.executionWorkspaceDetail.routines.running") : t("app.common.actions.runNow")}
      </Button>
    </div>
  );
}
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: t("app.workspaces.executionWorkspaceDetail.routines.started"),
        body: t("app.workspaces.executionWorkspaceDetail.routines.startedBody"),
        tone: "success",
      });
    },
    onSettled: () => {
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("app.workspaces.executionWorkspaceDetail.routines.runFailed"),
        body: mutationError instanceof Error ? mutationError.message : t("app.workspaces.executionWorkspaceDetail.routines.runFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("app.workspaces.executionWorkspaceDetail.routines.title")}</CardTitle>
          <CardDescription>
            {t("app.workspaces.executionWorkspaceDetail.routines.description")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.routines.loading")}</p>
          ) : error ? (
            <p className="text-sm text-destructive">
              {error instanceof Error ? error.message : t("app.workspaces.executionWorkspaceDetail.routines.loadFailed")}
            </p>
          ) : workspaceRoutines.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              <Repeat className="h-5 w-5 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">
                {t("app.workspaces.executionWorkspaceDetail.routines.empty")}
              </p>
            </div>
          ) : (
...
export function ExecutionWorkspaceDetail() {
  ...
  const workspaceTabItems = useMemo(
    () => orderExecutionWorkspaceTabItems([
      ...getExecutionWorkspaceBaseTabItems().filter((item) => item.value !== "configuration" || workspaceIsolationControlsVisible),
      ...workspacePluginTabItems,
    ]),
    [workspacePluginTabItems, workspaceIsolationControlsVisible, t],
  );
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("app.workspaces.executionWorkspaceDetail.saveFailed"));
    },
  });
  ...
  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.loading")}</p>;
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("app.workspaces.executionWorkspaceDetail.loadFailed")}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t("app.workspaces.executionWorkspaceDetail.buildUpdateFailed"));
      return;
    }
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-2">
            <div className="text-xs font-medium uppercase tracking-(--tracking-eyebrow) text-muted-foreground">
              {t("app.workspaces.projectWorkspaceSummaryCard.executionWorkspace")}
            </div>
        ...
        {activeTab === "services" ? (
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? t("app.workspaces.executionWorkspaceDetail.services.noneStarted")
                : t("app.workspaces.executionWorkspaceDetail.services.noConfig")
            }
            jobEmptyMessage={t("app.workspaces.executionWorkspaceDetail.services.noJobs")}
            disabledHint={
              canStartRuntimeServices
                ? null
                : t("app.workspaces.executionWorkspaceDetail.services.disabledHint")
            }
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("app.workspaces.executionWorkspaceDetail.settings.title")}</CardTitle>
                <CardDescription>
                  {t("app.workspaces.executionWorkspaceDetail.settings.description")}
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
                  ...
                  >
                    {workspace.status === "cleanup_failed" ? t("app.workspaces.executionWorkspaceCloseDialog.retryClose") : t("app.workspaces.executionWorkspaceCloseDialog.closeWorkspace")}
                  </Button>
                </CardAction>
              </CardHeader>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.common.labels.general")}</div>
                  <Field label={t("app.workspaces.executionWorkspaceDetail.field.workspaceName")}>
                    <Input
                      value={form.name}
                      onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                      placeholder={t("app.workspaces.executionWorkspaceDetail.field.workspaceNamePlaceholder")}
                    />
                  </Field>
                </div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.field.sourceControl")}</div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={t("app.workspaces.executionWorkspaceDetail.field.branchName")} hint={t("app.workspaces.executionWorkspaceDetail.field.branchNameHint")}>
                      <Input
                        className="font-mono"
                    ...
                    <Field label={t("app.workspaces.executionWorkspaceCloseDialog.baseRef")}>
                      <Input
                        className="font-mono"
                        value={form.baseRef}
                  ...
                  <Field label={t("app.workspaces.executionWorkspaceDetail.field.repoUrl")}>
                    <Input
                      value={form.repoUrl}
                ...
                */}
                {!hideHostPaths && (
                  <>
                    <div className="space-y-4">
                      <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.field.paths")}</div>
                      <Field label={t("app.workspaces.executionWorkspaceDetail.field.workingDirectory")}>
                        <Input
                          className="font-mono"
                          value={form.cwd}
                      ...
                      <Field label={t("app.workspaces.executionWorkspaceDetail.field.providerPathRef")}>
                        <Input
                          className="font-mono"
                          value={form.providerRef}
                          onChange={(event) => setForm((current) => current ? { ...current, providerRef: event.target.value } : current)}
                          placeholder={t("app.workspaces.executionWorkspaceDetail.field.providerPathRefPlaceholder")}
                        />
                      </Field>
                    </div>
                ...
                */}
                {!hideHostPaths && (
                  <>
                    <div className="space-y-4">
                      <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.field.lifecycleCommands")}</div>
                      <Field label={t("app.projects.properties.provisionCommand")} hint={t("app.workspaces.executionWorkspaceDetail.field.provisionCommandHint")}>
                        <Textarea
                      ...
                      <Field
                        label={t("app.projects.properties.runtimeProvisionCommand")}
                        hint={t("app.workspaces.executionWorkspaceDetail.field.runtimeProvisionCommandHint")}
                      >
                        <Textarea
                          className="min-h-20 font-mono"
                      ...
                      <Field label={t("app.workspaces.executionWorkspaceDetail.field.teardownCommand")} hint={t("app.workspaces.executionWorkspaceDetail.field.teardownCommandHint")}>
                        <Textarea
                          className="min-h-20 font-mono"
                      ...
                      <Field label={t("app.workspaces.executionWorkspaceDetail.field.cleanupCommand")} hint={t("app.workspaces.executionWorkspaceDetail.field.cleanupCommandHint")}>
                        <Textarea
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.runtimeConfig.title")}</div>
                  <div className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        <div className="text-sm font-medium text-foreground">
                          {t("app.workspaces.executionWorkspaceDetail.runtimeConfig.source")}
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {runtimeConfigSource === "execution_workspace"
                            ? t("app.workspaces.executionWorkspaceDetail.runtimeConfig.overriding")
                            : runtimeConfigSource === "project_workspace"
                              ? t("app.workspaces.executionWorkspaceDetail.runtimeConfig.inheriting")
                              : t("app.workspaces.executionWorkspaceDetail.runtimeConfig.none")}
                        </p>
                      </div>
                      <Button
                        variant="outline"
                      ...
                      >
                        {t("app.workspaces.executionWorkspaceDetail.runtimeConfig.resetToInherit")}
                      </Button>
                    </div>
                  </div>

                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("app.workspaces.executionWorkspaceDetail.runtimeConfig.advanced")}</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("app.workspaces.executionWorkspaceDetail.runtimeConfig.advancedHint")}
                    </p>
                    <div className="mt-3">
                      <Field label={t("app.workspaces.executionWorkspaceDetail.runtimeConfig.commandsJson")} hint={t("app.workspaces.executionWorkspaceDetail.runtimeConfig.commandsJsonHint")}>
                        <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          />
                          <label htmlFor="inherit-runtime-config">{t("app.workspaces.executionWorkspaceDetail.runtimeConfig.inherit")}</label>
                        </div>
                        <Textarea
                  ...
                  {configuredRuntimeServicePorts.length > 0 ? (
                    <div className="space-y-3 rounded-md border border-border bg-muted/20 p-4">
                      <div>
                        <div className="text-sm font-medium">{t("app.workspaces.executionWorkspaceDetail.ports.title")}</div>
                        <p className="mt-1 text-sm text-muted-foreground">
                          {t("app.workspaces.executionWorkspaceDetail.ports.description")}
                        </p>
                      </div>
                      <div className="grid gap-3 sm:grid-cols-2">
                        {configuredRuntimeServicePorts.map((service) => (
                          <Field key={`${service.collection}-${service.index}`} label={service.name} hint={t("app.workspaces.executionWorkspaceDetail.ports.fixedPort")}>
                            <Input
                              type="number"
                              min="1"
                              max="65535"
                      ...
                      <p className="text-sm text-muted-foreground">
                        {t("app.workspaces.executionWorkspaceDetail.ports.recheck")}
                      </p>
                    </div>
                  ) : null}
                </div>
              ...
              <div className="mt-6 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                ...
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.noUnsavedChanges")}</p> : null}
              </div>
              </CardContent>
            </Card>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("app.workspaces.executionWorkspaceDetail.context.title")}</CardTitle>
                <CardDescription>{t("app.workspaces.executionWorkspaceDetail.context.description")}</CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <DetailRow label={t("app.workspaces.executionWorkspaceDetail.context.sourceTask")}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("app.workspaces.executionWorkspaceDetail.context.derivedFrom")}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("app.workspaces.executionWorkspaceDetail.context.runtimeProvisioning")}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("app.workspaces.executionWorkspaceDetail.location.title")}</CardTitle>
                <CardDescription>{t("app.workspaces.executionWorkspaceDetail.location.description")}</CardDescription>
              </CardHeader>
              <CardContent>
              <DetailRow label={t("app.workspaces.executionWorkspaceDetail.location.workingDir")}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("app.common.none")}
              </DetailRow>
              <DetailRow label={t("app.workspaces.executionWorkspaceDetail.location.providerRef")}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("app.common.none")}
              </DetailRow>
              <DetailRow label={t("app.workspaces.executionWorkspaceDetail.field.repoUrl")}>
                {workspace.repoUrl && isSafeExternalUrl(workspace.repoUrl) ? (
                  <div className="inline-flex max-w-full items-start gap-2">
                    ...
                    <CopyText text={workspace.repoUrl} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel={t("app.common.states.copied")}>
                      <Copy className="h-3.5 w-3.5" />
                    </CopyText>
                  </div>
                ) : workspace.repoUrl ? (
              ...
              <DetailRow label={t("app.workspaces.executionWorkspaceCloseDialog.baseRef")}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : t("app.common.none")}
              </DetailRow>
              <DetailRow label={t("app.workspaces.executionWorkspaceCloseDialog.branch")}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : t("app.common.none")}
              </DetailRow>
              <DetailRow label={t("app.workspaces.executionWorkspaceDetail.location.opened")}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("app.common.labels.lastUsed")}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("app.workspaces.executionWorkspaceDetail.location.cleanup")}>
                {workspace.cleanupEligibleAt
                  ? `${formatDateTime(workspace.cleanupEligibleAt)}${workspace.cleanupReason ? ` · ${workspace.cleanupReason}` : ""}`
                  : t("app.workspaces.executionWorkspaceDetail.location.notScheduled")}
              </DetailRow>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("app.workspaces.executionWorkspaceDetail.logs.title")}</CardTitle>
              <CardDescription>{t("app.workspaces.executionWorkspaceDetail.logs.description")}</CardDescription>
            </CardHeader>
            <CardContent>
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.logs.loading")}</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : t("app.workspaces.executionWorkspaceDetail.logs.loadFailed")}
              </p>
              ...
              <p className="text-sm text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.logs.empty")}</p>
            )}
            </CardContent>
          </Card>
        ) : activeTab === "issues" ? (
          <div className="space-y-6">
            <SummarySlotCard
              ...
              title={t("app.workspaces.workspaces.summaryTitle")}
              description={t("app.workspaces.executionWorkspaceDetail.summaryDescription")}
            />
          ...
          <Card>
            <CardContent className="py-6 text-sm text-muted-foreground">{t("app.workspaces.executionWorkspaceDetail.loadingPlugin")}</CardContent>
          </Card>
          ...
          <MissingPluginTabPlaceholder
            ...
            defaultTabLabel={t("app.workspaces.executionWorkspaceDetail.backToTasks")}
          />
        ) : activeTab === "routines" ? (
