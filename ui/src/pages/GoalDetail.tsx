...
export function GoalPropertiesToggleButton({
  ...
  return (
    <Button
      ...
      title={t("app.goals.detail.showProperties")}
    >
...
export function GoalDetail() {
  ...
  const uploadImage = useMutation({
    mutationFn: async (file: File) => {
      if (!resolvedCompanyId) throw new Error(t("app.goals.detail.noOrgSelected"));
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("app.pages.goals"), href: "/goals" },
      { label: goal?.title ?? goalId ?? t("app.goals.detail.fallbackTitle") }
    ]);
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase text-muted-foreground">
            {t(`app.goals.level.${goal.level}`, { defaultValue: goal.level })}
          </span>
        ...
        <InlineEditor
          ...
          placeholder={t("app.goals.detail.addDescription")}
          multiline
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {t("app.goals.detail.subGoalsTab", { n: childGoals.length })}
          </TabsTrigger>
          <TabsTrigger value="projects">
            {t("app.goals.detail.projectsTab", { n: linkedProjects.length })}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          <div className="flex items-center justify-start">
            ...
            >
              ...
              {t("app.goals.detail.subGoal")}
            </Button>
          </div>
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("app.goals.detail.noSubGoals")}</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("app.goals.detail.noLinkedProjects")}</p>
          ) : (
