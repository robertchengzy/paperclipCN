...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("app.common.nouns.costs") }]);
  }, [setBreadcrumbs, t]);
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message={t("app.reports.costs.selectACompanyToViewCosts")} />;
  }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="budgets">{t("app.reports.costs.budgets")}</TabsTrigger>
            ...
            <>
              ...
              <div className="space-y-5">
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(`app.reports.costs.productionScope${scopeType}`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("app.reports.costs.companywideMonthlyPolicy")
                            : scopeType === "agent"
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">{t("app.reports.costs.noBudgetPoliciesYetSetAgentAndProjectBudgetsFromTheirDetail")}</CardContent>
                  </Card>
                ) : null}
              </div>
            </>
