...
function normalizeError(error: unknown): FileViewerErrorShape {
  ...
  return { status: 0, code: "", message: translate("app.workspaces.fileViewerSheet.somethingWentWrong") };
}
...
export function describeDenial(code: string, fallback: string): { title: string; body: string; icon: ReactNode } {
  ...
  if (lower.includes("policy") || lower.includes("denied") || lower.includes("sensitive")) {
    return {
      ...
      title: translate("app.workspaces.fileViewerSheet.denial.policyTitle"),
      body: translate("app.workspaces.fileViewerSheet.denial.policyBody"),
    };
  }
  if (lower.includes("outside") || lower.includes("traversal")) {
    return {
      icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
      title: translate("app.workspaces.fileViewerSheet.denial.outsideTitle"),
      body: translate("app.workspaces.fileViewerSheet.denial.outsideBody"),
    };
  }
  if (lower.includes("archive") || lower.includes("cleaned")) {
    return {
      icon: <FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />,
      title: translate("app.workspaces.fileViewerSheet.denial.cleanedTitle"),
      body: translate("app.workspaces.fileViewerSheet.denial.cleanedBody"),
    };
  }
  if (lower.includes("remote")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: translate("app.workspaces.fileViewerSheet.denial.remoteTitle"),
      body: translate("app.workspaces.fileViewerSheet.denial.remoteBody"),
    };
  }
  if (lower.includes("too_large") || lower.includes("size")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: translate("app.workspaces.fileViewerSheet.denial.tooLargeTitle"),
      body: translate("app.workspaces.fileViewerSheet.denial.tooLargeBody"),
    };
  }
  if (lower.includes("binary") || lower.includes("unsupported")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: translate("app.workspaces.fileViewerSheet.denial.unsupportedTitle"),
      body: translate("app.workspaces.fileViewerSheet.denial.unsupportedBody"),
    };
  }
  return {
    icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
    title: translate("app.workspaces.fileViewerSheet.denial.fallbackTitle"),
    body: fallback || translate("app.workspaces.fileViewerSheet.denial.fallbackBody"),
  };
}
        ...
        <>
          ...
          {state?.line ? (
            <>
              ...
              <span>
                {state.column
                  ? t("app.workspaces.fileViewerSheet.lineColumn", { line: state.line, column: state.column })
                  : t("app.workspaces.fileViewerSheet.line", { line: state.line })}
              </span>
            </>
          ) : null}
        </>
      ) : state ? (
        <span className="h-3 w-28 rounded bg-muted animate-pulse" aria-label={t("app.workspaces.fileViewerSheet.loadingDetails")} />
      ) : null}
    </div>
  );
}
...
export function FileContentViewer({ content, highlightedLine, onLoaded }: FileContentViewerProps) {
  ...
  useEffect(() => {
    if (!lines) return;
    onLoaded?.(lines.length === 1
      ? t("app.workspaces.fileViewerSheet.loadedOneLine")
      : t("app.workspaces.fileViewerSheet.loadedLines", { count: lines.length }));
  }, [lines, onLoaded, t]);
  ...
  if (resource.previewKind === "image") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("app.workspaces.fileViewerSheet.imageUnavailable")}
        />
      );
    }
    return (
  ...
  if (resource.previewKind === "video") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("app.workspaces.fileViewerSheet.videoUnavailable")}
        />
      );
    }
    return (
      <div className="flex flex-1 items-center justify-center overflow-auto bg-black p-4">
        <video
          ...
          preload="metadata"
          playsInline
          aria-label={t("app.workspaces.fileViewerSheet.videoPreview", { title: resource.title })}
  ...
  if (resource.previewKind === "unsupported" || !lines) {
    return (
      <FileViewerStateView
        icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
        title={t("app.workspaces.fileViewerSheet.denial.unsupportedTitle")}
        body={resource.contentType ? t("app.workspaces.fileViewerSheet.contentType", { contentType: resource.contentType }) : undefined}
      />
    );
  }
  ...
  const rawSourceView = (
    <div
      ref={codeScrollRef}
      role="region"
      aria-label={t("app.workspaces.fileViewerSheet.sourceRegion", { title: resource.title })}
      tabIndex={0}
  ...
  if (elapsedMs < 400) {
    return (
      <div className="flex-1 space-y-2 p-6" aria-busy="true" aria-live="polite">
        <span className="sr-only">{t("app.workspaces.fileViewerSheet.loadingPreview")}</span>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-muted-foreground">
        ...
        {t("app.workspaces.fileViewerSheet.loadingPreviewEllipsis")}
      </div>
    </div>
  );
}
...
export function FileViewerSheet({
  ...
  useEffect(() => {
    if (resolveQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("app.workspaces.fileViewerSheet.unableToLoad"));
    }
  ...
  useEffect(() => {
    if (contentQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("app.workspaces.fileViewerSheet.unableToLoadContent"));
    }
  ...
  const copyToClipboard = useCallback(async (value: string, field: "content" | "link", message: string) => {
    try {
      ...
      showCopyFeedback(null, t("app.common.messages.copyFailed"));
    } finally {
  ...
  const handleCopyContent = useCallback(() => {
    ...
    void (async () => {
      ...
      if (!content) {
        showCopyFeedback(null, t("app.workspaces.fileViewerSheet.contentsUnavailable"));
        return;
      }
      const message = content.content.encoding === "base64" ? t("app.workspaces.fileViewerSheet.copiedData") : t("app.workspaces.fileViewerSheet.copiedContents");
  ...
  const handleCopyLink = useCallback(() => {
    ...
    void copyToClipboard(window.location.href, "link", t("app.workspaces.fileViewerSheet.copiedLink"));
  }, [copyToClipboard, t]);
  ...
  const title = state ? basename(state.path) : t("app.workspaces.fileViewerSheet.browseWorkspace");
  const description = state
    ? middleTruncatePath(state.path)
    : t("app.workspaces.fileViewerSheet.browseDescription");
            ...
            <div className="flex shrink-0 items-center gap-1 self-start">
              ...
              {cameFromBrowse ? (
                <Button
                  ...
                  aria-label={t("app.workspaces.fileViewerSheet.backToFiles")}
                >
                ...
                downloadUrl ? (
                  ...
                  >
                    <a
                      ...
                      aria-label={t("app.workspaces.fileViewerSheet.downloadFile")}
                      title={t("app.workspaces.fileViewerSheet.downloadFile")}
                    >
              ...
              {state ? (
                <Button
                  ...
                  aria-label={copiedField === "content" ? t("app.workspaces.fileViewerSheet.copiedFileContents") : t("app.workspaces.fileViewerSheet.copyFileContents")}
                  title={copiedField === "content" ? t("app.workspaces.fileViewerSheet.copiedContents") : t("app.workspaces.fileViewerSheet.copyFileContents")}
                  className="h-7 w-7"
                >
              ...
              {state ? (
                <Button
                  ...
                  aria-label={copiedField === "link" ? t("app.workspaces.fileViewerSheet.copiedViewLink") : t("app.workspaces.fileViewerSheet.copyViewLink")}
                  title={copiedField === "link" ? t("app.workspaces.fileViewerSheet.copiedLink") : t("app.common.actions.copyLink")}
                  className="h-7 w-7"
                >
                  {copyingField === "link" ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
              ...
              <Button
                ...
                className="h-7 w-7"
                aria-label={t("app.workspaces.fileViewerSheet.closeViewer")}
                title={t("app.common.actions.close")}
              >
                <X className="h-4 w-4" />
              </Button>
            </div>
          ...
          {state ? (
            <div className="flex min-h-0 flex-1 gap-3 bg-muted/30 p-3">
              ...
              <div
                ...
                aria-label={t("app.workspaces.fileViewerSheet.resizeTree")}
  ...
  if (resolveQuery.isError) {
    ...
    if (normalized.status === 404) {
      return (
        <FileViewerStateView
          ...
          title={t("app.workspaces.fileViewerSheet.notFound")}
          body={t("app.workspaces.fileViewerSheet.notFoundBody")}
          actions={
            <>
              {onFallbackToProject ? (
                <Button type="button" variant="secondary" size="sm" onClick={onFallbackToProject}>
                  {t("app.workspaces.fileViewerSheet.tryProjectWorkspace")}
                </Button>
              ) : null}
              <Button type="button" variant="ghost" size="sm" onClick={onRetry}>
                <RefreshCcw aria-hidden="true" className="mr-1 h-3 w-3" /> {t("app.common.actions.retry")}
              </Button>
            </>
          }
        />
    ...
    if (normalized.status === 422) {
      return (
        <FileViewerStateView
          icon={<FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />}
          title={t("app.workspaces.fileViewerSheet.noWorkspace")}
          body={t("app.workspaces.fileViewerSheet.noWorkspaceBody")}
        />
      );
    }
  ...
  if (resource.kind === "remote_resource") {
    return (
      <FileViewerStateView
        ...
        title={t("app.workspaces.fileViewerSheet.remoteSoon")}
        body={t("app.workspaces.fileViewerSheet.remoteSoonBody")}
      />
    );
  }
