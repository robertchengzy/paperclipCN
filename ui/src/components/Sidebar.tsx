      ...
      <nav className={primarySidebarStyles.nav}>
        <div className={primarySidebarStyles.group}>
          ...
          {(() => {
            const newTaskButton = (
              <button
                ...
                aria-label={rail ? t("app.sidebar.newTask") : undefined}
                className={cn(
              ...
              >
                ...
                <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : "truncate"}>{t("app.sidebar.newTask")}</span>
              </button>
            );
            return rail ? (
              <Tooltip>
                <TooltipTrigger asChild>{newTaskButton}</TooltipTrigger>
                <TooltipContent side="right">{t("app.sidebar.newTask")}</TooltipContent>
              </Tooltip>
            ) : (
              newTaskButton
            );
          ...
          {showDecisions ? (
            <SidebarNavItem
              to="/decisions"
              label={t("app.sidebar.decisions")}
              icon={ListChecks}
              badge={attentionCount}
              badgeLabel={t("app.sidebar.decisions")}
            />
          ) : null}
          {showStatusCards ? (
            <SidebarNavItem to="/status" label={t("app.sidebar.status")} icon={LayoutGrid} textBadge={t("app.sidebar.beta")} />
          ) : null}
          {conferenceRoomChatEnabled ? (
            <SidebarNavItem to="/board-chat" label={t("app.sidebar.conferenceRoom")} icon={MessagesSquare} />
          ) : null}
        </div>
        ...
        <SidebarSection label={t("app.sidebar.work")} collapsible={{ open: workOpen, onOpenChange: setWorkOpen }}>
          ...
          <SidebarNavItem to="/routines" label={t("app.sidebar.routines")} icon={Repeat} />
          <SidebarNavItem to="/artifacts" label={t("app.sidebar.artifacts")} icon={Package} />
          {showCases ? (
            <SidebarNavItem to="/cases" label={t("app.sidebar.cases")} icon={Layers} textBadge={t("app.sidebar.beta")} />
          ) : null}
          {showPipelines ? (
            <SidebarNavItem to="/pipelines" label={t("app.sidebar.pipelines")} icon={GitBranch} />
          ) : null}
          {showGoalsLink ? (
            <SidebarNavItem to="/goals" label={t("app.sidebar.goals")} icon={Target} />
          ) : goalsLinkPending ? (
            <div
        ...
        {streamlinedUiEnabled ? (
          <SidebarSection
            label={t("app.sidebar.org")}
            collapsible={{ open: organizationOpen, onOpenChange: setOrganizationOpen }}
          >
            <SidebarNavItem to="/agents" label={t("app.sidebar.agents")} icon={Users} />
            <SidebarNavItem to="/skills" label={t("app.sidebar.skills")} icon={Boxes} />
            <SidebarNavItem to="/apps" label={t("app.sidebar.connectors")} icon={Unplug} />
            <SidebarNavItem to="/activity" label={t("app.sidebar.audit")} icon={History} />
          </SidebarSection>
        ) : null}
        ...
        {streamlinedUiEnabled ? (
          ...
          <>
            ...
            <SidebarAgents />
            <SidebarSection
              label={t("app.sidebar.organization")}
              collapsible={{ open: organizationOpen, onOpenChange: setOrganizationOpen }}
            >
              <SidebarNavItem to="/org" label={t("app.sidebar.org")} icon={Network} />
              <SidebarNavItem to="/apps" label={t("app.sidebar.connectors")} icon={Unplug} />
              <SidebarNavItem to="/timeline" label={t("app.sidebar.timeline")} icon={GanttChartSquare} />
              <SidebarNavItem to="/costs" label={t("app.sidebar.costs")} icon={DollarSign} />
              <SidebarNavItem to="/activity" label={t("app.sidebar.activity")} icon={History} />
              <SidebarNavItem to="/company/settings" label={t("app.sidebar.settings")} icon={Settings} />
            </SidebarSection>
          </>
        )}
