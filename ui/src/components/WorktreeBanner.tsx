  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap">
        <span className="shrink-0 opacity-70">{t("app.shell.worktreeBanner.worktree")}</span>
        <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-current opacity-70" aria-hidden="true" />
        <button
          type="button"
          onClick={handleCopyName}
          title={t("app.shell.worktreeBanner.clickToCopyWorktreeName")}
        ...
        >
          {copied ? t("app.shell.worktreeBanner.copied") : failed ? t("app.common.messages.copyFailed") : branding.name}
        </button>
      </div>
    </div>
  );
