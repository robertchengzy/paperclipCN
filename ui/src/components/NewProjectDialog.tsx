  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !create.isPending) onClose(); }}>
    <DialogContent showCloseButton={false} aria-describedby={undefined}
      ...
      {connecting ? <div className="min-h-0 overflow-y-auto p-5">
        <DialogTitle className="sr-only">{t("app.projects.dialog.connectGithub")}</DialogTitle>
        ...
        <div className="flex shrink-0 flex-col gap-4 px-5 pb-4 pt-5">
          <div className="flex items-center justify-between gap-3">
            <DialogTitle className="text-lg font-semibold">{t("app.projects.dialog.title")}</DialogTitle>
            <Button type="button" variant="ghost" size="icon-sm" disabled={create.isPending} aria-label={t("app.projects.dialog.close")} onClick={onClose}><X className="size-4" /></Button>
          </div>
          <div className="flex items-center gap-3 rounded-lg border border-input px-3 focus-within:border-ring focus-within:ring-1 focus-within:ring-ring">
            ...
            <input ref={input} aria-label={t("app.projects.dialog.nameLabel")} value={name} disabled={create.isPending} onChange={(event) => setName(event.target.value)} placeholder={t("app.projects.dialog.nameLabel")} required
        ...
        <div role="region" aria-label={t("app.projects.dialog.sourceRepos")} tabIndex={0} className="min-h-0 overflow-y-auto overscroll-contain px-5 pb-1 outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring">
        ...
        <div className="flex shrink-0 justify-end gap-2 px-5 py-5">
          <Button type="button" variant="ghost" disabled={create.isPending} onClick={onClose}>{t("app.common.actions.cancel")}</Button>
          <Button type="submit" disabled={!name.trim() || create.isPending}>{create.isPending ? t("app.projects.dialog.creating") : t("app.projects.dialog.title")}</Button>
        </div>
      </form>}
    </DialogContent>
  </Dialog>;
