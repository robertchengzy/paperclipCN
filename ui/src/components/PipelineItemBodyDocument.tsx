...
function getPipelineRevisionActor(
  ...
  return { kind: "system", name: translate("app.common.labels.system") };
}
...
export function PipelineItemBodyDocument({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await invalidateAll();
      pushToast({ title: t("app.pipelines.pipelineItemBodyDocument.revisionRestored"), tone: "success" });
    },
    onError: () => pushToast({ title: t("app.pipelines.pipelineItemBodyDocument.restoreFailed"), tone: "error" }),
  });
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      if (error instanceof ApiError && error.status === 409) {
        ...
        pushToast({
          title: t("app.pipelines.pipelineItemBodyDocument.conflictTitle"),
          body: t("app.pipelines.pipelineItemBodyDocument.conflictBody"),
          tone: "error",
        });
        return;
      }
      pushToast({ title: t("app.pipelines.pipelineItemBodyDocument.saveFailed"), tone: "error" });
    }
  ...
  if (editing) {
      ...
      >
        <div className="rounded-md border border-border bg-background">
          <MarkdownEditor
            ...
            placeholder={t("app.pipelines.pipelineItemBodyDocument.editorPlaceholder")}
            bordered={false}
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <span className="text-(length:--text-micro) text-muted-foreground">
            {t("app.pipelines.pipelineItemBodyDocument.editorHint", { number: (doc?.latestRevisionNumber ?? 0) + 1 })}
          </span>
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" onClick={() => void handleSave()} disabled={saveMutation.isPending}>
              ...
              {saveMutation.isPending ? t("app.common.actions.saving") : t("app.common.actions.save")}
            </Button>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                {t("app.pipelines.pipelineItemBodyDocument.viewingRevision", { number: selectedHistoricalRevision.revisionNumber })}
              </p>
              <p className="text-xs text-muted-foreground">
                {t("app.pipelines.pipelineItemBodyDocument.historicalPreview")}
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setSelectedRevisionId(null)}>
                {t("app.pipelines.pipelineItemBodyDocument.returnToLatest")}
              </Button>
              <Button
                size="sm"
              ...
              >
                {restoreMutation.isPending ? t("app.pipelines.pipelineItemBodyDocument.restoring") : t("app.pipelines.pipelineItemBodyDocument.restoreThisRevision")}
              </Button>
            </div>
          </div>
        </div>
    ...
    // Truly empty (A).
    bodyContent = (
      <EmptyState icon={FileText} message={t("app.pipelines.pipelineItemBodyDocument.emptyMessage")} action={t("app.pipelines.pipelineItemBodyDocument.emptyAction")} onAction={beginEdit} />
    );
  ...
  return (
    <section
      aria-label={t("app.pipelines.pipelineItemBodyDocument.itemBody")}
    ...
    >
      <DocumentFrameHeader
        ...
        documentLabel={t("app.pipelines.pipelineItemBodyDocument.itemBodyDocument")}
        folded={folded}
        ...
        actionsSlot={editing ? (
          <span className="text-(length:--text-micro) font-medium text-amber-700 dark:text-amber-300">{t("app.pipelines.pipelineItemBodyDocument.editingUnsaved")}</span>
        ) : (
