...
export function AgentProviderConnection({
  ...
  async function connect() {
    ...
    try {
      ...
      if (connected) onConnected(connection);
        ...
        setError(
          t("app.agentSetup.connection.noResponse"),
        );
    } catch (cause) {
      if (run !== epoch.current) return;
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : t("app.agentSetup.connection.connectFailed"),
      );
    } finally {
  ...
  return (
    <div className="min-w-0 max-w-full">
      <ModelSourceTiles
        label={t("app.agentSetup.connection.sourcesLabel")}
        sources={[
          {
            id: adapterType,
            label: provider,
      ...
      {!opened && savedKeys.options.length > 0 && (
        <p className="mt-2 text-sm text-muted-foreground">
          {savedKeys.options.length === 1
            ? t("app.agentSetup.connection.savedKeysOne", { n: savedKeys.options.length })
            : t("app.agentSetup.connection.savedKeysMany", { n: savedKeys.options.length })}
        </p>
      )}
      {method === "subscription" &&
            ...
            {method === "api" ? (
              <OnboardingLoginCard
                instruction={
                  savedKeys.options.length
                    ? t("app.agentSetup.connection.chooseSavedKey")
                    : t("app.agentSetup.connection.provideKey", { provider })
                }
              >
                <SavedProviderKeySelect
                  {...savedKeys}
                ...
                {!selectedKey && (
                  <OnboardingCardField
                    label={t("app.agentSetup.connection.apiKey")}
                    masked
                    autoFocus
                    value={apiKey}
                    placeholder={
                      storedConnection
                        ? t("app.agentSetup.connection.keyEntered")
                        : t("app.agentSetup.connection.enterKey")
                    }
                    onChange={(value) => {
                      setSelectedKeyId("");
                      setApiKey(value);
              ...
              <AdapterLoginPanel
                ...
                onConnected={(sessionId) => {
                  if (managedAccount) {
                    if (!sessionId) { setError(t("app.agentSetup.connection.loginNoConnection")); return; }
                    const run = epoch.current;
                    setLoginPhase("connecting");
                    void aiConnectionsApi.loginResult(companyId, sessionId).then((result) => {
                      ...
                      if (run !== epoch.current) return;
                      setLoginPhase("ready");
                      setError(t("app.agentSetup.connection.retrieveFailed"));
                    });
                    return;
                  }
              ...
              <p className="text-sm text-muted-foreground">
                {storedLogin.data
                  ? t("app.agentSetup.connection.useSavedClaude")
                  : canLogin
                    ? t("app.agentSetup.connection.useExisting")
                    : t("app.agentSetup.connection.noBrowserSignIn")}
              </p>
            )}
          </div>
        )}
      ...
      {method === "subscription" && storedLogin.isError && (
        <p role="alert" className="mt-4 text-sm text-destructive">
          {t("app.agentSetup.connection.checkClaudeFailed")}
        </p>
      )}
      {error && (
        <p role="alert" className="mt-4 text-sm text-destructive">
      ...
      {localEnvironment && health.isError && (
        <p role="alert" className="mt-4 text-sm text-destructive">{t("app.agentSetup.connection.prepareSignInFailed")}</p>
      )}
      <FooterNav
        onBack={() => {
          if (opened) cancel();
        ...
        primaryLabel={
          opened && needsLogin
            ? loginPhase === "waiting" ? t("app.agentSetup.connection.waitingForCode")
              : loginPhase === "connecting" ? t("app.agentSetup.connection.connecting")
              : t("app.agentSetup.connection.signIn", { provider })
            : busy
            ? t("app.agentSetup.connection.connecting")
            : method === "subscription" &&
                (storedLogin.data || savedSubscription)
              ? t("app.agentSetup.connection.useSavedSubscription")
              : method === "api" && selectedKey
                ? t("app.agentSetup.connection.useSavedApiKey")
                : t("app.agentSetup.connection.connect")
        }
        primaryDisabled={
