...
export function SidebarAccountMenu({
  ...
  const displayName = session?.user.name?.trim() || t("app.sidebarAccountMenu.board");
  const secondaryLabel =
    session?.user.email?.trim() || (deploymentMode === "authenticated" ? t("app.sidebarAccountMenu.signedIn") : t("app.sidebarAccountMenu.localWorkspaceBoard"));
        ...
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <button
              ...
              aria-label={t("app.sidebarAccountMenu.openAccountMenu")}
            >
              <Avatar size="sm">
          ...
          >
            ...
            <div className="flex flex-1 flex-col gap-0.5 border-t border-border px-2.5 pb-2.5 pt-2">
              ...
              />
              <MenuAction
                label={t("app.sidebarAccountMenu.viewProfile")}
                icon={UserRound}
                href={profileHref}
                onClick={closeNavigationChrome}
              />
              <MenuAction
                label={t("app.sidebarAccountMenu.editProfile")}
                icon={UserRoundPen}
                href={PROFILE_SETTINGS_PATH}
                onClick={closeNavigationChrome}
              />
              <MenuAction
                label={t("app.sidebarAccountMenu.documentation")}
                icon={BookOpen}
                href={DOCS_URL}
                external
                onClick={() => setOpen(false)}
              />
              <div className="flex h-(--profile-popover-row-height) items-center justify-between gap-2 px-2.5">
                <span className="text-(length:--text-compact) font-medium text-foreground">{t("app.language.label")}</span>
                <LanguageSwitcher />
              </div>
              ...
              {deploymentMode === "authenticated" ? (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1 truncate">
                    {signOutMutation.isPending ? t("app.sidebarAccountMenu.signingOut") : t("app.sidebarAccountMenu.signOut")}
                  </span>
                </button>
              ) : null}
              <SidebarServerInfo />
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                aria-label={t("app.sidebarAccountMenu.shareFeedbackAction")}
            ...
            <TooltipContent side="top">{t("app.sidebarAccountMenu.shareFeedbackAction")}</TooltipContent>
          </Tooltip>
        ) : null}
      </div>
    </div>
