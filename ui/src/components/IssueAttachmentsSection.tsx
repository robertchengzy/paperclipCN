    ...
    <div className="flex shrink-0 items-center gap-1">
      {onPreview ? (
        <Button
          variant="ghost"
          size="icon-sm"
          title={t("app.issueUi.issueAttachmentsSection.browseGallery")}
          aria-label={t("app.issueUi.issueAttachmentsSection.browseInGallery", { filename })}
      ...
      <Button asChild variant="ghost" size="icon-sm" title={t("app.common.actions.download")}>
        <a href={attachmentDownloadPath(attachment)} aria-label={t("app.issueUi.issueAttachmentsSection.downloadFile", { filename })}>
      ...
      {onDelete ? (
        <Button
          variant="ghost"
          size="icon-sm"
          title={t("app.issueUi.issueAttachmentsSection.deleteAttachment")}
...
function AttachmentMeta({ attachment }: { attachment: IssueAttachment }) {
  ...
  return (
    <p className="mt-0.5 text-(length:--text-micro) text-muted-foreground">
      {t("app.issueUi.issueAttachmentsSection.meta", { contentType: attachment.contentType, size: formatBytes(attachment.byteSize) })}
    </p>
  );
}
  ...
  return (
    <div id={`attachment-${attachment.id}`} className="scroll-mt-20 rounded-lg border border-border p-3">
      ...
      <div className="mt-3 rounded-md hover:bg-accent/10">
        {isLoading ? (
          <p className="px-1 py-2 text-xs text-muted-foreground">{t("app.issueUi.issueAttachmentsSection.loadingPreview")}</p>
        ) : error ? (
          <p className="px-1 py-2 text-xs text-destructive">{t("app.issueUi.issueAttachmentsSection.previewFailed")}</p>
        ) : (
          <FoldCurtain>
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-muted-foreground">{t("app.issueUi.issueAttachmentsSection.title")}</h3>
          ...
          {imageAttachments.map((attachment) => (
            ...
            >
              <img
                ...
                alt={attachment.originalFilename ?? t("app.issueUi.issueAttachmentsSection.altFallback")}
              ...
              {onDelete && confirmDeleteId === attachment.id ? (
                ...
                >
                  <p className="text-xs font-medium text-white">{t("app.issueUi.issueAttachmentsSection.deleteQuestion")}</p>
      ...
      {onDelete && confirmDeleteId && !imageAttachments.some((attachment) => attachment.id === confirmDeleteId) ? (
        <div className="flex items-center justify-between gap-3 rounded-md border border-destructive/20 bg-destructive/5 px-4 py-3">
          <p className="text-sm font-medium text-destructive">{t("app.issueUi.issueAttachmentsSection.deleteConfirm")}</p>
          <div className="flex shrink-0 items-center gap-2">
            ...
            <Button variant="destructive" size="sm" onClick={() => confirmDelete(confirmDeleteId)} disabled={deletePending}>
              {deletePending ? t("app.issueUi.issueAttachmentsSection.deleting") : t("app.common.actions.delete")}
            </Button>
          </div>
        </div>
      ) : null}
