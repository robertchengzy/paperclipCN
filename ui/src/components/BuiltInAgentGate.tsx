...
export function BuiltInAgentGate({ agentKey, companyId, featureLabel, children }: BuiltInAgentGateProps) {
  ...
  if (state.status === "not_provisioned" || state.status === "needs_setup") {
    return (
      <>
        <EmptyState
          icon={Bot}
          title={t("app.agentUi.builtInAgentGate.setUpThe", { value0: state.definition.displayName })}
          message={t("app.agentUi.builtInAgentGate.isGeneratedByABuiltInAgent", { value0: label })}
          action={t("app.agentUi.builtInAgentGate.setupAction", { name: state.definition.displayName })}
  ...
  if (state.status === "pending_approval") {
    return (
      <EmptyState
        icon={Clock3}
        title={t("app.agentUi.builtInAgentGate.pendingApproval", { name: state.definition.displayName })}
        message={t("app.agentUi.builtInAgentGate.availableAfterApproval", { label })}
      />
    );
  }
  ...
  if (state.status === "paused" && state.agent) {
    ...
    return (
      <div className="space-y-4">
        <InlineBanner
          tone="warning"
          title={t("app.agentUi.builtInAgentGate.pausedTitle", { label })}
          actions={
            <>
              <Button variant="ghost" size="sm" asChild>
                <Link to={agentUrl(state.agent)}>{t("app.agentUi.builtInAgentGate.viewAgent")}</Link>
              </Button>
              <Button
                size="sm"
              ...
              >
                {resume.isPending ? t("app.common.progress.resuming") : t("app.agentUi.builtInAgentGate.resumeAgent")}
              </Button>
            </>
          }
        >
          {pausedAt
            ? t("app.agentUi.builtInAgentGate.pausedBodyAt", { time: pausedAt, label: label.toLowerCase() })
            : t("app.agentUi.builtInAgentGate.pausedBody", { label: label.toLowerCase() })}
        </InlineBanner>
