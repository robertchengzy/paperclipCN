...
const FILTERS: Array<{ label: string; value: FilterValue }> = [
  { get label() { return t("app.common.labels.all"); }, value: "all" },
  { get label() { return t("app.common.issueStatus.in_progress"); }, value: "in_progress" },
  { get label() { return t("app.shell.artifactsPanel.forReview"); }, value: "for_review" },
  { get label() { return t("app.common.states.completed"); }, value: "completed" },
];
...
function statusBadge(status: string) {
  switch (status) {
    case "active":
    case "draft":
      return { label: t("app.common.issueStatus.in_progress"), className: "bg-blue-500/10 text-blue-600 dark:text-blue-400" };
    case "ready_for_review":
      return { label: t("app.shell.artifactsPanel.forReview"), className: "bg-amber-500/10 text-amber-600 dark:text-amber-400" };
    case "approved":
    case "merged":
      return { label: t("app.common.states.completed"), className: "bg-green-500/10 text-green-600 dark:text-green-400" };
    case "changes_requested":
      return { label: t("app.shell.artifactsPanel.changesRequested"), className: "bg-orange-500/10 text-orange-600 dark:text-orange-400" };
    case "failed":
      return { label: t("app.common.states.failed"), className: "bg-red-500/10 text-red-600 dark:text-red-400" };
    default:
...
export function ArtifactsPanel({ taskId, isAgentWorking, openDocKey, openDocTitle, onClearOpenDoc, onApprove, onReject }: ArtifactsPanelProps) {
  ...
  const effectiveViewingDoc = openDocKey
    ? { key: openDocKey, title: openDocTitle ?? t("app.common.nouns.document") }
    : viewingDoc;
  ...
  return (
    <div className="flex flex-col h-full" data-artifacts-panel>
      <div className="px-4 py-3 border-b border-border flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold">{t("app.common.nouns.artifacts")}</h3>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-auto-hide">
        {isLoading ? (
          <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
            <Loader2 className="h-4 w-4 mr-2 animate-spin" />{t("app.shell.artifactsPanel.loading")}</div>
        ) : filtered.length === 0 ? (
          <div className="px-4 py-8 text-center">
            <Package className="h-8 w-8 mx-auto text-muted-foreground/40 mb-3" />
            <p className="text-sm text-muted-foreground">
              {workProducts?.length === 0
                ? t("app.shell.artifactsPanel.yourTeamSDeliverablesAndPlansWill")
                : t("app.shell.artifactsPanel.noArtifactsMatchThisFilter")}
            </p>
          </div>
        ) : (
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {showGenerating ? (
                          <Badge variant="ghost" className="[&>svg]:size-2.5 text-(length:--text-nano) px-1.5 bg-blue-500/10 text-blue-600 dark:text-blue-400">
                            <Loader2 className="h-2.5 w-2.5 animate-spin" />{t("app.shell.artifactsPanel.generating")}</Badge>
                        ) : (
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-auto-hide p-4">
        {isLoading ? (
          <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
            <Loader2 className="h-4 w-4 mr-2 animate-spin" />{t("app.shell.artifactsPanel.loadingDocument")}</div>
        ) : error ? (
          <p className="text-sm text-muted-foreground">{t("app.shell.artifactsPanel.documentNotAvailableYet")}</p>
        ) : doc?.body ? (
          <div className="prose prose-sm dark:prose-invert max-w-none">
            <MarkdownBody>{doc.body}</MarkdownBody>
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">{t("app.shell.artifactsPanel.documentIsEmpty")}</p>
        )}
      </div>
      ...
      {needsAction && (
        <div className="border-t border-border px-4 py-3 bg-background shrink-0">
          <p className="text-(length:--text-micro) text-muted-foreground mb-2">{t("app.shell.artifactsPanel.thisDocumentNeedsYourReview")}</p>
          <div className="flex items-center gap-3">
            <Button size="lg" className="h-11 px-8 text-base font-semibold flex-1 rounded-lg bg-green-700 hover:bg-green-800 text-white border-0" onClick={onApprove}>{t("app.common.actions.approve")}</Button>
            <Button size="lg" className="h-11 px-8 text-base font-semibold flex-1 rounded-lg bg-red-900 hover:bg-red-950 text-white border-0" onClick={() => {
              onReject?.();
              onBack();
            }}>{t("app.common.actions.reject")}</Button>
          </div>
        </div>
      )}
      {isApproved && (
        <div className="border-t border-green-500/30 bg-green-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-(length:--text-compact) font-medium text-green-700 dark:text-green-400">{t("app.shell.artifactsPanel.approvedHireTasksCreated")}</p>
          </div>
        </div>
      )}
      {isRejected && (
        <div className="border-t border-orange-500/30 bg-orange-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-(length:--text-compact) font-medium text-orange-700 dark:text-orange-400">{t("app.shell.artifactsPanel.changesRequestedCeoIsRevising")}</p>
          </div>
        </div>
      )}
    </div>
