...
export function SkillFolderRail({
  ...
  return (
    <div className="relative hidden h-full shrink-0 md:flex" style={{ width: `${width}px` }}>
      <nav
        aria-label={t("app.skills.skillFolderTree.skillFolders")}
      ...
      <div className="mb-2 flex items-center justify-between gap-2 pt-0.5">
        <div className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">
          {t("app.skills.skillFolderTree.folders")}
        </div>
        <Button variant="ghost" size="icon-sm" title={t("app.skills.skillFolderTree.newFolder")} onClick={() => onCreateFolder(null)}>
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          <VirtualRow
            ...
            label={t("app.skills.skillFolderTree.allSkills")}
            count={allCount}
          ...
          {model.my ? (
            <TreeBranch
              ...
              rootLabel={t("app.skills.skillFolderTree.mySkills")}
          ...
          <RailHeading label={t("app.common.nouns.organization")} onCreate={() => onCreateFolder(null)} />
          {model.company.length > 0 ? (
            ...
            <div className="px-2 py-1 text-xs text-muted-foreground">{t("app.skills.skillFolderTree.noOrganizationFolders")}</div>
          )}
          ...
          {model.projects ? (
            <TreeBranch
              ...
              renamingId={renamingId}
              renameDraft={renameDraft}
              rootLabel={t("app.common.nouns.projects")}
          ...
          {model.bundled ? (
            <TreeBranch
              ...
              renamingId={renamingId}
              renameDraft={renameDraft}
              rootLabel={t("app.skills.skillFolderTree.bundled")}
              rootIcon={<Boxes className="h-3.5 w-3.5" />}
              onToggle={toggle}
              onSelect={onSelect}
              onCreateFolder={onCreateFolder}
          ...
          <VirtualRow
            ...
            label={t("app.skills.skillFolderTree.unfiled")}
            count={unfiledCount}
      ...
      <div
        role="separator"
        aria-label={t("app.skills.skillFolderTree.resizeSkillFolders")}
  ...
  return (
    <div className="group/heading flex items-center justify-between px-2 pb-0.5 pt-3">
      ...
      <button
        type="button"
        onClick={onCreate}
        title={t("app.skills.skillFolderTree.newLabelFolder", { label: label.toLowerCase() })}
  ...
  return (
    <div>
      ...
      >
        <button
          type="button"
          aria-label={isOpen ? t("app.skills.skillFolderTree.collapseFolder") : t("app.skills.skillFolderTree.expandFolder")}
          className={cn(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                aria-label={t("app.skills.skillFolderTree.folderActionsFor", { label })}
              >
            ...
            <DropdownMenuContent align="end">
              {canNest ? (
                <DropdownMenuItem onSelect={() => onCreateFolder(folder.id)}>
                  ...
                  {t("app.skills.skillFolderTree.newSubfolder")}
                </DropdownMenuItem>
              ) : null}
              {editable ? (
                <>
                  <DropdownMenuItem onSelect={() => onStartRename(folder)}>{t("app.common.actions.rename")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onEditFolder(folder)}>{t("app.skills.skillFolderTree.editColor")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onMoveFolder(folder, isInMySkills ? "company" : "my")}>
                    <MoveRight className="h-3.5 w-3.5" />
                    {t("app.skills.skillFolderTree.moveTo", { destination: isInMySkills ? t("app.common.nouns.organization") : t("app.skills.skillFolderTree.mySkills") })}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
...
export function FolderBreadcrumb({
  ...
  return (
    <nav aria-label={t("app.skills.skillFolderTree.folderPath")} className="flex flex-wrap items-center gap-1 text-sm">
      <button
        type="button"
      ...
      >
        ...
        {t("app.skills.skillFolderTree.allSkills")}
      </button>
      {selection === "unfiled" ? (
        <>
          <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/60" />
          <span className="rounded px-1.5 py-0.5 font-medium text-foreground">{t("app.skills.skillFolderTree.unfiled")}</span>
        </>
      ) : null}
...
export function MoveToFolderDialog({
  ...
  const previewPath = chosen === undefined
    ? null
    : chosen === null
      ? t("app.skills.skillFolderTree.unfiled")
    ...
    return (
      <div key={folder.id}>
        ...
        >
          ...
          {isCurrent ? <span className="text-xs text-muted-foreground">{t("app.skills.skillFolderTree.current")}</span> : null}
          {bundled ? <span className="text-xs text-muted-foreground">{t("app.skills.skillFolderTree.readOnly")}</span> : null}
          {isChosen ? <Check className="h-3.5 w-3.5" /> : null}
          {nestable ? (
            <button
              type="button"
              title={t("app.skills.skillFolderTree.newFolderInside")}
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="flex items-center gap-2 rounded-md border border-border px-2.5">
          ...
          <input
            ...
            placeholder={t("app.skills.skillFolderTree.searchFolders")}
        ...
        <div className="max-h-72 overflow-y-auto rounded-md border border-border p-1">
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate">{t("app.skills.skillFolderTree.unfiled")}</span>
            {currentFolderId == null ? <span className="text-xs text-muted-foreground">{t("app.skills.skillFolderTree.current")}</span> : null}
          ...
          <div className="mt-1 border-t border-border pt-1">
            {creatingParent === null ? (
              ...
              >
                <Plus className="h-3.5 w-3.5" />
                {t("app.skills.skillFolderTree.newTopLevelFolder")}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="min-h-5 text-xs text-muted-foreground">
          {previewPath ? (
            <span>
              <Trans
                i18nKey="app.skills.skillFolderTree.movingTo"
                values={{ path: previewPath }}
                components={{ path: <span className="font-mono text-foreground" /> }}
              />
            </span>
          ) : (
            <span>{t("app.skills.skillFolderTree.pickDestination")}</span>
          )}
        </div>
        ...
        <DialogFooter>
          ...
          >
            {pending ? t("app.skills.skillFolderTree.moving") : t("app.skills.skillFolderTree.moveHere")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className="flex items-center gap-2 py-1" style={{ paddingLeft: `${0.5 + depth * 0.9}rem` }}>
      ...
      <Input
        ...
        placeholder={t("app.skills.skillFolderTree.folderName")}
        autoFocus
