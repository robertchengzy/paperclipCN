      ...
      <PopoverTrigger asChild>
        ...
        >
          <span className={cn("min-w-0 truncate", !selectedAgent && "text-muted-foreground")}>
            {selectedAgent?.name ?? placeholder ?? t("app.agentUi.agentMultiSelect.selectAgent")}
          </span>
      ...
      <PopoverContent className="w-(--radix-popover-trigger-width) p-0" align="start">
        <div className="border-b border-border p-3">
          <Input
            ...
            placeholder={t("app.agentUi.agentMultiSelect.filterAgents")}
            className="h-8"
            autoFocus
          />
        </div>
        {agents.length === 0 ? (
          <div className="px-3 py-4 text-sm text-muted-foreground">{emptyMessage ?? t("app.agentUi.agentMultiSelect.noAgents")}</div>
        ) : (
          <div className="max-h-60 overflow-y-auto py-1">
            {filteredAgents.map((agent) => (
              <button
                ...
                aria-label={t("app.agentUi.agentMultiSelect.select", { value0: agent.name })}
                onClick={() => {
                  onChange(agent.id);
                  setOpen(false);
                }}
            ...
            {filteredAgents.length === 0 ? (
              <div className="px-3 py-4 text-sm text-muted-foreground">{t("app.agentUi.agentMultiSelect.noMatches")}</div>
            ) : null}
          </div>
        )}
      </PopoverContent>
            ...
            <span className="flex min-w-0 items-center">
              ...
              <span className="truncate">
                {triggerLabel ?? (selectedCount === 0
                  ? t("app.agentUi.agentMultiSelect.selectAgents")
                  : selectedCount === 1 ? t("app.agentUi.agentMultiSelect.selectedAgentOne", { count: selectedCount }) : t("app.agentUi.agentMultiSelect.selectedAgentMany", { count: selectedCount }))}
              </span>
            </span>
            <ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground" />
          </Button>
        ...
        {loading ? (
              ...
              return (
                ...
                >
                  <Checkbox
                    ...
                    aria-label={t("app.agentUi.agentMultiSelect.allow", { value0: agent.name })}
          ...
          <div className="flex items-center justify-between border-t border-border px-3 py-2">
            <span className="text-xs text-muted-foreground">
              {workingAgentIds.size === 0 ? t("app.agentUi.agentMultiSelect.noAgentsSelected") : t("app.agentUi.agentMultiSelect.selectedCount", { count: workingAgentIds.size })}
            </span>
            <div className="flex items-center gap-2">
              {staged ? (
                <Button type="button" variant="ghost" size="sm" onClick={() => setOpen(false)} disabled={pending}>{t("app.common.actions.cancel")}</Button>
              ) : null}
              <Button
                type="button"
                size="sm"
      ...
      {showSelectionPreview && selectedAgents.length > 0 ? (
        <div className="space-y-0.5">
          ...
          {selectedAgents.length > 3 ? (
            <p className="px-1.5 pt-0.5 text-xs text-muted-foreground">{t("app.agentUi.agentMultiSelect.moreSelected", { count: selectedAgents.length - 3 })}</p>
          ) : null}
        </div>
      ) : null}
    </div>
