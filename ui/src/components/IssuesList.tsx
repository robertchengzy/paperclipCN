...
function progressStatusLabel(t: TFunction, status: IssueStatus): string {
  switch (status) {
    case "backlog": return t("app.issues.progress.status.backlog");
    case "todo": return t("app.issues.progress.status.todo");
    case "in_progress": return t("app.issues.progress.status.in_progress");
    case "in_review": return t("app.issues.progress.status.in_review");
    case "done": return t("app.issues.progress.status.done");
    case "blocked": return t("app.issues.progress.status.blocked");
    case "cancelled": return t("app.issues.progress.status.cancelled");
    default: return status;
  }
}

function taskDateGroupLabel(t: TFunction, group: TaskDateGroup): string {
  switch (group) {
    case "today": return t("app.issues.list.dateGroup.today");
    case "yesterday": return t("app.issues.list.dateGroup.yesterday");
    default: return t("app.issues.list.dateGroup.earlier");
  }
}
...
export function issueAgeSeparatorLabel(bucket: 1 | 2): string {
  return bucket === 1 ? translate("app.issues.list.olderThanDay") : translate("app.issues.list.olderThanWeek");
}
  ...
  return (
    <div className="relative w-full sm:w-64 md:w-80">
      ...
      <Input
        ...
        placeholder={t("app.issues.list.searchPlaceholder")}
        className="pl-7 text-xs sm:text-sm"
        aria-label={t("app.issues.list.searchAria")}
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {t("app.issues.progress.done", { done: summary.doneCount, total: summary.totalCount })}
            </span>
            <span className="text-muted-foreground">
              {t("app.issues.progress.inProgress", { count: summary.inProgressCount })}
            </span>
            <span className="text-muted-foreground">
              {t("app.issues.progress.blocked", { count: summary.blockedCount })}
            </span>
            {showCostSummary && (
              <>
                <span
                ...
                <span className="text-muted-foreground tabular-nums">
                  {t("app.issues.progress.runtime", { value: formatDurationMs(costSummary.runtimeMs) })}
                </span>
              </>
            )}
          </div>
          <div
            role="progressbar"
            aria-label={t("app.issues.progress.ariaLabel")}
            aria-valuemin={0}
          ...
          >
            {statusEntries.map(({ status, count }) => (
              <span
                ...
                title={t("app.issues.progress.segmentTitle", { status: progressStatusLabel(t, status), count })}
                aria-hidden="true"
              />
            ))}
          </div>
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next" ? t("app.issues.progress.nextUp") : t("app.issues.progress.waitingOnBlockers")}
              </div>
              <Link
            ...
            <div className="text-sm font-medium text-foreground">{t("app.issues.progress.noActiveSubTasks")}</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">{t("app.issues.progress.allSubTasksDone")}</div>
          ) : (
            <div className="text-sm font-medium text-foreground">{t("app.issues.progress.noActionableSubTasks")}</div>
          )}
        </div>
      </div>
    </div>
...
function StreamlinedIssuesList({
  ...
  const creatorOptions = useMemo<CreatorOption[]>(() => {
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board" ? t("app.issues.list.creatorBoard") : t("app.issues.list.creatorMe"),
        kind: "user",
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "priority") {
      ...
      return issuePriorityOrder
        ...
        .map((p) => ({ key: p, label: priorityLabel(t, p), items: groups[p]! }));
    }
    if (viewState.groupBy === "workspace") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_workspace" ? t("app.issues.list.noWorkspace") : (workspaceNameMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "project") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_project" ? t("app.issues.list.noProject") : (projectById.get(key)?.name ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "parent") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_parent" ? t("app.issues.list.noParent") : (issueTitleMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    // assignee
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId, companyUserLabelMap) ?? t("app.issues.list.user"))
  ...
  const createActionLabel = createIssueLabel ? t("app.issues.list.createNamed", { label: createIssueLabel }) : t("app.issues.list.createTask");
  const createButtonLabel = createIssueLabel ? t("app.issues.list.newNamed", { label: createIssueLabel }) : t("app.issues.list.newTask");
  ...
  return (
    <div ref={rootRef} className="space-y-4">
      ...
      <IssuesToolbar
        ...
        ariaLabel={toolbarPresentation === "collection" ? t("app.issues.list.taskControls") : undefined}
        context={(
        ...
        controls={(
          <>
          {/* View mode toggle */}
          <div className="flex items-center border border-border rounded-md overflow-hidden mr-1" role="group" aria-label={t("app.issues.list.viewMode")}>
            <button
              ...
              title={t("app.issues.list.listView")}
              aria-label={t("app.issues.list.listView")}
            ...
            <button
              ...
              title={t("app.issues.list.boardView")}
              aria-label={t("app.issues.list.boardView")}
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled ? t("app.issues.list.disableNesting") : t("app.issues.list.enableNesting")}
            >
          ...
          {viewState.viewMode === "board" && (
            <>
              <Button
                ...
                title={boardCompactCards ? t("app.issues.list.useComfortableCards") : t("app.issues.list.useCompactCards")}
              >
              ...
              <Button
                ...
                title={boardCollapsedStatuses.length > 0 ? t("app.issues.list.expandColdLanes") : t("app.issues.list.collapseColdLanes")}
              >
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    title={t("app.issues.list.cardsPerColumn")}
                  >
                ...
                <PopoverContent align="end" className="w-40 p-0">
                  <div className="p-2 space-y-0.5">
                    {KANBAN_COLUMN_PAGE_SIZE_OPTIONS.map((pageSize) => (
                      ...
                      >
                        <span>{t("app.issues.list.perColumn", { count: pageSize })}</span>
              ...
              <Button
                ...
                title={t("app.issues.list.resetBoardDensity")}
              >
          ...
          <IssueColumnPicker
            ...
            title={t("app.issues.list.columnPickerTitle")}
            iconOnly
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("app.issues.list.sort")}>
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  ...
                  {([
                    ["workflow", t("app.issues.list.sortField.workflow")],
                    ["status", t("app.issues.list.sortField.status")],
                    ["priority", t("app.issues.list.sortField.priority")],
                    ["title", t("app.issues.list.sortField.title")],
                    ["created", t("app.issues.list.sortField.created")],
                    ["updated", t("app.issues.list.sortField.updated")],
                  ] as const)
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title={t("app.issues.list.group")}>
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  ...
                  {([
                    ["status", t("app.issues.list.groupBy.status")],
                    ["priority", t("app.issues.list.groupBy.priority")],
                    ["assignee", t("app.issues.list.groupBy.assignee")],
                    ["project", t("app.issues.list.groupBy.project")],
                    ["workspace", t("app.issues.list.groupBy.workspace")],
                    ["parent", t("app.issues.list.groupBy.parent")],
                    ["none", t("app.issues.list.groupBy.none")],
                  ] as const)
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          {t("app.issues.list.searchLimit", { count: ISSUE_SEARCH_RESULT_LIMIT })}
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          {t("app.issues.list.boardLimit", { count: ISSUE_BOARD_COLUMN_RESULT_LIMIT })}
        </p>
      )}
      {!isLoading && !externalObjectFilterLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("app.issues.list.emptyFiltered")}
        ...
        <div className="-mx-2 sm:mx-0">
              ...
              <IssueGroupHeader
                ...
                trailing={(
                  <Button
                    ...
                    title={t("app.issues.list.newTaskIn", { group: group.label })}
                    aria-label={t("app.issues.list.newTaskIn", { group: group.label })}
                ...
                const renderIssueRow = (issue: Issue, depth: number) => {
                  ...
                  const visibleBlockerChips = unresolvedVisibleBlockers
                    .map((blockerId) => {
                      ...
                      const chipLabel = blockerStep
                        ? t("app.issues.row.blockedByStep", { label, step: blockerStep })
                        : t("app.issues.row.blockedBy", { label });
                  ...
                  const firstVisibleBlockerDisplayLabel = firstVisibleBlockerChip
                    ? additionalVisibleBlockerCount > 0
                      ? t("app.issues.row.blockedByMore", { chip: firstVisibleBlockerChip.chipLabel, count: additionalVisibleBlockerCount })
                  ...
                  const firstVisibleBlockerTitle = additionalVisibleBlockerCount > 0
                    ? t("app.issues.row.blockedByList", { first: firstVisibleBlockerDisplayLabel, others: hiddenVisibleBlockerLabels })
                          ...
                          <>
                            {hasChildren && !isExpanded ? (
                              <span className="ml-1.5 text-xs text-muted-foreground">
                                {totalDescendants === 1
                                  ? t("app.issues.row.subTaskCountOne", { count: totalDescendants })
                                  : t("app.issues.row.subTaskCountMany", { count: totalDescendants })}
                              </span>
                            ) : null}
                            {issueBadge ? (
                              issueBadge === "Paused" ? (
                                <Badge variant="ghost"
                                  ...
                                  aria-label={t("app.issues.row.paused")}
                                  title={t("app.issues.row.paused")}
                                >
                                  <CircleSlash2 className="h-3 w-3" />
                                  {t("app.issues.row.paused")}
                                </Badge>
                              ) : (
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <Badge variant="outline"
                                ...
                                aria-label={t("app.issues.row.needsNextStep")}
                                title={t("app.issues.row.needsNextStepTitle")}
                              >
                                <CircleDot className="h-3 w-3" />
                                {t("app.issues.row.needsNextStep")}
                              </Badge>
                            ) : null}
                          </>
                        )}
                        ...
                        leadingControl={rowPresentation === "task" ? (
                          hasChildren ? (
                            <button
                              ...
                              aria-label={isExpanded ? t("app.issues.row.collapseSubTasks") : t("app.issues.row.expandSubTasks")}
                                ...
                                >
                                  <PopoverTrigger asChild>
                                    ...
                                    >
                                      {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                                        ...
                                        <Identity
                                          name={assigneeUserLabel ?? t("app.issues.list.user")}
                                  ...
                                  >
                                    <input
                                      ...
                                      placeholder={t("app.issues.row.searchResponsible")}
                                    ...
                                    <div className="max-h-48 overflow-y-auto overscroll-contain">
                                      ...
                                      >
                                        {t("app.issues.row.noResponsible")}
                                      </button>
                                      {currentUserId && (
                                        <button
                                          className={cn(
                ...
                const appendIssueRow = (issue: Issue, depth: number) => {
                  ...
                  if (separatorField && rowPresentation === "task") {
                    ...
                    const separatorLabel = taskDateGroupSeparator(previousDateGroup, currentDateGroup)
                      ? taskDateGroupLabel(t, currentDateGroup)
                      : null;
                    if (separatorLabel) {
                      nodes.push(
                        <IssueDateSeparator
          ...
          {(remainingIssueRowCount > 0 || hasMoreIssues || isLoadingMoreIssues) && (
            <div className="py-2" data-testid="issues-load-more-sentinel">
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ? t("app.issues.list.loadingMore")
                  : remainingIssueRowCount > 0
                    ? t("app.issues.list.renderingOf", { shown: Math.min(renderedIssueRowLimit, filtered.length), total: filtered.length })
                    : t("app.issues.list.scrollToLoadMore")}
              </p>
            </div>
          )}
        </div>
