...
const IssueTitleTextarea = memo(function IssueTitleTextarea({
  ...
  return (
    <textarea
      ...
      placeholder={t("app.newIssue.titlePlaceholder")}
      rows={1}
      value={draftValue}
      onChange={(e) => {
...
const IssueDescriptionEditor = memo(function IssueDescriptionEditor({
  ...
  return (
    <MarkdownEditor
      ...
      placeholder={t("app.newIssue.descriptionPlaceholder")}
      bordered={false}
      mentions={mentions}
...
export function NewIssueDialog() {
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: t("app.newIssue.toast.uploadWarningsTitle", { issueRef }),
          body: failures.length === 1
            ? t("app.newIssue.toast.uploadWarningsBodyOne", { count: failures.length })
            : t("app.newIssue.toast.uploadWarningsBodyMany", { count: failures.length }),
          tone: "warn",
          action: prefix
            ? { label: t("app.newIssue.toast.openIssue", { issueRef }), href: `/${prefix}/issues/${issueRef}` }
            : undefined,
        });
      }
      clearDraft();
  ...
  const assigneeOptionsTitle =
    assigneeAdapterType === "claude_local"
      ? t("app.newIssue.assigneeOptions.claude")
      : assigneeAdapterType === "codex_local"
        ? t("app.newIssue.assigneeOptions.codex")
        : assigneeAdapterType === "opencode_local"
          ? t("app.newIssue.assigneeOptions.openCode")
        : t("app.newIssue.assigneeOptions.agent");
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("app.newIssue.errors.createFailed");
  ...
  const workModeLabel = (option: { value: IssueWorkMode; label: string }) => {
    switch (option.value) {
      case "standard": return t("app.newIssue.workMode.standard");
      case "planning": return t("app.newIssue.workMode.planning");
      case "ask": return t("app.newIssue.workMode.ask");
      default: return option.label;
    }
  };
  const statusDescription = (option: { value: string; description?: string }) => {
    if (option.value === "backlog") return t("app.newIssue.statusDescription.backlog");
    if (option.value === "todo") return t("app.newIssue.statusDescription.todo");
    return option.description;
  };
  const executionWorkspaceModeLabel = (option: { value: string; label: string }) => {
    switch (option.value) {
      case "shared_workspace": return t("app.newIssue.executionWorkspace.modes.shared");
      case "isolated_workspace": return t("app.newIssue.executionWorkspace.modes.isolated");
      case "reuse_existing": return t("app.newIssue.executionWorkspace.modes.reuseExisting");
      default: return option.label;
    }
  };
  const thinkingEffortLabel = (option: { value: string; label: string }) =>
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{isSubIssueMode ? t("app.newIssue.newSubTask") : t("app.newIssue.newTask")}</span>
          </div>
        ...
        <div ref={dialogBodyRef} className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          <div className="px-4 pb-2">
            <div className="overflow-x-auto overscroll-x-contain">
              ...
              <span className="w-6 shrink-0 text-center">{t("app.newIssue.for")}</span>
              <InlineEntitySelector
                ...
                noneLabel={t("app.newIssue.noAssignee")}
                searchPlaceholder={t("app.newIssue.searchAssignees")}
                emptyMessage={t("app.newIssue.noAssigneesFound")}
                onChange={(value) => {
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("app.newIssue.assignee")}</span>
                  )
                }
                renderOption={(option) => {
                  ...
                  return (
                    <>
                      ...
                      {assignee && getTrustPreset(assignee.permissions) === "low_trust_review" ? (
                        <ShieldAlert className="ml-auto h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-300" aria-label={t("app.newIssue.lowTrustReviewAgent")} />
                      ) : null}
                    </>
                  );
                }}
              />
              <span>{t("app.newIssue.in")}</span>
              <InlineEntitySelector
                ...
                searchPlaceholder={t("app.newIssue.searchProjects")}
                emptyMessage={t("app.newIssue.noProjectsFound")}
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("app.newIssue.project")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              <Popover open={participantMenuOpen} onOpenChange={setParticipantMenuOpen}>
                <PopoverTrigger asChild>
                  <button
                    ...
                    title={t("app.newIssue.addParticipant")}
                  >
            ...
            {showReviewerRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("app.newIssue.reviewer")}
                contentStyle={entityPickerViewportStyle}
                noneLabel={t("app.newIssue.noReviewer")}
                searchPlaceholder={t("app.newIssue.searchReviewers")}
                emptyMessage={t("app.newIssue.noReviewersFound")}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("app.newIssue.reviewer")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            {showApproverRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("app.newIssue.approver")}
                contentStyle={entityPickerViewportStyle}
                noneLabel={t("app.newIssue.noApprover")}
                searchPlaceholder={t("app.newIssue.searchApprovers")}
                emptyMessage={t("app.newIssue.noApproversFound")}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("app.newIssue.approver")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
                ...
                <Popover open={watchdogEditorOpen} onOpenChange={setWatchdogEditorOpen}>
                  <PopoverTrigger asChild>
                    <button
                      ...
                      title={t("app.newIssue.configureWatchdog")}
                    >
                      {selectedWatchdogAgent ? (
                        ...
                        <span className="text-muted-foreground">{t("app.newIssue.setWatchdog")}</span>
                      )}
                    </button>
                  </PopoverTrigger>
                  <PopoverContent className="w-80 p-3 space-y-3" align="start">
                      ...
                      <InlineEntitySelector
                        ...
                        renderTriggerValue={(option) =>
                          option ? (
                            ...
                            <span className="text-muted-foreground">{t("app.newIssue.selectAgent")}</span>
                          )
                        }
                        renderOption={(option) => {
                    ...
                    <div className="space-y-1.5">
                      <div className="text-xs font-medium text-foreground">{t("app.newIssue.instructions")} <span className="font-normal text-muted-foreground">{t("app.newIssue.optional")}</span></div>
                      <Textarea
          ...
          {isSubIssueMode ? (
            ...
            <div className="max-w-full rounded-md border border-border bg-muted/30 px-2.5 py-1.5 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                <span className="shrink-0">{t("app.newIssue.subTaskOf")}</span>
          ...
          {workspaceIsolationControlsVisible && currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              <div className="text-xs font-medium">{t("app.newIssue.executionWorkspace.label")}</div>
              <div className="text-(length:--text-micro) text-muted-foreground">
                {t("app.newIssue.executionWorkspace.hint")}
              </div>
              <select
              ...
              >
                {EXECUTION_WORKSPACE_MODES.map((option) => (
                  <option key={option.value} value={option.value}>
                    {executionWorkspaceModeLabel(option)}
                  </option>
                ))}
              </select>
              {executionWorkspaceMode === "reuse_existing" && (
              ...
              */}
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-(length:--text-micro) text-muted-foreground">
                  {t("app.newIssue.executionWorkspace.reusing", {
                    name: selectedReusableExecutionWorkspace.name,
                    source: selectedReusableExecutionWorkspace.branchName ?? t("app.newIssue.executionWorkspace.existingFallback"),
                  })}
                </div>
              )}
              {showParentWorkspaceWarning ? (
                <div className="rounded-md border border-amber-300/60 bg-amber-50 px-2 py-1.5 text-(length:--text-micro) text-amber-900 dark:border-amber-800/70 dark:bg-amber-950/30 dark:text-amber-100">
                  {parentExecutionWorkspaceLabel
                    ? t("app.newIssue.executionWorkspace.parentWarningWithLabel", { label: parentExecutionWorkspaceLabel })
                    : t("app.newIssue.executionWorkspace.parentWarning")}
                </div>
              ) : null}
            </div>
            </div>
              ...
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  ...
                  <div
                    ...
                    role="radiogroup"
                    aria-label={t("app.newIssue.modelLane.label")}
                  >
                    {(["primary", "custom"] as const).map((lane) => (
                      ...
                      >
                        {lane === "primary" ? t("app.newIssue.modelLane.primary") : t("app.newIssue.modelLane.custom")}
                      </button>
                    ))}
                  </div>
                  {assigneeModelLane === "primary" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">{t("app.newIssue.modelLane.primaryHint")}</p>
                  )}
                  {assigneeModelLane === "custom" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">{t("app.newIssue.modelLane.customHint")}</p>
                  )}
                </div>
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
            ...
            {stagedFiles.length > 0 ? (
              ...
              {stagedDocuments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("app.newIssue.documents")}</div>
                  <div className="space-y-2">
                    {stagedDocuments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          disabled={createIssue.isPending}
                          title={t("app.newIssue.removeDocument")}
                        >
              ...
              {stagedAttachments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("app.newIssue.attachments")}</div>
                  <div className="space-y-2">
                    {stagedAttachments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          onClick={() => removeStagedFile(file.id)}
                          disabled={createIssue.isPending}
                          title={t("app.newIssue.removeAttachment")}
                        >
                          <X className="h-3.5 w-3.5" />
                        </Button>
                      </div>
          ...
          <Popover open={statusOpen} onOpenChange={setStatusOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {issueStatusLabel(t, currentStatus.value)}
              </button>
            </PopoverTrigger>
              ...
              {statuses.map((s) => (
                ...
                >
                  ...
                  <span className="flex flex-col text-left leading-tight">
                    <span>{issueStatusLabel(t, s.value)}</span>
                    {s.description ? (
                      <span className="text-(length:--text-nano) text-muted-foreground">{statusDescription(s)}</span>
                    ) : null}
                  </span>
                </button>
              ))}
          ...
          <Popover open={moreOpen} onOpenChange={setMoreOpen}>
            ...
            <PopoverContent className="w-44 p-1" align="start" data-testid="new-issue-more-menu">
              ...
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                ...
                {t("app.newIssue.startDate")}
              </button>
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                <Calendar className="h-3 w-3" />
                {t("app.newIssue.dueDate")}
              </button>
            </PopoverContent>
          </Popover>
        </div>
        ...
        {assigneeValue && status === "backlog" ? (
          ...
          >
            ...
            <span className="leading-snug">
              <Trans
                i18nKey="app.newIssue.assignedBacklogNote"
          ...
          <div data-testid="new-issue-paused-assignee-note" className="mx-4 mb-2">
            <InlineBanner tone="warning" icon={PauseCircle} compact>
              {selectedAssigneeAgent.pauseReason === "budget" ? (
                ...
                <Trans
                  i18nKey="app.newIssue.pausedAssigneeNote"
                  values={{ name: selectedAssigneeAgent.name }}
                  components={{ strong: <span className="font-medium" /> }}
                />
              )}
        ...
        {currentAssigneeLowTrust ? (
          ...
          >
            ...
            <span className="leading-snug">
              {t("app.newIssue.lowTrustAssigneeNote")}
            </span>
          </div>
        ) : null}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? t("app.newIssue.creating") : isSubIssueMode ? t("app.newIssue.createSubTask") : t("app.newIssue.createTask")}</span>
              </span>
            </Button>
          </div>
        </div>
