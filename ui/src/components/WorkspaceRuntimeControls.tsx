...
function buildJobItem(
  ...
  return {
    ...
    serviceIndex: null,
    disabledReason: command.disabledReason ?? (!command.command ? translate("app.workspaces.workspaceRuntimeControls.jobMissingCommand") : null),
  };
}
...
export function buildWorkspaceRuntimeControlSections(input: {
  ...
  const otherServices = runtimeServices
    ...
    .map((runtimeService) => ({
      ...
      disabledReason: translate("app.workspaces.workspaceRuntimeControls.serviceUnmatched"),
    }));
...
function exposureFailureCopy(exposure: RuntimeExposureStatus | null) {
  if (exposure?.state === "failed") {
    return {
      label: translate("app.workspaces.workspaceRuntimeControls.httpsUnavailable"),
      remediation: translate("app.workspaces.workspaceRuntimeControls.httpsUnavailableFix"),
    };
  }
  if (exposure?.state === "cleanup_pending") {
    return {
      label: translate("app.workspaces.workspaceRuntimeControls.httpsCleanupPending"),
      remediation: translate("app.workspaces.workspaceRuntimeControls.httpsCleanupFix"),
    };
  }
  return null;
}
  ...
  return [...input.sections.services, ...input.sections.otherServices].map((item) => {
    ...
    const failureDetail = state === "failed"
      ? runtimeService?.stoppedAt
        ? translate("app.workspaces.workspaceRuntimeControls.serviceFailedAt", { time: timeAgo(runtimeService.stoppedAt) })
        : translate("app.workspaces.workspaceRuntimeControls.serviceFailed")
      : null;
    ...
    const exposureDetail = exposure?.state === "pending"
      ? translate("app.workspaces.workspaceRuntimeControls.provisioningHttps")
      : exposure?.state === "ready"
        ? translate("app.workspaces.workspaceRuntimeControls.httpsReady")
        : exposureFailure
    ...
    <div className={cn("flex gap-2", iconOnly ? "w-auto flex-row flex-wrap" : "w-full flex-col sm:w-auto sm:flex-row sm:flex-wrap")}>
      {actions.map((action) => {
        ...
        const label = action === "run"
          ? t("app.workspaces.workspaceRuntimeControls.run")
          : action === "start"
            ? t("app.common.actions.start")
            : action === "stop"
              ? t("app.common.actions.stop")
              : t("app.common.actions.restart");
            ...
            <div key={item.key} className={cn("border border-border/80 bg-background px-3 py-3", square ? "rounded-none" : "rounded-xl")}>
              <div className="flex flex-col gap-3">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {item.port ? <div>{t("app.workspaces.workspaceRuntimeControls.port", { port: item.port })}</div> : null}
...
export function WorkspaceRuntimeControls({
  ...
  const resolvedServiceEmptyMessage = emptyMessage ?? serviceEmptyMessage ?? t("app.workspaces.workspaceRuntimeControls.noServicesConfigured");
  const resolvedJobEmptyMessage = jobEmptyMessage ?? t("app.workspaces.workspaceRuntimeControls.noJobsConfigured");
  ...
  return (
    <div className={cn("space-y-4", className)}>
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0 ? t("app.workspaces.workspaceRuntimeControls.servicesRunning", { count: runningCount }) : t("app.workspaces.workspaceRuntimeControls.noServicesRunning")}
            </Badge>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ? resolvedSections.jobs.length === 1
                  ? t("app.workspaces.workspaceRuntimeControls.oneJobAvailable")
                  : t("app.workspaces.workspaceRuntimeControls.jobsAvailable", { count: resolvedSections.jobs.length })
                : t("app.workspaces.workspaceRuntimeControls.independentControl")}
            </span>
          </div>
      ...
      <CommandSection
        title={t("app.workspaces.workspaceRuntimeControls.services")}
        description={t("app.workspaces.workspaceRuntimeControls.servicesDescription")}
      ...
      <CommandSection
        title={t("app.workspaces.workspaceRuntimeControls.jobs")}
        description={t("app.workspaces.workspaceRuntimeControls.jobsDescription")}
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          title={t("app.workspaces.workspaceRuntimeControls.untrackedServices")}
          description={t("app.workspaces.workspaceRuntimeControls.untrackedDescription")}
