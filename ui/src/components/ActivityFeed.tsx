...
function getFilterOptions(): Array<{ value: FilterValue; label: string }> {
  return [
    { value: "all", label: t("app.common.labels.all") },
    { value: "in-progress", label: t("app.common.issueStatus.in_progress") },
    { value: "for-review", label: t("app.common.issueStatus.in_review") },
    { value: "completed", label: t("app.common.issueStatus.done") },
  ];
}
    ...
    <div>
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          <Trans
            i18nKey="app.reports.activityFeed.actorUpdates"
            values={{ actor: actorName, count: group.events.length, entity: entityName ?? group.entityId }}
            components={{
...
export function ActivityFeed({ className }: ActivityFeedProps) {
      ...
      if (prevIsRecent && !thisIsRecent) {
        separator = (
          <div className="flex items-center gap-2 px-4 py-1.5" key={`sep-${index}`}>
            ...
            <span className="text-(length:--text-nano) font-medium text-muted-foreground uppercase tracking-wider">{t("app.common.labels.earlier")}</span>
            <div className="h-px flex-1 bg-border" />
          </div>
        );
      }
  ...
  const renderGrouped = () => {
    ...
    return Array.from(groups.entries()).map(([groupKey, items]) => {
      ...
      const label = isOther
        ? t("app.reports.activityFeed.otherActivity")
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (agents.length === 0) {
      return {
        text: t("app.reports.activityFeed.noAgentsSetUpYetAddAnAgentToGetStarted"),
        showPulse: false,
      };
    }
    ...
    if (allPaused) {
      return {
        text: t("app.reports.activityFeed.allAgentsArePausedResumeAgentsFromTheSidebarToSeeActivity"),
        showPulse: false,
      };
    }
    return {
      text: t("app.reports.activityFeed.yourAgentsAreRunningActivityWillAppearHereShortly"),
      showPulse: true,
    };
  }, [agents, t]);
    ...
    <aside className={cn("flex min-h-0 min-w-0 flex-1 flex-col bg-background", className)}>
      ...
      <div className="relative flex shrink-0 items-start justify-between gap-2 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-semibold">{t("app.reports.activityFeed.agentFeed")}</h3>
          <p className="text-xs text-muted-foreground">{t("app.reports.activityFeed.liveActivityFromYourAgents")}</p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t("app.reports.activityFeed.groupByTaskAria")}
            ...
            <TooltipContent side="bottom">
              {groupMode === "flat" ? t("app.reports.activityFeed.groupByTask") : t("app.reports.activityFeed.showFlat")}
            </TooltipContent>
          </Tooltip>
          ...
          <DropdownMenu>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={t("app.reports.activityFeed.filterByAria")}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent side="bottom">{t("app.reports.activityFeed.filterBy")}</TooltipContent>
            </Tooltip>
            <DropdownMenuContent align="end" className="w-48">
              ...
              >{t("app.reports.activityFeed.showAllActivity")}</DropdownMenuCheckboxItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center p-6 h-full">
            <div className="flex flex-col items-center gap-2 max-w-(--sz-16rem)">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {emptyMessage?.text ?? t("app.reports.activityFeed.activityFromYourAgentsWillAppearHere")}
              </p>
            </div>
          </div>
        ) : groupMode === "flat" ? (
