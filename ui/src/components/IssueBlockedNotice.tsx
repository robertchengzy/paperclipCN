...
function BlockerRecoveryIndicator({
  ...
  return (
    <Badge variant="outline"
      ...
      title={detail
        ? t("app.issueUi.issueBlockedNotice.recoveryTitleDetail", { label, detail })
        : t("app.issueUi.issueBlockedNotice.recoveryTitle", { label })}
...
function SuccessfulRunRetryNowControl({
  ...
  const scheduleMessage = relative === "now"
    ...
    : relative
      ...
      : t("app.issueUi.issueBlockedNotice.nextStep.scheduled");
        ...
        >
          {retryNow.isPending ? (
            <span className="inline-flex items-center gap-1.5">
              ...
              {t("app.issueUi.issueBlockedNotice.retrying")}
            </span>
          ) : success ? (
            <span className="inline-flex items-center gap-1.5">
              <CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true" />
              {retryNow.data?.outcome === "already_promoted" ? t("app.issueUi.issueBlockedNotice.alreadyPromoted") : t("app.issueUi.issueBlockedNotice.promoted")}
            </span>
          ) : (
            <span className="inline-flex items-center gap-1.5">
              <RotateCcw className="h-3.5 w-3.5" aria-hidden="true" />
              {t("app.issueUi.issueBlockedNotice.retryNow")}
            </span>
          )}
        </Button>
      </div>
...
/** Key for the "Still blocked by …" line, by whether a status is known and how many other leaves remain. */
function stillBlockedKey(hasStatus: boolean, otherCount: number): string {
  if (hasStatus) {
    if (otherCount === 0) return "app.issueUi.issueBlockedNotice.stillBlocked.status";
    return otherCount === 1 ? "app.issueUi.issueBlockedNotice.stillBlocked.statusOtherOne" : "app.issueUi.issueBlockedNotice.stillBlocked.statusOtherMany";
  }
  if (otherCount === 0) return "app.issueUi.issueBlockedNotice.stillBlocked.base";
  return otherCount === 1 ? "app.issueUi.issueBlockedNotice.stillBlocked.otherOne" : "app.issueUi.issueBlockedNotice.stillBlocked.otherMany";
}

function waitingTaskStatusLabel(status: string): string {
  switch (status) {
    case "backlog":
      return t("app.common.issueStatus.backlog");
    case "todo":
      return t("app.common.issueStatus.todo");
    case "in_progress":
      return t("app.common.issueStatus.in_progress");
    case "in_review":
      return t("app.common.issueStatus.in_review");
    case "done":
      return t("app.common.issueStatus.done");
    case "blocked":
      return t("app.common.issueStatus.blocked");
    case "cancelled":
      return t("app.common.issueStatus.cancelled");
    default:
  ...
  return (
    ...
    >
      <StatusGlyph
        ...
        size="sm"
        title={t("app.issueUi.issueBlockedNotice.statusTitle", { status: waitingTaskStatusLabel(blocker.status) })}
      />
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <p className="min-w-0 leading-5">
          {t("app.issueUi.issueBlockedNotice.inFlight")}
          {shortRunId ? (
            <>
              {" "}
              {assigneeAgentId ? (
                ...
                >
                  {t("app.issueUi.issueBlockedNotice.runId", { id: shortRunId })}
                </Link>
              ) : (
                <span className="font-mono">{t("app.issueUi.issueBlockedNotice.runId", { id: shortRunId })}</span>
              )}
            </>
          ) : null}
        </p>
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="space-y-1">
            <p className="font-medium leading-5">{t("app.issueUi.issueBlockedNotice.live.title")}</p>
            <p className="leading-5">
              {total === 1
                ? t("app.issueUi.issueBlockedNotice.live.queuedOne", { count: total })
                : t("app.issueUi.issueBlockedNotice.live.queuedMany", { count: total })}
            </p>
          </div>
          ...
          <div className="space-y-1" data-testid="issue-blocked-notice-progress">
            <div className="text-xs font-medium text-blue-800 dark:text-blue-200">
              {runningCount > 0
                ? t("app.issueUi.issueBlockedNotice.live.progressRunning", { done: doneCount, total, running: runningCount })
                : t("app.issueUi.issueBlockedNotice.live.progress", { done: doneCount, total })}
            </div>
            <div
              role="progressbar"
              aria-label={t("app.issueUi.issueBlockedNotice.live.progressAria")}
              aria-valuemin={0}
            ...
            <div className="flex items-stretch gap-2">
              ...
              <div className="min-w-0 pb-0.5">
                <span className="inline-block rounded-md border border-dashed border-blue-300/70 px-2 py-1 text-xs text-blue-800 dark:border-blue-500/40 dark:text-blue-200">
                  {t("app.issueUi.issueBlockedNotice.live.thisTask")}
                </span>
              </div>
            </div>
          </div>
          ...
          {nowRunning.length > 0 ? (
            ...
            >
              <div className="text-xs font-medium text-blue-800 dark:text-blue-200">
                {t("app.issueUi.issueBlockedNotice.live.nowRunning")}
              </div>
          ...
          {parkedBlockers.length > 0 ? (
            ...
            >
              <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-800 dark:text-amber-200">
                ...
                {t("app.issueUi.issueBlockedNotice.blockedByParked")}
              </span>
...
export function IssueBlockedNotice({
  ...
  const responsibleName = agentName ?? t("app.issueUi.issueBlockedNotice.theAssignee");
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 space-y-1.5">
          {showSuccessfulRunHandoff ? (
            <>
              <p className="font-medium leading-5">{t("app.issueUi.issueBlockedNotice.handoff.title")}</p>
              <p className="leading-5">
                {t("app.issueUi.issueBlockedNotice.handoff.body")}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-xs leading-5 text-amber-900 dark:text-amber-100">
                <li>{t("app.issueUi.issueBlockedNotice.handoff.markDone")}</li>
                <li>{t("app.issueUi.issueBlockedNotice.handoff.sendReview")}</li>
                <li>{t("app.issueUi.issueBlockedNotice.handoff.recordBlocker")}</li>
                <li>{t("app.issueUi.issueBlockedNotice.handoff.delegate")}</li>
              </ul>
              <div className="flex flex-wrap gap-1.5 text-xs">
                {successfulRunHandoff.sourceRunId && successfulRunHandoff.assigneeAgentId ? (
                  ...
                  >
                    {t("app.issueUi.issueBlockedNotice.runId", { id: successfulRunHandoff.sourceRunId.slice(0, 8) })}
                  </Link>
                ...
                <span className="rounded-md border border-amber-300/70 bg-background/80 px-2 py-1 text-amber-900 dark:border-amber-500/40 dark:bg-background/40 dark:text-amber-100">
                  {t("app.issueUi.issueBlockedNotice.handoff.asked", { name: agentName ?? t("app.issueUi.issueBlockedNotice.theAssignee") })}
                </span>
              </div>
              {successfulRunHandoff.detectedProgressSummary ? (
                <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
                  {t("app.issueUi.issueBlockedNotice.handoff.detectedProgress", { summary: successfulRunHandoff.detectedProgressSummary })}
                </p>
              ) : null}
          ...
          {blockers.length > 0 || issueStatus === "blocked" ? (
            <>
                ...
                {blockers.length > 0
                  ? isStalled
                    ? stalledLeafBlockers.length > 1
                      ? singleBlocker
                        ? t("app.issueUi.issueBlockedNotice.blocked.stalledReviewsOne")
                        : t("app.issueUi.issueBlockedNotice.blocked.stalledReviewsMany")
                      : singleBlocker
                        ? t("app.issueUi.issueBlockedNotice.blocked.stalledReviewOne")
                        : t("app.issueUi.issueBlockedNotice.blocked.stalledReviewMany")
                    : reopenSuppressed
                      ? singleBlocker
                        ? t("app.issueUi.issueBlockedNotice.blocked.reopenSuppressedOne", { name: responsibleName })
                        : t("app.issueUi.issueBlockedNotice.blocked.reopenSuppressedMany", { name: responsibleName })
                      : singleBlocker
                        ? t("app.issueUi.issueBlockedNotice.blocked.untilCompleteOne")
                        : t("app.issueUi.issueBlockedNotice.blocked.untilCompleteMany")
                  : t("app.issueUi.issueBlockedNotice.blocked.untilMovedBack")}
              </p>
              {reopenSuppressed && reopenSuppressedLeafId ? (
                ...
                >
                  <Trans
                    i18nKey={stillBlockedKey(Boolean(reopenSuppressedLeafStatus), reopenSuppressedOtherCount)}
                    values={{
              ...
              {showStalledRow ? (
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("app.issueUi.issueBlockedNotice.stalledInReview")}
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                  <span className="text-xs font-medium text-amber-800 dark:text-amber-200">
                    {t("app.issueUi.issueBlockedNotice.ultimatelyWaitingOn")}
                  </span>
