      ...
      {warning.href ? (
        <>
          ...
          <Link to={warning.href} className="font-medium underline underline-offset-2">
            {warning.hrefLabel ?? t("app.common.actions.open")}
          </Link>
        </>
      ) : null}
    </>
  ...
  return (
    ...
    >
      <h2 id="pipeline-health-bar-heading" className="flex items-center gap-2 text-sm font-semibold">
        ...
        <span>
          {warnings.length === 1
            ? t("app.pipelines.pipelineHealthWarnings.someStepsWontRunOne")
            : t("app.pipelines.pipelineHealthWarnings.someStepsWontRunMany", { count: warnings.length })}
        </span>
      </h2>
          ...
          return (
            <li key={`${warning.stageId}-${warning.code}-${index}`} className="list-disc">
              {warning.href ? (
                ...
                <button
                  type="button"
                  aria-label={t("app.pipelines.pipelineHealthWarnings.openStageSettings", { name: warning.stageName })}
      ...
      {overflow > 0 ? (
        <p className="mt-1.5 pl-6 text-xs text-amber-800/80 dark:text-amber-200/70">
          {t("app.pipelines.pipelineHealthWarnings.moreInStageSettings", { count: overflow })}
        </p>
      ) : null}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span>
          {warnings.length === 1
            ? t("app.pipelines.pipelineHealthWarnings.stepWontRun")
            : t("app.pipelines.pipelineHealthWarnings.stepWontRunWithCount", { count: warnings.length })}
        </span>
      </h2>
