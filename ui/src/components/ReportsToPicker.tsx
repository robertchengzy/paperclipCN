  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{t("app.shell.reportsToPicker.unknownManagerStaleId")}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {terminatedManager ? t("app.shell.reportsToPicker.reportsTerminated", { name: current.name }) : t("app.shell.reportsToPicker.reportsName", { name: current.name })}
              </span>
            </>
          ) : (
            <>
              ...
              <span className="min-w-0 truncate">
                {disabled ? (disabledEmptyLabel ?? t("app.shell.reportsToPicker.ceo")) : (chooseLabel ?? t("app.shell.reportsToPicker.choose"))}
              </span>
            </>
          )}
        </button>
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        >{t("app.shell.reportsToPicker.noManager")}</button>
        {terminatedManager && (
          <div className="flex min-w-0 items-center gap-2 overflow-hidden px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            ...
            <span className="min-w-0 truncate">{t("app.shell.reportsToPicker.current")} {current.name} {t("app.shell.reportsToPicker.terminated")}</span>
          </div>
        )}
        {unknownManager && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">{t("app.shell.reportsToPicker.savedManagerIsMissingFromThisOrganization")}</div>
        )}
        {rows.map((a) => (
          <button
            type="button"
          ...
          >
            ...
            <span className="text-muted-foreground ml-auto shrink-0">{t(`app.agents.roles.${a.role}`, { defaultValue: roleLabels[a.role] ?? a.role })}</span>
          </button>
        ))}
      </PopoverContent>
    </Popover>
