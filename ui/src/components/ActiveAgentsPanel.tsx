...
const runStatusLabels: Record<string, string> = {
  get running() { return t("app.common.states.running"); },
  get queued() { return t("app.common.states.queued"); },
  get succeeded() { return t("app.agentUi.activeAgentsPanel.succeeded"); },
  get failed() { return t("app.common.states.failed"); },
  get timed_out() { return t("app.agentUi.activeAgentsPanel.timedOut"); },
  get cancelled() { return t("app.common.states.cancelled"); },
  get interrupted() { return t("app.agentUi.activeAgentsPanel.interrupted"); },
};
  ...
  return (
    <div>
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
        {title ?? t("app.common.nouns.agents")}
      </h3>
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">{emptyMessage ?? t("app.agentUi.activeAgentsPanel.noRecentRuns")}</p>
        </div>
      ) : (
      ...
      {showMoreLink && runs.length > 0 && (
        <div className="mt-3 flex justify-end text-xs text-muted-foreground">
          <Link to="/dashboard/live" className="hover:text-foreground hover:underline">
            {hiddenRunCount > 0
              ? hiddenRunCount === 1 ? t("app.agentUi.activeAgentsPanel.moreRunOne", { count: hiddenRunCount }) : t("app.agentUi.activeAgentsPanel.moreRunMany", { count: hiddenRunCount })
              : t("app.agentUi.activeAgentsPanel.viewAllRuns")}
          </Link>
        </div>
      )}
    </div>
...
export const AgentRunCard = memo(function AgentRunCard({
  ...
  const timestamp = run.finishedAt
    ? t("app.agentUi.activeAgentsPanel.finished", { value0: relativeTime(run.finishedAt) })
    : run.startedAt ? t("app.agentUi.activeAgentsPanel.started", { value0: relativeTime(run.startedAt) }) : t("app.agentUi.activeAgentsPanel.queued", { value0: relativeTime(run.createdAt) });
  const taskStatus = issue?.status === "in_review" && issue.externalConversationState === "waiting" ? "idle" : issue?.status ?? "backlog";
  const taskTitle = issue?.title ?? (issueLoadFailed ? t("app.agentUi.activeAgentsPanel.taskUnavailable") : t("app.agentUi.activeAgentsPanel.loadingTask"));
    ...
    <div className={cn(
      ...
      <div className={cn("flex shrink-0 flex-col gap-3 p-3", showTranscript && "border-b border-border/60")}>
        <Link
          ...
          aria-label={t("app.agentUi.activeAgentsPanel.viewRun", { value0: run.agentName, value1: statusLabel })}
        ...
        {run.issueId ? (
            ...
            <span className="flex min-w-0 items-baseline gap-2">
              <span className="flex min-w-0 flex-1 items-baseline gap-1.5">
                <StatusGlyph
                  ...
                  title={issue ? t("app.agentUi.activeAgentsPanel.task", { value0: t(`app.agentUi.activeAgentsPanel.taskStatus.${taskStatus}`, { defaultValue: taskStatus.replace(/_/g, " ") }) }) : undefined}
                />
          ...
          <Link to={runUrl} className="flex items-center gap-1.5 rounded-lg border border-border/60 bg-background/60 px-2.5 py-2 text-sm text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            ...
            <span className="truncate">{run.invocationSource === "timer" ? t("app.agentUi.activeAgentsPanel.scheduledHeartbeat") : t("app.agentUi.activeAgentsPanel.noLinkedTask")}</span>
          </Link>
        )}
        <time
