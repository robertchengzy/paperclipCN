...
export function RunWorkspaceRecoverySurface({ run }: { run: HeartbeatRun }) {
  ...
  const reconcile = useMutation({
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("app.issueDetail.toasts.reconcileFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const reissue = useMutation({
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("app.issueDetail.toasts.reissueFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const resolve = useMutation({
    mutationFn: (data: {
      ...
      if (!issueId || !recoveryAction) throw new Error(t("app.workspaces.runWorkspaceRecoverySurface.noRecoveryAction"));
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("app.issueDetail.toasts.recoveryResolutionFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  return (
    <div className="space-y-2" data-testid="run-workspace-recovery-surface">
      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-medium text-muted-foreground">{t("app.workspaces.runWorkspaceRecoverySurface.title")}</span>
        {issue?.identifier ? (
          <a
