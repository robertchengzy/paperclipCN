...
function collectValidation(form: FormModel, isSkillFile: boolean): ValidationIssue[] {
  ...
  if (isSkillFile && !name) issues.push({ field: "name", message: t("app.shell.frontmatterPanel.skillMdNeedsAName") });
  if (name && !SLUG_RE.test(name)) {
    issues.push({ field: "name", message: t("app.shell.frontmatterPanel.useLowercaseLettersNumbersAndHyphens") });
  }
  if (isSkillFile && !description) {
    issues.push({ field: "description", message: t("app.shell.frontmatterPanel.skillMdNeedsADescription") });
  }
  if (form.allowedToolsPresent && form.allowedTools === null) {
    issues.push({ field: "allowed-tools", message: t("app.shell.frontmatterPanel.expectedAListEditInYaml") });
  }
  return issues;
}
...
export function FrontmatterPanel({
  ...
  const initialAnalysis = useMemo(() => analyzeFrontmatterBlock(frontmatterText), [frontmatterText, t]);
  ...
  const analysis = useMemo(() => analyzeFrontmatterBlock(yamlText), [yamlText, t]);
    ...
    <TooltipProvider>
      ...
      >
        <div className="flex items-center gap-2 px-3 py-1.5">
          ...
          >
            {chevron}
            <span className="text-sm font-medium">{t("app.shell.frontmatterPanel.frontmatter")}</span>
            {!open && present ? (
          ...
          {present ? (
            ...
            >
              <TabsList variant="line" className="h-7">
                {canUseFields ? (
                  <TabsTrigger value="fields" className="px-2 py-0.5 text-xs">{t("app.shell.frontmatterPanel.fields")}</TabsTrigger>
                ) : (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span>
                        ...
                        >{t("app.shell.frontmatterPanel.fields")}</TabsTrigger>
                      </span>
                    </TooltipTrigger>
                    <TooltipContent className="max-w-60">{t("app.shell.frontmatterPanel.switchToYamlToEditThisFrontmatter")}</TooltipContent>
                  </Tooltip>
                )}
            ...
            <Button variant="ghost" size="sm" onClick={addFrontmatter} data-testid="add-frontmatter">
              <Plus className="mr-1 h-3.5 w-3.5" />{t("app.shell.frontmatterPanel.addFrontmatter")}</Button>
          ) : null}
          ...
          {present && effectiveMode === "fields" && warningCount > 0 ? (
            <Badge variant="outline" className="gap-1 text-amber-500" data-testid="frontmatter-warning-chip">
              ...
              {warningCount === 1 ? t("app.shell.frontmatterPanel.warningOne", { count: warningCount }) : t("app.shell.frontmatterPanel.warningMany", { count: warningCount })}
            </Badge>
          ) : null}
        </div>
        ...
        <CollapsibleContent id="frontmatter-panel-body">
          {present ? (
            ...
            <div className="px-3 pb-2 text-xs text-muted-foreground">{t("app.shell.frontmatterPanel.thisFileHasNoFrontmatter")}</div>
          )}
        </CollapsibleContent>
      </Collapsible>
    </TooltipProvider>
...
function buildSummary(parsed: Record<string, unknown>): string {
  ...
  if (tools && tools.length > 0) parts.push(tools.length === 1 ? t("app.shell.frontmatterPanel.toolOne", { count: tools.length }) : t("app.shell.frontmatterPanel.toolMany", { count: tools.length }));
  if (isFrontmatterPlainRecord(parsed.metadata)) {
    const count = Object.keys(parsed.metadata).length;
    if (count > 0) parts.push(t("app.shell.frontmatterPanel.metadataCount", { count }));
  }
  return parts.join(" · ");
}
        ...
        <div>
          ...
          {form.allowedTools === null ? (
            <p className="mt-1 text-xs text-amber-500">
              {toolsWarning ?? t("app.shell.frontmatterPanel.expectedAListEditInYaml")}
            </p>
          ) : (
            <ChipInput
              values={form.allowedTools}
              readOnly={readOnly}
              placeholder={t("app.shell.frontmatterPanel.addATool")}
      ...
      {form.metadataPresent ? (
        <div>
          ...
          {form.metadataComplex !== null ? (
            <p className="mt-1 text-xs text-muted-foreground">{t("app.shell.frontmatterPanel.complexValueEditInYaml")}</p>
          ) : (
            <MetadataRows
              rows={form.metaRows}
              readOnly={readOnly}
    ...
    <div className="mt-1 space-y-1.5">
      {rows.map((row, index) => (
        <div key={row.id} className="flex items-center gap-1.5">
          <Input
            aria-label={t("app.shell.frontmatterPanel.metadataKey", { value1: index + 1 })}
            value={row.key}
            readOnly={readOnly}
            placeholder={t("app.shell.frontmatterPanel.keyPlaceholder")}
            onChange={(event) => update(index, { key: event.target.value })}
            className="h-8 flex-1 font-mono text-xs"
          />
          <Input
            aria-label={t("app.shell.frontmatterPanel.valueFor", { value1: row.key || t("app.shell.frontmatterPanel.fieldIndex", { index: index + 1 }) })}
            value={row.text}
            readOnly={readOnly}
            placeholder={t("app.shell.frontmatterPanel.valuePlaceholder")}
          ...
          {!readOnly ? (
            <Button
              ...
              aria-label={t("app.shell.frontmatterPanel.remove", { value1: row.key || t("app.shell.frontmatterPanel.fieldIndex", { index: index + 1 }) })}
      ...
      {!readOnly ? (
        <Button variant="ghost" size="sm" onClick={add} className="text-xs">
          ...
          {t("app.shell.frontmatterPanel.addField")}
        </Button>
      ) : null}
    </div>
  );
    ...
    <div className="mt-1 flex flex-wrap items-center gap-1.5 rounded-md border border-border px-2 py-1.5">
      {values.map((value, index) => (
        <Badge key={`${value}-${index}`} variant="secondary" className="gap-1">
          ...
          {!readOnly ? (
            <button
              type="button"
              aria-label={t("app.shell.frontmatterPanel.remove", { value1: value })}
      ...
      {!readOnly ? (
        <input
          ...
          onBlur={commit}
          aria-label={t("app.shell.frontmatterPanel.addTool")}
    ...
    <div className="pt-1">
      {!canReturnToFields && !parseError ? (
        <div className="mb-1.5 flex items-start gap-2 rounded-md bg-muted/40 px-2 py-1.5 text-xs text-muted-foreground">
          ...
          <span>{t("app.shell.frontmatterPanel.editingRawYamlToPreserveFormattingThe")}</span>
        </div>
      ) : null}
      <Textarea
        value={value}
        ...
        aria-label={t("app.shell.frontmatterPanel.frontmatterYaml")}
      />
      <p className="mt-1 text-xs text-muted-foreground">{t("app.shell.frontmatterPanel.rawYamlIsTheSourceOfTruth")}</p>
    </div>
  );
}
