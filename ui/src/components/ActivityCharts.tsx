...
function runDayTooltip(entry: DashboardRunActivityDay): string {
  const lines = [t(entry.total === 1 ? "app.reports.activityCharts.runCountOne" : "app.reports.activityCharts.runCountMany", { date: entry.date, count: entry.total })];
  if (entry.succeeded > 0) lines.push(t("app.reports.activityCharts.succeededCount", { count: entry.succeeded }));
  if (entry.recovered > 0) lines.push(t("app.reports.activityCharts.recoveredCount", { count: entry.recovered }));
  if (entry.failed > 0) {
    lines.push(t("app.reports.activityCharts.failedCount", { count: entry.failed }));
    const codes = Object.entries(entry.failedByErrorCode ?? {}).sort((a, b) => b[1] - a[1]);
    for (const [code, count] of codes) lines.push(`    ${code}: ${count}`);
  }
  if (entry.other > 0) lines.push(t("app.reports.activityCharts.otherCount", { count: entry.other }));
  return lines.join("\n");
}
...
export function RunActivityChart(props: RunChartProps) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("app.reports.activityCharts.noRunsYet")}</p>;

  const legendItems = [
    { color: runSegmentColors.succeeded, label: t("app.reports.activityCharts.succeeded") },
    ...(hasRecovered ? [{ color: runSegmentColors.recovered, label: t("app.reports.activityCharts.recovered") }] : []),
    { color: runSegmentColors.failed, label: t("app.common.states.failed") },
    { color: runSegmentColors.other, label: t("app.reports.activityCharts.other") },
  ];
...
export function PriorityChart({ issues }: { issues: { priority: string; createdAt: Date }[] }) {
  ...
  if (!hasData) return <p className="text-xs text-muted-foreground">{t("app.reports.activityCharts.noTasks")}</p>;
    ...
    <div>
      <div className="flex items-end gap-(--sz-3px) h-20">
        {days.map(day => {
          ...
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("app.reports.activityCharts.issueCount", { date: day, count: total })}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map(p => ({ color: priorityColors[p], label: t(`app.common.priority.${p}`) }))} />
    </div>
  );
}
...
function getStatusLabels(): Record<string, string> {
  return {
    todo: t("app.reports.activityCharts.toDo"),
    in_progress: t("app.common.issueStatus.in_progress"),
    in_review: t("app.common.issueStatus.in_review"),
    done: t("app.common.issueStatus.done"),
    blocked: t("app.common.states.blocked"),
    cancelled: t("app.common.states.cancelled"),
    backlog: t("app.common.issueStatus.backlog"),
  };
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "var(--hex-6b7280)", label: getStatusLabels()[s] ?? s }))} />
    </div>
  );
}
