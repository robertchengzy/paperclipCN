  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("app.reports.financeTimelineCard.recentFinancialEvents")}</CardTitle>
        <CardDescription>{t("app.reports.financeTimelineCard.topupsFeesCreditsCommitmentsAndOtherNonrequestCharges")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">{emptyMessage ?? t("app.reports.financeTimelineCard.emptyMessage")}</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.id}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("app.reports.financeTimelineCard.invoice", { value: row.externalInvoiceId })}</div> : null}
                      {row.region ? <div>{t("app.reports.financeTimelineCard.region", { value: row.region })}</div> : null}
                      {row.pricingTier ? <div>{t("app.reports.financeTimelineCard.tier", { value: row.pricingTier })}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-amber-600">{t("app.reports.financeTimelineCard.estimated")}</div> : null}
                </div>
              </div>
            </div>
          ))
