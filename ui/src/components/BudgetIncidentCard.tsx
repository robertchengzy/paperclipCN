...
function incidentStateLabel(incident: BudgetIncident) {
  if (incident.status === "resolved") return t("app.reports.budgetIncidentCard.resolved");
  if (incident.status === "dismissed") return t("app.reports.budgetIncidentCard.dismissed");
  if (incident.approvalStatus === "revision_requested") return t("app.reports.budgetIncidentCard.escalated");
  if (incident.approvalStatus === "pending") return t("app.reports.budgetIncidentCard.pendingApproval");
  return t("app.reports.budgetIncidentCard.open");
}
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-red-700/90 dark:text-red-200/80">
                {t("app.reports.budgetIncidentCard.scopeHardStop", { scope: t(`app.reports.budgetIncidentCard.scope.${incident.scopeType}`) })}
              </div>
        ...
        <div className="flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-950/90 dark:text-red-50/90">
          ...
          <div>
            {incident.scopeType === "project"
              ? t("app.reports.budgetIncidentCard.projectExecutionIsPausedNewWorkInThisProjectWillNotStart")
              : t("app.reports.budgetIncidentCard.thisScopeIsPausedNewHeartbeatsWillNotStartUntilYouResolve")}
          </div>
        </div>

        <div className="rounded-xl border border-border/60 bg-background/60 p-3">
          <label className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("app.reports.budgetIncidentCard.newBudgetUSD")}</label>
          <div className="mt-2 flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {isMutating ? t("app.reports.budgetIncidentCard.applying") : t("app.reports.budgetIncidentCard.raiseBudgetResume")}
            </Button>
          </div>
          {parsed !== null && parsed <= incident.amountObserved ? (
            <p className="mt-2 text-xs text-red-700 dark:text-red-200/80">{t("app.reports.budgetIncidentCard.theNewBudgetMustExceedCurrentObservedSpend")}</p>
          ) : null}
        </div>

        <div className="flex justify-end">
          <Button variant="ghost" className="text-muted-foreground" disabled={isMutating} onClick={onKeepPaused}>{t("app.reports.budgetIncidentCard.keepPaused")}</Button>
        </div>
      </CardContent>
    </Card>
  );
