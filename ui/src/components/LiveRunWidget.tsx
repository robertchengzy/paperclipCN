  ...
  return (
    <div className="overflow-hidden rounded-xl border border-blue-500/25 bg-background/80 shadow-(--shadow-extract-11)">
      <div className="border-b border-border/60 bg-blue-500/[0.04] px-4 py-3">
        <div className="text-xs font-semibold uppercase tracking-(--tracking-caps) text-blue-700 dark:text-blue-300">{t("app.shell.liveRunWidget.liveRuns")}</div>
        <div className="mt-1 text-xs text-muted-foreground">{t("app.shell.liveRunWidget.usesTheSharedChatStyleRunSurface")}</div>
      </div>
                ...
                <div className="flex items-center gap-2">
                  {isActive && (
                    ...
                    >
                      ...
                      {cancellingRunIds.has(run.id) ? t("app.shell.liveRunWidget.stopping") : t("app.common.actions.stop")}
                    </button>
                  )}
                  <Link
                    to={`/agents/${run.agentId}/runs/${run.id}`}
                  ...
                  >{t("app.shell.liveRunWidget.openRun")}<ExternalLink className="h-3 w-3" />
                  </Link>
                </div>
              </div>
