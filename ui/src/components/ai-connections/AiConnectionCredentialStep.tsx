...
function SubscriptionConnectionStep({ companyId, provider, initialMethod, fixedMethod, connectionId, name: initialName, ownership, agentIds, allAgents, environmentId: suppliedEnvironmentId, onComplete, onCancel }: Props) {
  ...
  } catch (error) { environmentError = error instanceof Error ? error.message : t("app.connections.aiConnectionCredentialStep.environmentError"); }
  ...
  return <div className="mx-auto w-full min-w-0 max-w-xl space-y-6">
    <label className="block space-y-2 text-sm">{t("app.connections.aiConnectionCredentialStep.connectionName")}<Input value={name} onChange={(event) => setName(event.target.value)} disabled={Boolean(connectionId)} /></label>
    {!suppliedEnvironmentId && !forced.forced && loginEnvironments.length > 1 && <Select value={environmentId ?? ""} onValueChange={setChosenEnvironment}>
      <SelectTrigger aria-label={t("app.connections.aiConnectionCredentialStep.signInEnvironment")}><SelectValue placeholder={t("app.connections.aiConnectionCredentialStep.signInEnvironment")} /></SelectTrigger>
