...
export function TaskChatSkillCreatedCard({ item, onOpen }: { item: TaskChatSkillCreatedItem; onOpen?: (skillId: string, name: string) => void }) {
  ...
  return (
    <article className="rounded-lg border border-border bg-card p-3" aria-label={t("app.taskChat.taskChatSkillCreatedCard.ariaLabel", { name: item.name })}>
      <button type="button" className="flex w-full items-start gap-3 text-left" onClick={() => onOpen?.(item.skillId, item.name)}>
        ...
        <span className="min-w-0 flex-1 space-y-1">
          <span className="block text-xs text-muted-foreground">{t("app.taskChat.taskChatSkillCreatedCard.title")}</span>
