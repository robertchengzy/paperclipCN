...
function workspaceKindLabel(kind: ProjectWorkspaceSummary["kind"]) {
  return kind === "execution_workspace" ? translate("app.workspaces.projectWorkspaceSummaryCard.executionWorkspace") : translate("app.workspaces.projectWorkspaceSummaryCard.projectWorkspace");
}
          ...
          <div className="min-w-0 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge variant="outline" className="border-border/70 bg-background px-2.5 py-1 text-muted-foreground">
                {t("app.workspaces.projectWorkspaceSummaryCard.updated", { time: timeAgo(summary.lastUpdatedAt) })}
              </Badge>
              {summary.serviceCount > 0 ? (
                ...
                >
                  ...
                  />
                  {t("app.workspaces.projectWorkspaceSummaryCard.servicesCount", { running: summary.runningServiceCount, total: summary.serviceCount })}
                </Badge>
              ) : null}
              {summary.executionWorkspaceStatus ? (
                <Badge variant="outline" className="border-border/70 bg-background px-2.5 py-1 text-muted-foreground">
                  {t(`app.common.status.${summary.executionWorkspaceStatus}`, { defaultValue: summary.executionWorkspaceStatus.replace(/_/g, " ") })}
                </Badge>
              ) : null}
            </div>
            <Link
          ...
          >
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices ? t("app.workspaces.projectWorkspaceSummaryCard.stopServices") : t("app.workspaces.projectWorkspaceSummaryCard.startServices")}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed" ? t("app.workspaces.executionWorkspaceCloseDialog.retryClose") : t("app.workspaces.executionWorkspaceCloseDialog.closeWorkspace")}
              </Button>
            ) : null}
          </div>
        </div>
            ...
            {summary.branchName ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("app.workspaces.executionWorkspaceCloseDialog.branch")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.branchName}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("app.workspaces.projectWorkspaceSummaryCard.branchCopied")}
                    >
                      {summary.branchName}
                    </CopyText>
                    <CopyText
                      text={summary.branchName}
                      ariaLabel={t("app.workspaces.projectWorkspaceSummaryCard.copyBranch")}
            ...
            */}
            {summary.cwd && !hideHostPaths ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("app.workspaces.projectWorkspaceSummaryCard.path")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.cwd}
                      title={summary.cwd}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("app.workspaces.projectWorkspaceSummaryCard.pathCopied")}
                    >
                      {truncatePath(summary.cwd)}
                    </CopyText>
                    <CopyText
                      text={summary.cwd}
                      ariaLabel={t("app.workspaces.projectWorkspaceSummaryCard.copyPath")}
                      className="mt-0.5 shrink-0 text-muted-foreground hover:text-foreground"
                      copiedLabel={t("app.workspaces.projectWorkspaceSummaryCard.pathCopied")}
                    >
                      <Copy className="h-3.5 w-3.5" />
            ...
            {summary.primaryServiceUrl ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-(length:--text-micro) uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("app.workspaces.projectWorkspaceSummaryCard.service")}</div>
                  <a
        ...
        {summary.issues.length > 0 ? (
          <div className="space-y-2">
            <div className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-eyebrow) text-muted-foreground">
              {t("app.workspaces.projectWorkspaceSummaryCard.linkedTasks")}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {hiddenIssueCount > 0 ? (
                ...
                >
                  {t("app.workspaces.projectWorkspaceSummaryCard.moreCount", { count: hiddenIssueCount })}
                </Link>
              ) : null}
            </div>
          </div>
