...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime" ? t("app.reports.budgetPolicyCard.lifetimeBudget") : t("app.reports.budgetPolicyCard.monthlyUTCBudget");
}
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("app.reports.budgetPolicyCard.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("app.reports.budgetPolicyCard.limitPercent", { percent: summary.utilizationPercent }) : t("app.reports.budgetPolicyCard.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("app.common.nouns.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("app.common.states.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.paused && summary.pauseReason ? t("app.reports.budgetPolicyCard.softAlertPaused", { percent: summary.warnPercent, reason: summary.pauseReason }) : t("app.reports.budgetPolicyCard.softAlert", { percent: summary.warnPercent })}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("app.reports.budgetPolicyCard.unlimited")}</span>
      </div>
      <div className={cn("h-2 overflow-hidden rounded-full", isPlain ? "bg-border/70" : "bg-muted/70")}>
        <div
          ...
          aria-valuemin={0}
          aria-valuemax={100}
          aria-label={t("app.reports.budgetPolicyCard.utilization", { percent: Math.round(progress) })}
          className={cn(
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-900 dark:text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("app.reports.budgetPolicyCard.executionIsPausedForThisProjectUntilTheBudgetIsRaisedOr")
          : t("app.reports.budgetPolicyCard.heartbeatsArePausedForThisScopeUntilTheBudgetIsRaisedOr")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        <label className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("app.reports.budgetPolicyCard.budgetUSD")}</label>
        <Input
          value={draftBudget}
      ...
      >
        {isSaving ? t("app.reports.budgetPolicyCard.saving") : summary.amount > 0 ? t("app.reports.budgetPolicyCard.updateBudget") : t("app.reports.budgetPolicyCard.setBudget")}
      </Button>
    </div>
  ) : null;
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t(`app.reports.budgetPolicyCard.scope.${summary.scopeType}`)}
            </div>
          ...
          >
            ...
            {summary.paused ? t("app.common.states.paused") : summary.status === "warning" ? t("app.common.labels.warning") : summary.status === "hard_stop" ? t("app.reports.budgetPolicyCard.hardStop") : t("app.common.states.healthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("app.reports.budgetPolicyCard.enterAValidNonnegativeDollarAmount")}</p>
        ) : null}
      </div>
    );
  }
