...
export function DocumentAnnotationPanel(props: AnnotationPanelProps) {
      ...
      <Sheet open={props.open} onOpenChange={props.onOpenChange}>
        ...
        >
          <SheetTitle className="sr-only">
            {t("app.issueUi.documentAnnotationPanel.sheetTitle", { key: props.documentKey, revision: props.documentRevisionNumber })}
          </SheetTitle>
  ...
  return (
    <aside
      role="complementary"
      aria-label={t("app.issueUi.documentAnnotationPanel.ariaLabel", { key: props.documentKey.toUpperCase(), revision: props.documentRevisionNumber })}
    ...
    <>
      ...
      >
        <span className="text-(length:--text-micro) tabular-nums text-muted-foreground">
          {t("app.issueUi.documentAnnotationPanel.rev", { revision: props.documentRevisionNumber })}
        </span>
        <Button
          type="button"
          size="icon-xs"
          ...
          aria-label={t("app.issueUi.documentAnnotationPanel.close")}
        >
          <X className="h-4 w-4" />
        </Button>
      </div>
        ...
        {props.expanded ? (
          <div className="space-y-2 px-3 py-2">
            ...
            <Textarea
              ...
              placeholder={t("app.issueUi.documentAnnotationPanel.replyPlaceholder")}
              className="resize-y rounded-none text-sm"
            ...
            <div className="flex items-center justify-end gap-2">
              ...
              >
                {thread.status === "resolved" ? (
                  <>
                    <RotateCcw className="h-3 w-3" /> {t("app.issueUi.documentAnnotationPanel.reopen")}
                  </>
                ) : (
                  <>
                    <Check className="h-3 w-3" /> {t("app.issueUi.documentAnnotationPanel.resolve")}
                  </>
                )}
              </Button>
              <Button
              ...
              >
                {props.pendingReply ? t("app.issueUi.documentAnnotationPanel.sending") : t("app.issueUi.documentAnnotationPanel.reply")}
              </Button>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    aria-label={t("app.issueUi.documentAnnotationPanel.moreThreadActions")}
                  >
          ...
          <p className="px-3 py-2 text-xs text-muted-foreground">
            <span className="font-medium text-foreground">
              {thread.comments.length === 1
                ? t("app.issueUi.documentAnnotationPanel.oneComment", { count: 1 })
                : t("app.issueUi.documentAnnotationPanel.manyComments", { count: thread.comments.length })}
            </span>
    ...
    >
      <div className="mb-0.5 flex items-center justify-between gap-2 text-(length:--text-micro)">
        <span className="flex min-w-0 items-center gap-1.5">
          ...
          {author.role === "agent" ? (
            <span className="text-muted-foreground">{t("app.issueUi.documentAnnotationPanel.agentSuffix")}</span>
          ) : null}
        </span>
...
export function useCopyAnnotationLink() {
  ...
  return useCallback(
    (documentKey: string, threadId: string) => {
      ...
      void copyWithToast(`${window.location.origin}${pathname}${hash}`, t("app.issueUi.documentAnnotationPanel.linkCopied"));
    },
    [copyWithToast],
  );
}
