...
export function ThemeToggle({ className, variant = "icon", onAfterToggle }: ThemeToggleProps) {
  ...
  const label = isDark ? t("app.themeToggle.switchToLight") : t("app.themeToggle.switchToDark");
    ...
    return (
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block text-xs text-muted-foreground">{t("app.themeToggle.description")}</span>
        </span>
      </button>
    );
  }
