...
function ExternalObjectsSection({
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border p-3">
      <div className="space-y-1">
        <h3 className="text-sm font-semibold">{t("app.issueUi.issueRelatedWorkPanel.external.title")}</h3>
        <p className="text-xs text-muted-foreground">
          {t("app.issueUi.issueRelatedWorkPanel.external.description")}
        </p>
      </div>

      {isError ? (
        <p className="text-xs text-muted-foreground">
          {t("app.issueUi.issueRelatedWorkPanel.external.loadFailed")}{" "}
          {onRetry ? (
            <button
              type="button"
              onClick={onRetry}
        ...
        <p className="text-xs text-muted-foreground">{t("app.issueUi.issueRelatedWorkPanel.external.loading")}</p>
      ) : sorted.length === 0 ? (
        <p className="text-xs text-muted-foreground">
          {t("app.issueUi.issueRelatedWorkPanel.external.empty")}
        </p>
      ) : (
        <ul className="-mx-1 flex flex-col">
...
export function IssueRelatedWorkPanel({
  ...
  return (
    <div className="space-y-3">
      <Section
        title={t("app.issueUi.issueRelatedWorkPanel.references.title")}
        description={t("app.issueUi.issueRelatedWorkPanel.references.description")}
        items={outbound}
        emptyLabel={t("app.issueUi.issueRelatedWorkPanel.references.empty")}
      />
      ...
      <Section
        title={t("app.issueUi.issueRelatedWorkPanel.referencedBy.title")}
        description={t("app.issueUi.issueRelatedWorkPanel.referencedBy.description")}
        items={inbound}
        emptyLabel={t("app.issueUi.issueRelatedWorkPanel.referencedBy.empty")}
      />
    </div>
  );
}
