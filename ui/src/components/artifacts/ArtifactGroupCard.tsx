...
export function ArtifactGroupCard({ group, to }: ArtifactGroupCardProps) {
  ...
  const countLabel = t(group.count === 1 ? "app.shell.artifactGroupCard.countOne" : "app.shell.artifactGroupCard.countMany", { count: group.count });
      ...
      >
        ...
        <div className="flex flex-1 flex-col gap-1 p-3">
          ...
          <div className="mt-0.5 flex items-center gap-1.5 text-(length:--text-micro) text-muted-foreground/65">
            ...
            <span>{t("app.common.labels.updated")}{" "}{formatDate(group.updatedAt)}</span>
          </div>
        </div>
      </Link>
    </div>
