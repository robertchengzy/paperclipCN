...
export function MediaArtifactCard({ id, title, contentPath, contentType, originalFilename, downloadPath, detail, badge }: {
  ...
  return (
    <>
      <button
        type="button"
        aria-label={t("app.shell.mediaArtifactCard.openGallery", { value0: title })}
      ...
      >
        ...
        <span className="flex w-full flex-1 flex-col gap-1 p-2.5">
          ...
          <span className="flex flex-wrap items-center justify-between gap-1.5">
            <span className="text-xs text-muted-foreground">{detail ?? (mediaKind === "video" ? t("app.shell.mediaArtifactCard.video") : t("app.shell.mediaArtifactCard.image"))}</span>
            {badge}
          </span>
        </span>
      </button>
