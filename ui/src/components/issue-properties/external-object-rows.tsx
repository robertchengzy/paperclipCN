...
export function ExternalObjectRows({
  ...
  if (externalObjectsError) {
    return (
      <PropertyRow label={t("app.newIssue.properties.rows.externalObjects")}>
        <span className="text-xs text-muted-foreground">
          {t("app.newIssue.properties.externalObjects.loadFailed")}
          {onRetryExternalObjects ? (
            <>
              {" "}
              <button
  ...
  return (
    <>
      ...
      {expanded || hiddenExternalObjectCount > 0 ? (
        <PropertyRow label={t("app.newIssue.properties.rows.references")}>
