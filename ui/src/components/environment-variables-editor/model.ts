...
export function validateName(
  ...
  if (!ENV_NAME_RE.test(trimmed)) {
    return { level: "error", message: t("app.secrets.model.invalidName") };
  }
  if (duplicateNames.has(trimmed)) {
    return { level: "error", message: t("app.secrets.model.duplicateName") };
  }
  for (const prefix of reservedPrefixes) {
    if (prefix && trimmed.startsWith(prefix)) {
      return { level: "warn", message: t("app.secrets.model.reservedPrefix") };
    }
  }
  return null;
}
...
export function computeRowHealth(row: EnvRow, secrets: readonly CompanySecret[]): SecretHealth | null {
  ...
  if (!secret) {
    return {
      level: "error",
      kind: "missing",
      message: t("app.secrets.model.secretMissing"),
    };
  }
  if (secret.status !== "active") {
    return {
      level: "warn",
      kind: "disabled",
      message: t("app.secrets.model.secretDisabled"),
    };
  }
  return null;
}
...
export function computeUserSecretRowHealth(
  ...
  if (!definition) {
    return {
      level: "error",
      kind: "missing",
      message: t("app.secrets.model.userSecretMissing"),
    };
  }
  if (definition.status !== "active") {
    return {
      level: "warn",
      kind: "disabled",
      message: t("app.secrets.model.userSecretDisabled"),
    };
  }
  return null;
}
