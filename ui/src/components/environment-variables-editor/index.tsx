...
const DEFAULT_HINT_KEY = "app.secrets.index.defaultHint";
...
function formatChangedNames(names: readonly string[]): string {
  ...
  return names.length > CHANGE_SUMMARY_MAX_NAMES
    ? translate("app.secrets.index.namesMore", { names: shown, count: names.length - CHANGE_SUMMARY_MAX_NAMES })
    : shown;
}
...
export const EnvironmentVariablesEditor = forwardRef<EnvironmentVariablesEditorHandle, EnvironmentVariablesEditorProps>(function EnvironmentVariablesEditor({
  ...
  const changeSummaryText = useMemo(() => {
    const parts: string[] = [];
    if (changeSummary.added.length > 0) parts.push(t("app.secrets.index.summaryNew", { names: formatChangedNames(changeSummary.added) }));
    if (changeSummary.changed.length > 0) parts.push(t("app.secrets.index.summaryEdited", { names: formatChangedNames(changeSummary.changed) }));
    if (changeSummary.removed.length > 0) parts.push(t("app.secrets.index.summaryRemoved", { names: formatChangedNames(changeSummary.removed) }));
    return parts.join(" · ");
  }, [changeSummary, t]);
  ...
  function bulkImport(text: string, targetRowId: string): boolean {
    ...
    updateDraft(working);
    toast?.pushToast({
      title: pairs.length === 1 ? t("app.secrets.index.importedOneVariable") : t("app.secrets.index.importedVariables", { count: pairs.length }),
      tone: "success",
    });
    return true;
  }
  ...
  const hint = footerHint === undefined ? t(DEFAULT_HINT_KEY) : footerHint;
    ...
    <TooltipProvider>
      ...
      {attentionCount > 1 ? (
        <p className="inline-flex items-center gap-1.5 text-(length:--text-micro) font-medium text-amber-700 dark:text-amber-400">
          ...
          {t("app.secrets.index.bindingsNeedAttention", { count: attentionCount })}
        </p>
      ) : null}
      ...
      {hasRows ? (
        <>
          ...
          <div className="hidden gap-x-1.5 @[40rem]/env:grid @[40rem]/env:grid-cols-(--gtc-14)">
            <span className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("app.common.labels.name")}</span>
            <span className="text-(length:--text-micro) font-medium uppercase tracking-wide text-muted-foreground">{t("app.common.labels.value")}</span>
            <span />
          </div>
        ...
        <p className="text-sm text-muted-foreground">{t("app.secrets.index.noVariables")}</p>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
        ...
        >
          ...
          {t("app.secrets.index.addVariable")}
        </button>
        ...
        {quickBind.length > 0 && !disabled ? (
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-muted-foreground/70">
              ...
              {t("app.secrets.index.recentlyUsedLabel")}
            </span>
            {quickBind.map((secret) => (
              <button
                ...
                title={t("app.secrets.index.bindSecret", { name: secret.name })}
              >
                + {secret.name}
              </button>
            ))}
        ...
        >
          <div className="flex min-w-0 flex-col gap-0.5">
            <div className="flex items-center gap-2 text-sm font-medium">
              ...
              <span>{t("app.common.states.unsavedChanges")}</span>
            </div>
            {changeSummaryText ? (
      ...
      {rows.some((row) => row.source === "user_secret" && row.userSecretKey) ? (
        <p className="inline-flex items-start gap-1 text-(length:--text-micro) text-muted-foreground/70">
          ...
          <span>
            {t("app.secrets.index.userSecretsFooter")}
          </span>
        </p>
      ) : null}
      </div>
