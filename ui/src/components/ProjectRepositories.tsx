...
export function ProjectRepositories({ project }: { project: Project }) {
  ...
  return <section aria-label={translateCopy("app.issueUi.projectRepositories.repositories")} className="flex min-w-0 flex-col gap-4 py-4">
    ...
    <div className="flex flex-wrap items-center justify-end gap-2 border-t border-border pt-4">
      ...
      {save.isSuccess && <span role="status" className="mr-auto text-sm text-muted-foreground">{translateCopy("app.issueUi.projectRepositories.changesSaved")}</span>}
      {draft && <Button type="button" variant="ghost" disabled={save.isPending} onClick={() => { setDraft(null); save.reset(); }}>{translateCopy("app.issueUi.projectRepositories.discardChanges")}</Button>}
      <Button type="button" disabled={!draft || save.isPending} onClick={() => save.mutate()}>{save.isPending ? translateCopy("app.issueUi.projectRepositories.saving") : translateCopy("app.issueUi.projectRepositories.saveChanges")}</Button>
    </div>
    <Dialog open={connecting} onOpenChange={setConnecting}><DialogContent showCloseButton={false} aria-describedby={undefined} className="max-h-(--sz-calc-18) overflow-y-auto sm:max-w-2xl">
      <DialogTitle className="sr-only">{translateCopy("app.issueUi.projectRepositories.connectGitHub")}</DialogTitle>
