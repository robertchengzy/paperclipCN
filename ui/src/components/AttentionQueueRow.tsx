...
/** Snooze presets, resolved to a future ISO timestamp at click time. Labels resolve in the current UI language. */
function getSnoozePresets(): ReadonlyArray<{ id: string; label: string; resolve: () => string }> {
  return [
    ...
    { id: "tomorrow", label: t("app.issueUi.attentionQueueRow.snooze.tomorrowMorning"), resolve: tomorrowMorningIso },
...
export const AttentionQueueRow = memo(function AttentionQueueRow({
  ...
  const toggle = expandable ? (
    <button
      ...
      aria-label={expanded ? t("app.issueUi.attentionQueueRow.collapseDecision") : t("app.issueUi.attentionQueueRow.expandDecision")}
      aria-expanded={expanded}
      onClick={activate}
    >
      {expanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
      {expanded ? t("app.issueUi.attentionQueueRow.seeLess") : t("app.issueUi.attentionQueueRow.seeMore")}
    </button>
  ) : null;
          ...
          {item.decideBy && (
            <>
              ...
              <span
                ...
                title={decideByProvenance(item) ? t("app.issueUi.attentionQueueRow.setByTitle", { name: decideByProvenance(item) }) : undefined}
              >
                <CalendarClock className="h-3 w-3" />
                {decideByLabel(item.decideBy)}
                {decideByProvenance(item) && (
                  <span className="text-muted-foreground/80">{t("app.issueUi.attentionQueueRow.setByInline", { name: decideByProvenance(item) })}</span>
                )}
              </span>
            </>
          )}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  aria-label={t("app.issueUi.attentionQueueRow.rowActions")}
                >
                ...
                {href && (
                  <>
                    ...
                    <DropdownMenuItem asChild>
                      <Link to={href}>{t("app.issueUi.attentionQueueRow.openSource")}</Link>
                    </DropdownMenuItem>
                  </>
                )}
              </DropdownMenuContent>
...
function CompactDecisionActions({
  ...
  const decision = useMutation<unknown, Error, CompactDecisionAction>({
    mutationFn: (action: CompactDecisionAction) => {
      ...
      if (item.sourceKind === "issue_thread_interaction") {
        ...
        if (typeof issueId !== "string") throw new Error(t("app.issueUi.attentionQueueRow.missingIssueReference"));
      ...
      throw new Error(t("app.issueUi.attentionQueueRow.completeFromDetail"));
    },
    ...
    onError: (error, action) => {
      ...
      pushToast({
        title: decisionFailureLabel(action),
  ...
  return (
    <div className="flex w-full flex-wrap items-center gap-2 @xl:w-auto @xl:justify-end @xl:gap-1" aria-label={t("app.issueUi.attentionQueueRow.decisionActions")}>
...
function decisionFailureLabel(action: CompactDecisionAction): string {
  if (action === "request_revision") return t("app.issueUi.attentionQueueRow.failed.sentForRevision");
  if (action === "accept" || action === "approve") return t("app.issueUi.attentionQueueRow.failed.approved");
  return t("app.issueUi.attentionQueueRow.failed.rejected");
}

function compactDecisionSuccessLabel(sourceKind: AttentionItem["sourceKind"], action: CompactDecisionAction): string {
  const revision = action === "request_revision";
  const approved = action === "accept" || action === "approve";
  if (sourceKind === "approval") {
    return revision ? t("app.issueUi.attentionQueueRow.success.approvalSentForRevision") : approved ? t("app.issueUi.attentionQueueRow.success.approvalApproved") : t("app.issueUi.attentionQueueRow.success.approvalRejected");
  }
  if (sourceKind === "join_request") {
    return revision ? t("app.issueUi.attentionQueueRow.success.joinRequestSentForRevision") : approved ? t("app.issueUi.attentionQueueRow.success.joinRequestApproved") : t("app.issueUi.attentionQueueRow.success.joinRequestRejected");
  }
  return action === "accept" ? t("app.issueUi.attentionQueueRow.success.confirmationAccepted") : t("app.issueUi.attentionQueueRow.success.confirmationDeclined");
}
  ...
  return (
    <div className="flex flex-wrap items-stretch gap-2" data-attention-expanded-images="true">
      ...
      {extra > 0 && (issueHref ? (
        ...
        >
          <span className="text-base font-semibold">{t("app.issueUi.attentionQueueRow.moreImages", { count: extra })}</span>
          <span className="mt-0.5 inline-flex items-center gap-1 text-(length:--text-nano)">
            {t("app.issueUi.attentionQueueRow.viewIssue")}
            <ExternalLink className="h-3 w-3" />
          </span>
        </Link>
      ) : (
        <span className="flex h-32 w-24 items-center justify-center rounded-md border border-dashed border-border bg-muted/40 text-sm font-semibold text-muted-foreground">
          {t("app.issueUi.attentionQueueRow.moreImages", { count: extra })}
        </span>
      ))}
    </div>
  );
      ...
      <DropdownMenuSubContent>
        ...
        >
          ...
          <Button type="button" size="xs" disabled={!customValue} onClick={applyCustom}>
            {t("app.issueUi.attentionQueueRow.snooze.until")}
          </Button>
        </div>
      </DropdownMenuSubContent>
    </DropdownMenuSub>
...
function decideByProvenance(item: AttentionItem): string | null {
  ...
  if (attribution.type === "agent") return attribution.agentName ?? t("app.issueUi.attentionQueueRow.provenance.anAgent");
  return t("app.issueUi.attentionQueueRow.provenance.you");
}
...
function reappearLabel(snoozedUntil: string): string {
  ...
  if (!Number.isFinite(diffMs) || diffMs <= 0) return t("app.issueUi.attentionQueueRow.reappear.soon");
  const diffMin = Math.round(diffMs / 60000);
  if (diffMin < 60) return t("app.issueUi.attentionQueueRow.reappear.minutes", { count: diffMin });
  const diffHr = Math.round(diffMin / 60);
  if (diffHr < 24) return t("app.issueUi.attentionQueueRow.reappear.hours", { count: diffHr });
  const diffDay = Math.round(diffHr / 24);
  return t("app.issueUi.attentionQueueRow.reappear.days", { count: diffDay });
}
...
function InlineResolver({
  ...
  if (item.sourceKind === "issue_thread_interaction") {
    ...
    if (!issueId) {
      return <p className="text-xs text-muted-foreground">{t("app.issueUi.attentionQueueRow.missingIssueReference")}</p>;
    }
    return (
      <>
  ...
  return (
    <>
      <Textarea
        ...
        placeholder={t("app.issueUi.attentionQueueRow.optionalNote")}
      ...
      <ResolverFooter toggle={toggle}>
        <Button size="sm" variant="outline" onClick={() => revise.mutate()} disabled={pending}>
          ...
          {t("app.issueUi.attentionQueueRow.requestRevision")}
        </Button>
