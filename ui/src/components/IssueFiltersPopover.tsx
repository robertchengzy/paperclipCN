...
const INBOX_CATEGORY_OPTIONS: ReadonlyArray<InboxCategoryFilter> = [
  "everything",
  "issues_i_touched",
  "join_requests",
  "approvals",
...
const INBOX_APPROVAL_STATUS_OPTIONS: ReadonlyArray<InboxApprovalFilter> = [
  "all",
  "actionable",
  "resolved",
];

function inboxCategoryLabel(t: TFunction, value: InboxCategoryFilter): string {
  switch (value) {
    case "everything": return t("app.issues.filters.inboxCategory.everything");
    case "issues_i_touched": return t("app.issues.filters.inboxCategory.issuesITouched");
    case "join_requests": return t("app.issues.filters.inboxCategory.joinRequests");
    case "approvals": return t("app.issues.filters.inboxCategory.approvals");
    case "failed_runs": return t("app.issues.filters.inboxCategory.failedRuns");
    case "alerts": return t("app.issues.filters.inboxCategory.alerts");
    default: return value;
  }
}

function inboxApprovalStatusLabel(t: TFunction, value: InboxApprovalFilter): string {
  switch (value) {
    case "all": return t("app.issues.filters.inboxApproval.all");
    case "actionable": return t("app.issues.filters.inboxApproval.actionable");
    case "resolved": return t("app.issues.filters.inboxApproval.resolved");
    default: return value;
  }
}

function quickFilterPresetLabel(t: TFunction, label: string): string {
  switch (label) {
    case "All": return t("app.issues.filters.quickPreset.all");
    case "Active": return t("app.issues.filters.quickPreset.active");
    case "Backlog": return t("app.issues.filters.quickPreset.backlog");
    case "Done": return t("app.issues.filters.quickPreset.done");
    default: return label;
  }
}

function externalObjectStatusFilterLabel(t: TFunction, value: string): string {
  switch (value) {
    case "failed": return t("app.issues.filters.externalObject.failed");
    case "waiting": return t("app.issues.filters.externalObject.waiting");
    case "running": return t("app.issues.filters.externalObject.running");
    case "auth_required": return t("app.issues.filters.externalObject.authRequired");
    case "unreachable": return t("app.issues.filters.externalObject.unreachable");
    case "stale": return t("app.issues.filters.externalObject.stale");
    case "none": return t("app.issues.filters.externalObject.none");
    default: return externalObjectFilterLabel(value);
  }
}
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? t("app.issues.filters.filtersCount", { count: activeFilterCount }) : t("app.issues.filters.filter")) : undefined}>
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{activeFilterCount > 0 ? t("app.issues.filters.filtersCount", { count: activeFilterCount }) : t("app.issues.filters.filter")}</span>}
      ...
      >
        <div className="space-y-3 p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">{t("app.issues.filters.title")}</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
            ...
            <>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="space-y-1" data-filter-options="inbox-category">
                  <span className="text-xs text-muted-foreground">{t("app.issues.filters.category")}</span>
                  <div className="space-y-0.5">
                    {INBOX_CATEGORY_OPTIONS.map((value) => {
                      const label = inboxCategoryLabel(t, value);
                ...
                {inboxScopeFilters.showApprovalStatus ? (
                  <div className="space-y-1" data-filter-options="inbox-approval-status">
                    <span className="text-xs text-muted-foreground">{t("app.issues.filters.approvalStatus")}</span>
                    <div className="space-y-0.5">
                      {INBOX_APPROVAL_STATUS_OPTIONS.map((value) => {
                        const label = inboxApprovalStatusLabel(t, value);
          ...
          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("app.issues.filters.quickFilters")}</span>
            <div className="flex flex-wrap gap-1.5">
              {issueQuickFilterPresets.map((preset) => {
                ...
                return (
                  ...
                  >
                    {quickFilterPresetLabel(t, preset.label)}
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("app.issues.filters.status")}</span>
                <div className="space-y-0.5">
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("app.issues.filters.priority")}</span>
                <div className="space-y-0.5">
                  {issuePriorityOrder.map((priority) => (
                    <label key={priority} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{priorityLabel(t, priority)}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("app.issues.filters.responsible")}</span>
                {streamlined && (agents?.length ?? 0) + (currentUserId ? 2 : 1) > SEARCHABLE_FILTER_THRESHOLD ? (
                  <FilterOptionSearch value={assigneeSearch} onChange={setAssigneeSearch} placeholder={t("app.issues.filters.searchPlaceholder.responsible")} ariaLabel={t("app.issues.filters.searchAria.responsible")} />
                ) : null}
                <div data-filter-options="responsible" className={streamlined ? "space-y-0.5" : "max-h-32 space-y-0.5 overflow-y-auto"}>
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("app.issues.filters.noResponsible")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("app.issues.filters.me")}</span>
                    </label>
                  ) : null}
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("app.issues.filters.creator")}</span>
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("app.issues.filters.removeCreator", { name: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("app.issues.filters.searchCreators")}
                      className="h-8 pl-7 text-xs"
                    />
                  </div>
                  <div data-filter-options="creators" className={streamlined ? "space-y-0.5" : "max-h-32 space-y-0.5 overflow-y-auto"}>
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("app.issues.filters.noCreatorsMatch")}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("app.issues.filters.project")}</span>
                  {streamlined && projects.length > SEARCHABLE_FILTER_THRESHOLD ? (
                    <FilterOptionSearch value={projectSearch} onChange={setProjectSearch} placeholder={t("app.issues.filters.searchPlaceholder.projects")} ariaLabel={t("app.issues.filters.searchAria.projects")} />
                  ) : null}
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("app.issues.filters.labels")}</span>
                  {streamlined && labels.length > SEARCHABLE_FILTER_THRESHOLD ? (
                    <FilterOptionSearch value={labelSearch} onChange={setLabelSearch} placeholder={t("app.issues.filters.searchPlaceholder.labels")} ariaLabel={t("app.issues.filters.searchAria.labels")} />
                  ) : null}
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("app.issues.filters.workspace")}</span>
                  {streamlined && workspaces.length > SEARCHABLE_FILTER_THRESHOLD ? (
                    <FilterOptionSearch value={workspaceSearch} onChange={setWorkspaceSearch} placeholder={t("app.issues.filters.searchPlaceholder.workspaces")} ariaLabel={t("app.issues.filters.searchAria.workspaces")} />
                  ) : null}
              ...
              {enableExternalObjectFilters ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("app.issues.filters.externalObjectStatus")}</span>
                  <div className="space-y-0.5">
                    {externalObjectFilterOrder.map((value) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-sm">{externalObjectStatusFilterLabel(t, value)}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("app.issues.filters.visibility")}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{t("app.issues.filters.liveRunsOnly")}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("app.issues.filters.hideRoutineRuns")}</span>
                  </label>
                ) : null}
              </div>
            </div>
